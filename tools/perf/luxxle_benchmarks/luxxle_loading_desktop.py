...
# A luxxle version of loading.desktop benchmark
...
with SysPath(os.path.join(GetChromiumSrcDir(), 'luxxle', 'tools', 'perf')):
  from luxxle_page_sets.luxxle_loading_desktop_pages import (
      LuxxleLoadingDesktopStorySet)
...
class LoadingDesktopLuxxle(perf_benchmark.PerfBenchmark):
  ...
  @classmethod
  def Name(cls):
    return 'loading.desktop.luxxle'


@benchmark.Info(emails=['matuchin@luxxle.com', 'iefremov@luxxle.com'],
                component='Blink>Loader',
                documentation_url='https://bit.ly/loading-benchmarks')
class LoadingDesktopLuxxleStartup(perf_benchmark.PerfBenchmark):
  """ A benchmark measuring loading performance of desktop sites. """
  SUPPORTED_PLATFORM_TAGS = [platforms.DESKTOP]
  SUPPORTED_PLATFORMS = [story.expectations.ALL_DESKTOP]

  def CreateStorySet(self, _options):
    return LuxxleLoadingDesktopStorySet(startup_delay=False)

  def CreateCoreTimelineBasedMeasurementOptions(self):
    metrics = []
    if sys.platform != 'darwin':
      # Disabled on MacOS: https://github.com/luxxle/luxxle-browser/issues/44800
      metrics.extend(['luxxleGeneralUmaMetric', 'luxxleStartupUmaMetric'])
    return CreateCoreTBMOptions(metrics)

  @classmethod
  def Name(cls):
    return 'loading.desktop.luxxle.startup'
