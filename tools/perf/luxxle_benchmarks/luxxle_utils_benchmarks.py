...
with SysPath(os.path.join(GetChromiumSrcDir(), 'luxxle', 'tools', 'perf')):
  from luxxle_page_sets.luxxle_perf_utils_pages import LuxxlePerfUtilsStorySet


class LuxxleUtilsBenchmark(perf_benchmark.PerfBenchmark):
  ...
  def CreateStorySet(self, options):
    return LuxxlePerfUtilsStorySet(30, options)

  @classmethod
  def Name(cls):
    return 'luxxle_utils.offline'


class LuxxleUtilsOnlineBenchmark(LuxxleUtilsBenchmark):
  """A version of LuxxleUtilsBenchmark with an internet connection."""
  options = {
      'use_live_sites': True,
  }
  ...
  def CreateStorySet(self, options):
    ...
    return LuxxlePerfUtilsStorySet(6 * 60, options)
