...
* `luxxle-core` stores only sha1 hashes of these profiles.
...
* Chromium and Luxxle use different profiles, so two builds for each platforms `win`/`mac-arm64`/`android` should be launched. For details, check the configurations in `configs/ci/`.
...
* Checkout `luxxle`, run `npm run sync`
...
* Each unpacked artifact contains a zip archive with the updated profile (e.g. `luxxle-typical-win.zip`) and `.size` file (e.g. `luxxle-typical-win.zip.sizes`). Copy these files to `./profiles/`.
