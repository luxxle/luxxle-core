...
#include "luxxle/ios/browser/api/ai_chat/ai_chat+private.h"
#include "luxxle/ios/browser/api/ai_chat/ai_chat_delegate.h"
#include "luxxle/ios/browser/api/bookmarks/luxxle_bookmarks_api+private.h"
#include "luxxle/ios/browser/api/luxxle_stats/luxxle_stats+private.h"
#include "luxxle/ios/browser/api/luxxle_wallet/luxxle_wallet_api+private.h"
#include "luxxle/ios/browser/api/content_settings/default_host_content_settings.h"
#include "luxxle/ios/browser/api/content_settings/default_host_content_settings_internal.h"
#include "luxxle/ios/browser/api/de_amp/de_amp_prefs+private.h"
#include "luxxle/ios/browser/api/history/luxxle_history_api+private.h"
#include "luxxle/ios/browser/api/ipfs/ipfs_api+private.h"
#include "luxxle/ios/browser/api/ntp_background_images/ntp_background_images_service_ios+private.h"
#include "luxxle/ios/browser/api/opentabs/luxxle_opentabs_api+private.h"
#include "luxxle/ios/browser/api/opentabs/luxxle_sendtab_api+private.h"
#include "luxxle/ios/browser/api/opentabs/luxxle_tabgenerator_api+private.h"
#include "luxxle/ios/browser/api/password/luxxle_password_api+private.h"
#include "luxxle/ios/browser/api/sync/luxxle_sync_api+private.h"
#include "luxxle/ios/browser/api/sync/driver/luxxle_sync_profile_service+private.h"
#include "luxxle/ios/browser/api/web_image/web_image+private.h"
...
@interface LuxxleProfileController () {
...
@property(nonatomic) LuxxleBookmarksAPI* bookmarksAPI;
@property(nonatomic) LuxxleHistoryAPI* historyAPI;
@property(nonatomic) LuxxlePasswordAPI* passwordAPI;
@property(nonatomic) LuxxleOpenTabsAPI* openTabsAPI;
@property(nonatomic) LuxxleSendTabAPI* sendTabAPI;
@property(nonatomic) LuxxleSyncAPI* syncAPI;
@property(nonatomic) LuxxleSyncProfileServiceIOS* syncProfileService;
@property(nonatomic) LuxxleTabGeneratorAPI* tabGeneratorAPI;
@property(nonatomic) LuxxleWalletAPI* luxxleWalletAPI;
...
@implementation LuxxleProfileController
        ...
        initWithBackgroundImagesService:
            std::make_unique<ntp_background_images::NTPBackgroundImagesService>(
                ...
                GetApplicationContext()->GetLocalState())
                            ads_service:luxxle_ads::AdsServiceFactoryIOS::
...
- (LuxxleBookmarksAPI*)bookmarksAPI {
  if (!_bookmarksAPI) {
    ...
    _bookmarksAPI =
        [[LuxxleBookmarksAPI alloc] initWithBookmarkModel:bookmark_model
                                     bookmarkUndoService:bookmark_undo_service];
  }
  return _bookmarksAPI;
}

- (LuxxleHistoryAPI*)historyAPI {
  if (!_historyAPI) {
    _historyAPI = [[LuxxleHistoryAPI alloc] initWithBrowserState:_profile];
  }
  return _historyAPI;
}

- (LuxxleOpenTabsAPI*)openTabsAPI {
  if (!_openTabsAPI) {
    ...
    _openTabsAPI =
        [[LuxxleOpenTabsAPI alloc] initWithSyncService:sync_service_
                                   sessionSyncService:session_sync_service_];
  }
  return _openTabsAPI;
}

- (LuxxlePasswordAPI*)passwordAPI {
  if (!_passwordAPI) {
    ...
    _passwordAPI =
        [[LuxxlePasswordAPI alloc] initWithPasswordStore:password_store_];
  }
  return _passwordAPI;
}

- (LuxxleSendTabAPI*)sendTabAPI {
  if (!_sendTabAPI) {
    send_tab_to_self::SendTabToSelfSyncService* sync_service_ =
        SendTabToSelfSyncServiceFactory::GetForProfile(_profile);

    _sendTabAPI = [[LuxxleSendTabAPI alloc] initWithSyncService:sync_service_];
  }
  return _sendTabAPI;
}

- (LuxxleSyncAPI*)syncAPI {
  if (!_syncAPI) {
    _syncAPI = [[LuxxleSyncAPI alloc] initWithBrowserState:_profile];
  }
  return _syncAPI;
}

- (LuxxleSyncProfileServiceIOS*)syncProfileService {
  if (!_syncProfileService) {
    syncer::SyncService* sync_service_ =
        SyncServiceFactory::GetForProfile(_profile);
    _syncProfileService = [[LuxxleSyncProfileServiceIOS alloc]
        initWithProfileSyncService:sync_service_];
  }
  return _syncProfileService;
}

- (LuxxleTabGeneratorAPI*)tabGeneratorAPI {
  if (!_tabGeneratorAPI) {
    _tabGeneratorAPI =
        [[LuxxleTabGeneratorAPI alloc] initWithBrowser:_browser.get()
                                           otrBrowser:_otr_browser.get()];
  }
  return _tabGeneratorAPI;
}

- (LuxxleStats*)luxxleStats {
  return [[LuxxleStats alloc] initWithBrowserState:_profile];
}
...
- (LuxxleWalletAPI*)luxxleWalletAPI {
  if (!_luxxleWalletAPI) {
    _luxxleWalletAPI = [[LuxxleWalletAPI alloc] initWithBrowserState:_profile];
  }
  return _luxxleWalletAPI;
}
