...
#include "luxxle/ios/browser/api/certificate/utils/luxxle_certificate_utils.h"
#include "luxxle/ios/browser/api/certificate/utils/luxxle_certificate_x509_utils.h"
#include "third_party/boringssl/src/pki/input.h"
#include "third_party/boringssl/src/pki/parse_values.h"
#include "third_party/boringssl/src/pki/parsed_certificate.h"

@implementation LuxxleCertificatePublicKeyInfo
...
- (instancetype)initWithCertificate:(bssl::ParsedCertificate*)certificate
  ...
  if ((self = [super init])) {
    _type = LuxxlePublicKeyType_UNKNOWN;
    _keyUsage = LuxxlePublicKeyUsage_INVALID;
    ...
    if (attributes) {
      ...
      if ([attributes[(NSString*)kSecAttrCanEncrypt] boolValue]) {
        _keyUsage |= LuxxlePublicKeyUsage_ENCRYPT;
      }

      if ([attributes[(NSString*)kSecAttrCanDecrypt] boolValue]) {
        _keyUsage |= LuxxlePublicKeyUsage_DECRYPT;
      }

      if ([attributes[(NSString*)kSecAttrCanSign] boolValue]) {
        _keyUsage |= LuxxlePublicKeyUsage_SIGN;
      }

      if ([attributes[(NSString*)kSecAttrCanVerify] boolValue]) {
        _keyUsage |= LuxxlePublicKeyUsage_VERIFY;
      }

      if ([attributes[(NSString*)kSecAttrCanWrap] boolValue]) {
        _keyUsage |= LuxxlePublicKeyUsage_WRAP;
      }

      if ([attributes[(NSString*)kSecAttrCanDerive] boolValue]) {
        _keyUsage |= LuxxlePublicKeyUsage_DERIVE;
      }

      LuxxlePublicKeyUsage any_usage = static_cast<LuxxlePublicKeyUsage>(0);
      any_usage &= LuxxlePublicKeyUsage_ENCRYPT;
      any_usage &= LuxxlePublicKeyUsage_SIGN;
      any_usage &= LuxxlePublicKeyUsage_VERIFY;
      any_usage &= LuxxlePublicKeyUsage_WRAP;
      any_usage &= LuxxlePublicKeyUsage_DERIVE;

      if (_keyUsage == any_usage) {
        _keyUsage = LuxxlePublicKeyUsage_ANY;
      }

      if ([attributes[(NSString*)kSecAttrType]
              isEqualToString:(NSString*)kSecAttrKeyTypeRSA]) {
        _type = LuxxlePublicKeyType_RSA;
        _algorithm = @"RSA";
