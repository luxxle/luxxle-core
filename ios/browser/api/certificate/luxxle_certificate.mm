...
#include "luxxle/ios/browser/api/certificate/luxxle_certificate.h"
...
#include "luxxle/ios/browser/api/certificate/models/luxxle_certificate_model+private.h"
...
@interface LuxxleCertificateModel () {
...
@implementation LuxxleCertificateModel
...
- (void)parseCertificate {
  ...
  _subjectName = [[LuxxleCertificateRDNSequence alloc]
      initWithBERName:extended_cert_->tbs().subject_tlv
             uniqueId:subject_unique_id];
  _issuerName = [[LuxxleCertificateRDNSequence alloc]
  ...
  _version = static_cast<std::int32_t>(extended_cert_->tbs().version) +
             1;  // version + 1
  _signature = [[LuxxleCertificateSignature alloc]
  ...
  _notValidAfter = certificate::x509_utils::GeneralizedTimeToTime(
                       ...
                       .ToNSDate();
  _publicKeyInfo = [[LuxxleCertificatePublicKeyInfo alloc]
      initWithCertificate:extended_cert_.get()
                  withKey:public_key_.get()];
  //  _extensions = nullptr;
  _sha1Fingerprint = [[LuxxleCertificateFingerprint alloc]
      initWithCertificate:cert_data_.get()
                 withType:LuxxleFingerprintType_SHA1];
  _sha256Fingerprint = [[LuxxleCertificateFingerprint alloc]
      initWithCertificate:cert_data_.get()
                 withType:LuxxleFingerprintType_SHA256];
