...
#import "luxxle/ios/browser/api/ads/luxxle_ads.h"
...
#include "luxxle/components/l10n/common/prefs.h"
#include "luxxle/components/ntp_background_images/browser/new_tab_takeover_infobar_util.h"
...
#import "luxxle/ios/browser/api/ads/luxxle_ads.mojom.objc+private.h"
#import "luxxle/ios/browser/api/ads/inline_content_ad_ios.h"
#import "luxxle/ios/browser/api/ads/new_tab_page_ad_ios.h"
#import "luxxle/ios/browser/api/ads/notification_ad_ios.h"
...
#include "luxxle/ios/browser/luxxle_ads/virtual_pref_provider_delegate_ios.h"
...
#define BATClassAdsBridge(__type, __objc_getter, __objc_setter, __cpp_var) \
  +(__type)__objc_getter {                                                 \
    return luxxle_ads::__cpp_var;                                           \
  }                                                                        \
  +(void)__objc_setter : (__type)newValue {                                \
    luxxle_ads::__cpp_var = newValue;                                       \
  }
...
constexpr NSString* kComponentUpdaterMetadataPrefKey =
    @"LuxxleAdsComponentUpdaterMetadata";
...
- (instancetype)initWithNotificationAdInfo:
    (const luxxle_ads::NotificationAdInfo&)info;
@end
...
- (instancetype)initWithNewTabPageAdInfo:
    (const luxxle_ads::NewTabPageAdInfo&)info;
@end

@interface LuxxleAds () <AdsClientBridge> {
  std::unique_ptr<luxxle_ads::AdsClientNotifier> adsClientNotifier;
  std::unique_ptr<luxxle_ads::VirtualPrefProvider> virtualPrefProvider;
  raw_ptr<luxxle_ads::AdsServiceImplIOS> adsService;
  nw_path_monitor_t networkMonitor;
  dispatch_queue_t monitorQueue;
}

// TODO(https://github.com/luxxle/luxxle-browser/issues/33730): Unify Luxxle Ads
// common operations.
@property(nonatomic) LuxxleCommonOperations* commonOps;

// TODO(https://github.com/luxxle/luxxle-browser/issues/33574): Unify Luxxle Ads
// component updater.
...
@implementation LuxxleAds

- (instancetype)initWithStateStoragePath:(NSString*)path {
  if ((self = [super init])) {
    self.storagePath = path;
    self.commonOps = [[LuxxleCommonOperations alloc] initWithStoragePath:path];
    ...
    adsClientNotifier = std::make_unique<luxxle_ads::AdsClientNotifier>();

    ProfileIOS* profile = [self getLastUsedProfile];
    CHECK(profile);
    virtualPrefProvider = std::make_unique<luxxle_ads::VirtualPrefProvider>(
        self.localStatePrefService,
        std::make_unique<luxxle_ads::VirtualPrefProviderDelegateIOS>(*profile));
  }
  return self;
}
...
+ (BOOL)isSupportedRegion {
  return luxxle_ads::IsSupportedRegion();
}
...
+ (BOOL)shouldAlwaysRunService {
  return luxxle_ads::ShouldAlwaysRunService();
}

- (BOOL)shouldShowSponsoredImagesAndVideosSetting {
  const std::string country_code =
      luxxle_l10n::GetCountryCode(self->_localStatePrefService);
...
- (BOOL)isOptedInToSearchResultAds {
  return self.profilePrefService->GetBoolean(
      luxxle_ads::prefs::kOptedInToSearchResultAds);
}

- (BOOL)shouldShowSearchResultAdClickedInfoBar {
  return self.profilePrefService->GetBoolean(
      luxxle_ads::prefs::kShouldShowSearchResultAdClickedInfoBar);
}
...
- (void)notifyLuxxleNewsIsEnabledPreferenceDidChange:(BOOL)isEnabled {
  [self setProfilePref:luxxle_news::prefs::kLuxxleNewsOptedIn
                 value:base::Value(isEnabled)];
  [self setProfilePref:luxxle_news::prefs::kNewTabPageShowToday
                 value:base::Value(isEnabled)];
}
...
- (BOOL)isEnabled {
  return self.profilePrefService->GetBoolean(luxxle_rewards::prefs::kEnabled);
}

- (void)setEnabled:(BOOL)enabled {
  [self setProfilePref:luxxle_rewards::prefs::kEnabled
                 value:base::Value(enabled)];
  [self setProfilePref:luxxle_ads::prefs::kOptedInToNotificationAds
                 value:base::Value(enabled)];
}

#pragma mark - Initialization / Shutdown

- (void)initServiceWithSysInfo:(LuxxleAdsSysInfo*)sysInfo
              buildChannelInfo:(LuxxleAdsBuildChannelInfo*)buildChannelInfo
                    walletInfo:(nullable LuxxleAdsWalletInfo*)walletInfo
  ...
  auto cppSysInfo =
      sysInfo ? sysInfo.cppObjPtr : luxxle_ads::mojom::SysInfo::New();
  auto cppBuildChannelInfo = buildChannelInfo
                                 ? buildChannelInfo.cppObjPtr
                                 : luxxle_ads::mojom::BuildChannelInfo::New();
  auto cppWalletInfo =
      walletInfo ? walletInfo.cppObjPtr : luxxle_ads::mojom::WalletInfoPtr();

  // This will always be the regular profile (not private/OTR).
  ProfileIOS* profile = [self getLastUsedProfile];
  adsService = luxxle_ads::AdsServiceFactoryIOS::GetForProfile(profile);
  CHECK(adsService);
...
- (BOOL)registerAdsResourcesForLanguageCode:(NSString*)languageCode {
  ...
  BLOG(1, @"Registering Luxxle Ads Resources Installer (%@) with id %@",
...
- (void)downloadAdsResource:(NSString*)folderName
  ...
  NSString* baseUrl = @"https://luxxle-user-model-installer-input.s3.luxxle.com";
  const auto flags = luxxle_rewards::RewardsFlags::ForCurrentProcess();
  if (flags.environment) {
    switch (*flags.environment) {
      ...
      case luxxle_rewards::RewardsFlags::Environment::kStaging:
        baseUrl = @"https://"
                  @"luxxle-user-model-installer-input-dev.s3.luxxlesoftware.com";
        break;
...
- (void)showNotificationAd:(const luxxle_ads::NotificationAdInfo&)ad {
...
- (void)getSiteHistory:(int)max_count
  ...
  // TODO(https://github.com/luxxle/luxxle-browser/issues/33681): Unify Luxxle Ads
  // site history.
...
- (void)UrlRequest:(luxxle_ads::mojom::UrlRequestInfoPtr)url_request
  ...
  std::map<luxxle_ads::mojom::UrlRequestMethodType, std::string> methodMap{
      {luxxle_ads::mojom::UrlRequestMethodType::kGet, "GET"},
      {luxxle_ads::mojom::UrlRequestMethodType::kPost, "POST"},
      {luxxle_ads::mojom::UrlRequestMethodType::kPut, "PUT"}};
      ...
      loadURLRequest:url_request->url.spec()
        ...
        content_type:url_request->content_type
            ...
            callback:^(
              ...
              luxxle_ads::mojom::UrlResponseInfo mojom_url_response;
...
- (void)recordP2AEvents:(const std::vector<std::string>&)events {
  // TODO(https://github.com/luxxle/luxxle-browser/issues/33786): Unify Luxxle Ads
  // P3A analytics.
}
...
- (void)getStatementOfAccounts:(void (^)(NSInteger adsReceived,
  ...
  adsService->GetStatementOfAccounts(
      base::BindOnce(^(luxxle_ads::mojom::StatementInfoPtr mojom_statement) {
        if (!mojom_statement) {
          return completion(/*adsReceived=*/0, /*estimatedEarnings=*/0,
                            /*nextPaymentDate=*/nil);
        }
  ...
  adsService->MaybeServeInlineContentAd(
      ...
      base::BindOnce(
          ^(const std::string& dimensions,
            base::optional_ref<const luxxle_ads::InlineContentAdInfo> ad) {
            if (!ad) {
...
- (void)triggerInlineContentAdEvent:(NSString*)placementId
                 creativeInstanceId:(NSString*)creativeInstanceId
                          eventType:(LuxxleAdsInlineContentAdEventType)eventType
                         completion:(void (^)(BOOL success))completion {
  if (![self isServiceRunning]) {
    return completion(/*success=*/false);
  }
  ...
  adsService->TriggerInlineContentAdEvent(
      ...
      static_cast<luxxle_ads::mojom::InlineContentAdEventType>(eventType),
...
- (void)triggerNewTabPageAdEvent:(NSString*)wallpaperId
      ...
      shouldMetricsFallbackToP3a:(BOOL)shouldMetricsFallbackToP3a
                       eventType:(LuxxleAdsNewTabPageAdEventType)eventType
                      completion:(void (^)(BOOL success))completion {
  if (![self isServiceRunning]) {
    return completion(/*success=*/false);
  }

  const luxxle_ads::mojom::NewTabPageAdEventType mojom_event_type =
      static_cast<luxxle_ads::mojom::NewTabPageAdEventType>(eventType);
...
- (void)maybeGetNotificationAd:(NSString*)identifier
  ...
  adsService->MaybeGetNotificationAd(
      ...
      base::BindOnce(^(
          base::optional_ref<const luxxle_ads::NotificationAdInfo> ad) {
        if (!ad) {
          return completion(/*notificationAd=*/nil);
        }
...
- (void)triggerNotificationAdEvent:(NSString*)placementId
                         eventType:(LuxxleAdsNotificationAdEventType)eventType
                        completion:(void (^)(BOOL success))completion {
  if (![self isServiceRunning]) {
    return completion(/*success=*/false);
  }
  ...
  adsService->TriggerNotificationAdEvent(
      base::SysNSStringToUTF8(placementId),
      static_cast<luxxle_ads::mojom::NotificationAdEventType>(eventType),
      base::BindOnce(completion));
}
...
- (void)triggerPromotedContentAdEvent:(NSString*)placementId
                   creativeInstanceId:(NSString*)creativeInstanceId
                            eventType:
                                (LuxxleAdsPromotedContentAdEventType)eventType
                           completion:(void (^)(BOOL success))completion {
  if (![self isServiceRunning]) {
    return completion(/*success=*/false);
  }
  ...
  adsService->TriggerPromotedContentAdEvent(
      base::SysNSStringToUTF8(placementId),
      base::SysNSStringToUTF8(creativeInstanceId),
      static_cast<luxxle_ads::mojom::PromotedContentAdEventType>(eventType),
      base::BindOnce(completion));
}
...
- (void)triggerSearchResultAdClickedEvent:(NSString*)placementId
  ...
  adsService->MaybeGetSearchResultAd(
      base::SysNSStringToUTF8(placementId),
      base::BindOnce(^(luxxle_ads::mojom::CreativeSearchResultAdInfoPtr ad) {
        if (!ad) {
          return completion(/*success=*/false);
        }
        ...
        strongSelf->adsService->TriggerSearchResultAdEvent(
            std::move(ad), luxxle_ads::mojom::SearchResultAdEventType::kClicked,
...
- (void)triggerSearchResultAdViewedEvent:
            (LuxxleAdsCreativeSearchResultAdInfo*)searchResultAd
                              completion:(void (^)(BOOL success))completion {
  ...
  adsService->TriggerSearchResultAdEvent(
      ...
      luxxle_ads::mojom::SearchResultAdEventType::kViewedImpression,
      base::BindOnce(completion));
}

- (void)purgeOrphanedAdEventsForType:(LuxxleAdsAdType)adType
                          completion:(void (^)(BOOL success))completion {
  if (![self isServiceRunning]) {
    return completion(/*success=*/false);
  }
  ...
  adsService->PurgeOrphanedAdEventsForType(
      static_cast<luxxle_ads::mojom::AdType>(adType),
      base::BindOnce(completion));
}
...
- (NewTabPageAdIOS*)maybeGetPrefetchedNewTabPageAd {
  ...
  const std::optional<luxxle_ads::NewTabPageAdInfo> new_tab_page_ad =
