...
#include "luxxle/ios/browser/api/history/history_driver_ios.h"
...
#pragma mark - LuxxleHistoryAPI

@interface LuxxleHistoryAPI () {
...
@implementation LuxxleHistoryAPI {
...
- (void)removeAllWithCompletion:(void (^)())completion {
  ...
  __weak LuxxleHistoryAPI* weak_history_api = self;
  auto delete_history = ^(void (^callback)()) {
    LuxxleHistoryAPI* historyAPI = weak_history_api;
    if (!historyAPI) {
      callback();
      return;
    }
