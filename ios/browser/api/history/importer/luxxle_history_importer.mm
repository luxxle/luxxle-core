...
#include "luxxle/ios/browser/api/history/importer/luxxle_history_importer.h"
...
#include "luxxle/ios/browser/api/history/importer/history_json_reader.h"
...
@implementation LuxxleImportedHistory
...
- (void)importFromFile:(NSString*)filePath
  ...
  __weak LuxxleHistoryImporter* weakSelf = self;

  auto start_import = ^{
    // Import cancelled as the importer has been deallocated
    __strong LuxxleHistoryImporter* importer = weakSelf;
    if (!importer) {
      listener(LuxxleHistoryImporterStateStarted, nullptr);
      listener(LuxxleHistoryImporterStateCancelled, nullptr);
      return;
    }
        ...
        web::GetUIThreadTaskRunner({})->PostTask(
            FROM_HERE, base::BindOnce(
                           ^(std::vector<history::URLRow> items) {
                             ...
                             listener(LuxxleHistoryImporterStateAutoCompleted,
                                      nullptr);
                           },
                           std::move(history_items)));
      } else {
        listener(LuxxleHistoryImporterStateCompleted,
...
- (void)importFromArray:(NSArray<LuxxleImportedHistory*>*)historyItems
  ...
  auto start_import = ^{
    ...
    __strong LuxxleHistoryImporter* importer = weakSelf;
    if (!importer) {
      listener(LuxxleHistoryImporterStateStarted);
      listener(LuxxleHistoryImporterStateCancelled);
      return;
    }
...
- (NSArray<LuxxleImportedHistory*>*)convertToIOSImportedHistory:
    (const std::vector<history::URLRow>&)historyItems {
  NSMutableArray<LuxxleImportedHistory*>* results =
      [[NSMutableArray alloc] init];
  for (const auto& historyItem : historyItems) {
    LuxxleImportedHistory* imported_history_item = [[LuxxleImportedHistory alloc]
...
- (std::vector<history::URLRow>)convertToChromiumImportedHistory:
    (NSArray<LuxxleImportedHistory*>*)historyItems {
  std::vector<history::URLRow> results;
  for (LuxxleImportedHistory* historyItem in historyItems) {
