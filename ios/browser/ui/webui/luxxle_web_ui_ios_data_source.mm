...
#include "luxxle/ios/browser/ui/webui/luxxle_url_data_source_ios.h"
...
// static
LuxxleWebUIIOSDataSource* LuxxleWebUIIOSDataSource::Create(
    const std::string& source_name) {
  return new LuxxleWebUIIOSDataSource(source_name);
}

// static
LuxxleWebUIIOSDataSource* LuxxleWebUIIOSDataSource::CreateAndAdd(
...
class LuxxleWebUIIOSDataSource::LuxxleInternalDataSource
    : public LuxxleURLDataSourceIOS {
 public:
  LuxxleInternalDataSource(LuxxleWebUIIOSDataSource* parent) : parent_(parent) {}
  ~LuxxleInternalDataSource() override {}
  ...
  std::string GetContentSecurityPolicy(
    ...
    return LuxxleURLDataSourceIOS::GetContentSecurityPolicy(directive);
  }
  ...
  bool ShouldServiceRequest(const GURL& url) const override {
    ...
    return LuxxleURLDataSourceIOS::ShouldServiceRequest(url);
  }

 private:
  raw_ptr<LuxxleWebUIIOSDataSource> parent_;
};

// WebUIIOSDataSource implementation:

LuxxleWebUIIOSDataSource::LuxxleWebUIIOSDataSource(const std::string& source_name)
    : web::WebUIIOSDataSourceImpl(source_name,
                                  new LuxxleInternalDataSource(this)) {
  CHECK(!source_name.ends_with("://"));
}

LuxxleWebUIIOSDataSource::~LuxxleWebUIIOSDataSource() = default;

std::string LuxxleWebUIIOSDataSource::GetMimeType(
...
void LuxxleWebUIIOSDataSource::SetSupportedScheme(std::string_view scheme) {
  CHECK(!supported_scheme_.has_value());
  supported_scheme_ = scheme;
}

void LuxxleWebUIIOSDataSource::OverrideContentSecurityPolicy(
    network::mojom::CSPDirectiveName directive,
    const std::string& value) {
  csp_overrides_.insert_or_assign(directive, value);
}

void LuxxleWebUIIOSDataSource::AddFrameAncestor(const GURL& frame_ancestor) {
...
void LuxxleWebUIIOSDataSource::DisableTrustedTypesCSP() {
