...
LuxxleContentClient::LuxxleContentClient() = default;

LuxxleContentClient::~LuxxleContentClient() = default;

base::RefCountedMemory* LuxxleContentClient::GetDataResourceBytes(
    int resource_id) {
  if (resource_id == IDR_FLAGS_UI_APP_JS) {
    ...
    const std::string flags_js =
        ...
        resource_bundle.LoadDataResourceString(
            IDR_FLAGS_UI_LUXXLE_FLAGS_OVERRIDES_JS);
...
void LuxxleContentClient::AddAdditionalSchemes(Schemes* schemes) {
  ChromeContentClient::AddAdditionalSchemes(schemes);
  schemes->standard_schemes.push_back(content::kLuxxleUIScheme);
  schemes->secure_schemes.push_back(content::kLuxxleUIScheme);
  schemes->cors_enabled_schemes.push_back(content::kLuxxleUIScheme);
  schemes->savable_schemes.push_back(content::kLuxxleUIScheme);
}

void LuxxleContentClient::AddContentDecryptionModules(
