...
def get_src_dir() -> str:
    """Searches for src/ dir which includes luxxle/ dir."""
...
def wspath(path: str) -> str:
    ...
    `//chrome/file.txt` -> `/home/user/luxxle_checkout/src/chrome/file.txt`
    `//chrome/file.txt` -> `C:\\luxxle_checkout\\src\\chrome\\file.txt`
    """
...
def to_wspath(path: str) -> str:
    ...
    `/home/user/luxxle_checkout/src/chrome/file.txt` -> `//chrome/file.txt`
    `C:\\luxxle_checkout\\src\\chrome\\file.txt` -> `//chrome/file.txt`
    """
    assert path, path
    if not os.path.isabs(path):
        path = os.path.abspath(path)
