...
fn blocker_new(c: &mut Criterion) {
    ...
    let luxxle_list_rules: Vec<_> = rules_from_lists(&[
        "data/luxxle/luxxle-main-list.txt",
    ]).collect();

    group.bench_function("el+ep", move |b| b.iter(|| get_blocker(&easylist_rules)));
    group.bench_function("luxxle-list", move |b| b.iter(|| get_blocker(&luxxle_list_rules)));
