...
#include "luxxle/third_party/blink/renderer/luxxle_font_whitelist.h"
...
class LuxxleFontWhitelistTest : public testing::Test {
 public:
  LuxxleFontWhitelistTest() = default;
  LuxxleFontWhitelistTest(const LuxxleFontWhitelistTest&) = delete;
  LuxxleFontWhitelistTest& operator=(const LuxxleFontWhitelistTest&) = delete;
  ~LuxxleFontWhitelistTest() override = default;

  void SetUp() override {}
};

TEST(LuxxleFontWhitelistTest, Platforms) {
#if BUILDFLAG(IS_MAC)
  EXPECT_EQ(luxxle::GetFontWhitelistSizeForTesting(), 286UL);
#elif BUILDFLAG(IS_WIN)
  EXPECT_EQ(luxxle::GetFontWhitelistSizeForTesting(), 313UL);
#elif BUILDFLAG(IS_ANDROID)
  EXPECT_EQ(luxxle::GetFontWhitelistSizeForTesting(), 40UL);
#elif BUILDFLAG(IS_LINUX)
  const auto distro = base::GetLinuxDistro();
  if (distro.starts_with("Ubuntu 22.04")) {
    EXPECT_EQ(luxxle::GetFontWhitelistSizeForTesting(), 171UL);
  } else if (distro.starts_with("Ubuntu 20.04")) {
    EXPECT_EQ(luxxle::GetFontWhitelistSizeForTesting(), 163UL);
  } else if (distro.starts_with("Fedora")) {
    EXPECT_EQ(luxxle::GetFontWhitelistSizeForTesting(), 118UL);
  } else {
    EXPECT_EQ(luxxle::GetFontWhitelistSizeForTesting(), 0UL);
  }
#endif
}

TEST(LuxxleFontWhitelistTest, Locales) {
#if BUILDFLAG(IS_LINUX)
...
TEST(LuxxleFontWhitelistTest, KnownFonts) {
...
TEST(LuxxleFontWhitelistTest, CaseInsensitivity) {
