...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  std::string output;
  luxxle_wallet::EncodeString(input, &output);
  luxxle_wallet::EncodeStringArray(std::vector<std::string>(array_size, input),
                                  &output);

  luxxle_wallet::DecodeString(offset, input, &output);
  return 0;
}
