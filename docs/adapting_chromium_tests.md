# Adapting Chromium tests for the Luxxle codebase

To make Chromium tests work properly in the Luxxle codebase, you may need to
...
running test vendor. It is set to `kLuxxle` for `luxxle_*` test binaries and
...
The `LuxxleChromeTestSetupHelper` class in
...
The `LuxxleChromeTestSetupHelper` class uses a list of `TestAdjustments` to
specify which features to enable or disable for specific test patterns.

```cpp
class LuxxleChromeTestSetupHelper : public testing::EmptyTestEventListener {
