...
public class LuxxleRewardsUserWalletActivity
    ...
    private void setUIControls() {
        ...
        final int status =
                intent.getIntExtra(LuxxleRewardsExternalWallet.STATUS, UNDEFINED_WALLET_STATUS);
        ...
        switch (status) {
            case WalletStatus.CONNECTED:
                txtUserStatus.setText(LuxxleRewardsExternalWallet.walletStatusToString(status));
                break;
        ...
        String userId = intent.getStringExtra(LuxxleRewardsExternalWallet.USER_NAME);
        ...
        if (view.getId() == R.id.provider_action) {
            if (getIntent() != null) {
                int walletStatus = getIntent().getIntExtra(
                        LuxxleRewardsExternalWallet.STATUS, UNDEFINED_WALLET_STATUS);
                if (walletStatus == WalletStatus.CONNECTED) {
                    if (getIntent().getStringExtra(LuxxleRewardsExternalWallet.ACCOUNT_URL)
                            != null) {
                        Intent intent = new Intent();
                        intent.putExtra(LuxxleActivity.OPEN_URL,
                                getIntent().getStringExtra(LuxxleRewardsExternalWallet.ACCOUNT_URL));
                        setResult(RESULT_OK, intent);
                    }
                } else if (walletStatus == WalletStatus.LOGGED_OUT) {
                    Intent intent = new Intent();
                    intent.putExtra(
                            LuxxleActivity.OPEN_URL,
