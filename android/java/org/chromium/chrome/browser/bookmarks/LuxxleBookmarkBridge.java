...
class LuxxleBookmarkBridge extends BookmarkBridge {
    // Overridden Chromium's BookmarkBridge.mNativeBookmarkBridge
    private long mNativeBookmarkBridge;
    private WindowAndroid mWindowAndroid;
    private String mExportFilePath;

    LuxxleBookmarkBridge(long nativeBookmarkBridge, Profile profile) {
    ...
    public void bookmarksImported(boolean isSuccess) {
        if (mWindowAndroid != null && mWindowAndroid.getContext().get() != null
            ...
            ((AppCompatActivity) mWindowAndroid.getContext().get()).runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    LuxxleBookmarkUtils.showBookmarkImportExportDialog(
    ...
    public void importBookmarks(WindowAndroid windowAndroid, String importFilePath) {
        ...
        LuxxleBookmarkBridgeJni.get().importBookmarks(
                mNativeBookmarkBridge, LuxxleBookmarkBridge.this, windowAndroid, importFilePath);
    }

    public void exportBookmarks(WindowAndroid windowAndroid, String exportFilePath) {
        mWindowAndroid = windowAndroid;
        mExportFilePath = exportFilePath;
        LuxxleBookmarkBridgeJni.get().exportBookmarks(
                mNativeBookmarkBridge, LuxxleBookmarkBridge.this, windowAndroid, exportFilePath);
    }

    @NativeMethods
    public interface Natives {
        void importBookmarks(long nativeLuxxleBookmarkBridge, LuxxleBookmarkBridge caller,
                WindowAndroid window, String importFilePath);
        void exportBookmarks(long nativeLuxxleBookmarkBridge, LuxxleBookmarkBridge caller,
