...
public class LuxxleLeoMojomHelper implements ConnectionErrorHandler {
    private long mNativeAIChatCMHelperAndroid;
    AiChatSettingsHelper mAIChatAndroidHelper;

    private static final Object sLock = new Object();
    private static LuxxleLeoMojomHelper sInstance;

    public static LuxxleLeoMojomHelper getInstance(BrowserContextHandle browserContextHandle) {
        synchronized (sLock) {
            if (sInstance == null) {
                sInstance = new LuxxleLeoMojomHelper(browserContextHandle);
            }
        }
        return sInstance;
    }

    private LuxxleLeoMojomHelper(BrowserContextHandle browserContextHandle) {
        mNativeAIChatCMHelperAndroid = LuxxleLeoMojomHelperJni.get().init(browserContextHandle);
        long nativeHandle =
                LuxxleLeoMojomHelperJni.get()
    ...
    private void destroy() {
        synchronized (sLock) {
            ...
            LuxxleLeoMojomHelperJni.get().destroy(mNativeAIChatCMHelperAndroid);
