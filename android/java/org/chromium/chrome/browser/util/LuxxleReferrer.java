...
@JNINamespace("android_luxxle_referrer")
public class LuxxleReferrer implements InstallReferrerStateListener {
    private static final String TAG = "LuxxleReferrer";
    private static final String APP_CHROME_DIR = "app_chrome";
    private static final String PROMO_CODE_FILE_NAME = "promoCode";
    private static final String LUXXLE_REFERRER_RECEIVED = "luxxle_referrer_received";
    ...
    private long mNativeLuxxleReferrer;

    private LuxxleReferrer(long nativeLuxxleReferrer) {
        mNativeLuxxleReferrer = nativeLuxxleReferrer;
    }

    @CalledByNative
    private static LuxxleReferrer create(long nativeLuxxleReferrer) {
        return new LuxxleReferrer(nativeLuxxleReferrer);
    }

    @CalledByNative
    private void destroy() {
        assert mNativeLuxxleReferrer != 0;
        mNativeLuxxleReferrer = 0;
    }

    private class InitReferrerRunnable implements Runnable {
        private Context mContext;
        private LuxxleReferrer mLuxxleReferrer;
        public InitReferrerRunnable(Context context, LuxxleReferrer luxxleReferrer) {
            mContext = context;
            mLuxxleReferrer = luxxleReferrer;
        }

        @Override
        public void run() {
            SharedPreferences sharedPref = ContextUtils.getAppSharedPreferences();
            if (sharedPref.getBoolean(LUXXLE_REFERRER_RECEIVED, false)
            ...
            try {
                referrerClient.startConnection(mLuxxleReferrer);
            } catch (Exception e) {
        ...
        switch (responseCode) {
            case InstallReferrerResponse.OK:
                try {
                    ...
                    editor.putBoolean(LUXXLE_REFERRER_RECEIVED, true);
                    editor.apply();
    ...
    private void onReferrerReady() {
        PostTask.postTask(TaskTraits.UI_BEST_EFFORT,
                () -> { LuxxleReferrerJni.get().onReferrerReady(mNativeLuxxleReferrer); });
    }

    @NativeMethods
    interface Natives {
        void onReferrerReady(long nativeLuxxleReferrer);
    }
}
