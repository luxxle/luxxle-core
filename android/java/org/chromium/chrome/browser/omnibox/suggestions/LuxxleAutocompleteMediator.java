...
@NullMarked
class LuxxleAutocompleteMediator extends AutocompleteMediator
        implements LuxxleSuggestionHost, LuxxleLeoAutocompleteDelegate {
    private static final String AUTOCOMPLETE_ENABLED = "luxxle.autocomplete_enabled";
    ...
    public LuxxleAutocompleteMediator(
            Context context,
    ...
    public void removeLuxxleSearchSuggestion() {
        if (mDropdownViewInfoListManager instanceof LuxxleDropdownItemViewInfoListManager) {
            ((LuxxleDropdownItemViewInfoListManager) mDropdownViewInfoListManager)
                    .removeLuxxleSearchSuggestion();
        }
    }
    ...
    void initDefaultProcessors() {
        if (mDropdownViewInfoListBuilder != null
                && mDropdownViewInfoListBuilder instanceof LuxxleDropdownItemViewInfoListBuilder) {
            ((LuxxleDropdownItemViewInfoListBuilder) mDropdownViewInfoListBuilder)
    ...
    public boolean isLeoEnabled() {
        return LuxxleLeoPrefUtils.isLeoEnabled();
    }

    @Override
    public void openLeoQuery(WebContents webContents, String conversationUuid, String query) {
        mDelegate.clearOmniboxFocus();
        LuxxleLeoUtils.openLeoQuery(webContents, conversationUuid, query, true);
    }
    ...
    // in multi-window/split-mode or when switch away from Luxxle.
