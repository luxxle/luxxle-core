...
public class LuxxleNtpAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private OnLuxxleNtpListener mOnLuxxleNtpListener;
    ...
    private static final String TAG = "LuxxleNtpAdapter";

    public LuxxleNtpAdapter(Activity activity, OnLuxxleNtpListener onLuxxleNtpListener,
            RequestManager glide, CopyOnWriteArrayList<FeedItemsCard> newsItems,
            LuxxleNewsController luxxleNewsController, View mvTilesContainerLayout, NTPImage ntpImage,
            ...
            int recyclerViewHeight, boolean isTopSitesEnabled, boolean isLuxxleStatsEnabled,
            boolean isDisplayNewsFeed, boolean isDisplayNewsOptin) {
        mActivity = activity;
        mOnLuxxleNtpListener = onLuxxleNtpListener;
        mGlide = glide;
        ...
        mIsLuxxleStatsEnabled = isLuxxleStatsEnabled;
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof StatsViewHolder) {
            ...
            statsViewHolder.mHideStatsImg.setOnClickListener(
                    view -> {
                        ChromeSharedPreferences.getInstance()
                                .writeBoolean(
                                        BackgroundImagesPreferences.PREF_SHOW_LUXXLE_STATS, false);
                    });
            List<Pair<String, String>> statsPairs = LuxxleStatsUtil.getStatsPairs();
            ...
            statsViewHolder.mNtpStatsLayout.setOnClickListener(
                    view -> {
                        mOnLuxxleNtpListener.checkForLuxxleStats();
                    });
            ...
            newContentViewHolder.mNewContentLayout.setOnClickListener(
                    view -> {
                        mOnLuxxleNtpListener.loadNewContent();
                    });
                ...
                imageCreditViewHolder.mSuperReferralLogo.setOnClickListener(
                        view -> {
                            ...
                            qRCodeShareDialogFragment.setQRCodeText(
                                    LuxxleConstants.LUXXLE_REF_URL
                                            + mNTPBackgroundImagesBridge.getSuperReferralCode());
                            qRCodeShareDialogFragment.show(
                                    ((LuxxleActivity) mActivity).getSupportFragmentManager(),
            ...
            newsOptinViewHolder.mOptinClose.setOnClickListener(
                    view -> {
                        mOnLuxxleNtpListener.updateNewsOptin(false);
                    });

            newsOptinViewHolder.mOptinLearnMore.setOnClickListener(
                    view -> {
                        TabUtils.openUrlInSameTab(LuxxleConstants.LUXXLE_NEWS_LEARN_MORE_URL);
                    });

            newsOptinViewHolder.mOptinButton.setOnClickListener(
                    view -> {
                        mOnLuxxleNtpListener.updateNewsOptin(true);
                        mOnLuxxleNtpListener.getFeed(false);
                    });
            ...
            noSourcesViewHolder.mBtnChooseContent.setOnClickListener(
                    view -> {
                        if (mActivity instanceof LuxxleActivity) {
                            ((LuxxleActivity) mActivity).openLuxxleNewsSettings();
                        }
                    });
        }
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == TYPE_STATS) {
            view = LayoutInflater.from(parent.getContext())
                           .inflate(R.layout.luxxle_stats_layout, parent, false);
            ...
            view = LayoutInflater.from(parent.getContext())
                           .inflate(R.layout.luxxle_news_load_new_content, parent, false);
            ...
            view = LayoutInflater.from(parent.getContext())
                           .inflate(R.layout.luxxle_news_no_sources, parent, false);
            return new NoSourcesViewHolder(view);

        } else {
            view = LayoutInflater.from(parent.getContext())
                           .inflate(R.layout.luxxle_news_row, parent, false);
    ...
    public void setLuxxleStatsEnabled(boolean isLuxxleStatsEnabled) {
        if (mIsLuxxleStatsEnabled != isLuxxleStatsEnabled) {
            mIsLuxxleStatsEnabled = isLuxxleStatsEnabled;
            if (mIsLuxxleStatsEnabled) {
                notifyItemInserted(getStatsCount());
            } else {
                notifyItemRemoved(getStatsCount());
            }
    ...
    public void setLuxxleNewsController(LuxxleNewsController luxxleNewsController) {
        mLuxxleNewsController = luxxleNewsController;
        notifyItemChanged(
                getStatsCount() + getTopSitesCount() + getNewContentCount() + ONE_ITEM_SPACE);
    }
    ...
    public void setImageCreditAlpha(float alpha) {
        ...
        // here https://github.com/luxxle/luxxle-browser/issues/29343
    ...
    public static class StatsViewHolder extends RecyclerView.ViewHolder {
        ...
        StatsViewHolder(View itemView) {
            ...
            this.mTitleLayout = (LinearLayout) itemView.findViewById(R.id.luxxle_stats_title_layout);
            this.mHideStatsImg = (ImageView) itemView.findViewById(R.id.widget_more_option);
            this.mAdsBlockedCountTv =
                    (TextView) itemView.findViewById(R.id.luxxle_stats_text_ads_count);
            this.mAdsBlockedCountTextTv =
                    (TextView) itemView.findViewById(R.id.luxxle_stats_text_ads_count_text);
            this.mDataSavedValueTv =
                    (TextView) itemView.findViewById(R.id.luxxle_stats_data_saved_value);
            this.mDataSavedValueTextTv =
                    (TextView) itemView.findViewById(R.id.luxxle_stats_data_saved_value_text);
            this.mEstTimeSavedCountTv =
                    (TextView) itemView.findViewById(R.id.luxxle_stats_text_time_count);
            this.mEstTimeSavedCountTextTv =
                    (TextView) itemView.findViewById(R.id.luxxle_stats_text_time_count_text);
            LuxxleTouchUtils.ensureMinTouchTarget(this.mHideStatsImg);
        }
    }
    ...
    public static class ImageCreditViewHolder extends RecyclerView.ViewHolder {
        ...
        ImageCreditViewHolder(View itemView) {
            ...
            LuxxleTouchUtils.ensureMinTouchTarget(this.mCreditTv);
        }
    }
    ...
    public static class NewsOptinViewHolder extends RecyclerView.ViewHolder {
        ...
        NewsOptinViewHolder(View itemView) {
            ...
            LuxxleTouchUtils.ensureMinTouchTarget(mOptinButton);
            LuxxleTouchUtils.ensureMinTouchTarget(mOptinLearnMore);
        }
    }
