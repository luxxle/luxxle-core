...
public class LuxxleNewsPreferencesTypeAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private LuxxleNewsController mLuxxleNewsController;
    private LuxxleNewsPreferencesListener mLuxxleNewsPreferencesListener;
    private LuxxleNewsPreferencesSearchType mLuxxleNewsPreferencesSearchType;
    ...
    public LuxxleNewsPreferencesTypeAdapter(
            Context context,
            LuxxleNewsPreferencesListener luxxleNewsPreferencesListener,
            LuxxleNewsPreferencesSearchType luxxleNewsPreferencesSearchType,
            LuxxleNewsController luxxleNewsController,
            RequestManager glide,
            String luxxleNewsPreferencesType,
            List<Channel> channelList,
            List<Publisher> publisherList) {
        mContext = context;
        mLuxxleNewsController = luxxleNewsController;
        mLuxxleNewsPreferencesSearchType = luxxleNewsPreferencesSearchType;
        mLuxxleNewsPreferencesListener = luxxleNewsPreferencesListener;
        mGlide = glide;
        mLuxxleNewsPreferencesType = luxxleNewsPreferencesType;
        ...
        if (mChannelList.size() > 0) {
            mChannelIcons = LuxxleNewsUtils.getChannelIcons();
        }
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof NewsPreferencesSectionViewHolder) {
            ...
            if ((mLuxxleNewsPreferencesType.equalsIgnoreCase(
                        ...
                        || mLuxxleNewsPreferencesType.equalsIgnoreCase(
                ...
                if (mChannelList.size() > 0 && position < getChannelItemsCount()) {
                    setChannel(position, viewHolder, mChannelList.get(position - ONE_ITEM_SPACE));
                        ...
                        && (mLuxxleNewsPreferencesSearchType
                                        == LuxxleNewsPreferencesSearchType.SearchUrl
                                || mLuxxleNewsPreferencesSearchType
                                        == LuxxleNewsPreferencesSearchType.GettingFeed
                                || mLuxxleNewsPreferencesSearchType
                ...
                } else if (mLuxxleNewsPreferencesSearchType
    ...
    private void setSearchUrl(int position, NewsPreferencesViewHolder viewHolder) {
        ...
        if (mLuxxleNewsPreferencesSearchType == LuxxleNewsPreferencesSearchType.SearchUrl
                || mLuxxleNewsPreferencesSearchType == LuxxleNewsPreferencesSearchType.GettingFeed) {
            viewHolder.name.setText(mSearchUrl);
            viewHolder.subtitle.setVisibility(View.GONE);
            viewHolder.btnFollow.setVisibility(View.VISIBLE);
            if (mLuxxleNewsPreferencesSearchType == LuxxleNewsPreferencesSearchType.GettingFeed) {
            ...
            viewHolder.btnFollow.setOnClickListener(view -> {
                if (mLuxxleNewsPreferencesSearchType == LuxxleNewsPreferencesSearchType.SearchUrl) {
                    ...
                    mLuxxleNewsPreferencesListener.findFeeds(mSearchUrl);
                }
            });
        } else if (mLuxxleNewsPreferencesSearchType == LuxxleNewsPreferencesSearchType.NotFound) {
    ...
    private void setFeedSearchResultItem(int position, NewsPreferencesViewHolder viewHolder) {
        if (position >= 0 && position < mFeedSearchResultItemList.size()) {
            ...
            viewHolder.btnFollow.setOnClickListener(view -> {
                if (!isFollow) {
                    mLuxxleNewsPreferencesListener.subscribeToNewDirectFeed(
                    ...
                    mLuxxleNewsPreferencesListener.updateFeedSearchResultItem(
                            position, feedSearchResultItem.feedUrl.url, publisherId);

                    mLuxxleNewsPreferencesListener.onPublisherPref(
    ...
    private void setSource(
        ...
        if (mLuxxleNewsController != null && publisher.coverUrl != null) {
            mLuxxleNewsController.getImageData(publisher.coverUrl, imageData -> {
                if (imageData != null) {
    ...
    private void setChannel(int position, NewsPreferencesViewHolder viewHolder, Channel channel) {
        ...
        viewHolder.btnFollow.setVisibility(View.VISIBLE);
        if (subscribedLocalesList.contains(LuxxleNewsUtils.getLocale())) {
            displayFollowButton(true, R.string.unfollow, viewHolder);
        } else {
            displayFollowButton(false, R.string.follow, viewHolder);
        }
        ...
        viewHolder.btnFollow.setOnClickListener(view -> {
            boolean isSubscribed = subscribedLocalesList.contains(LuxxleNewsUtils.getLocale());
            isSubscribed = !isSubscribed;
            if (isSubscribed) {
                subscribedLocalesList.add(LuxxleNewsUtils.getLocale());
            } else {
                subscribedLocalesList.remove(LuxxleNewsUtils.getLocale());
            }
            ...
            mLuxxleNewsPreferencesListener.onChannelSubscribed(position, channel, isSubscribed);
        });
    }
    ...
    private void displayFollowButton(
        ...
        if (isFollowing) {
            holder.btnFollow.setBackgroundResource(R.drawable.luxxle_news_settings_unfollow_bg);
    ...
    public void setFindFeeds(List<FeedSearchResultItem> feedSearchResultItemList,
        ...
        mLuxxleNewsPreferencesSearchType = luxxleNewsPreferencesSearchType;
        if (luxxleNewsPreferencesSearchType == LuxxleNewsPreferencesSearchType.NewSource) {
            notifyItemRangeInserted(startPosition, feedSearchResultItemList.size());
        } else if (luxxleNewsPreferencesSearchType == LuxxleNewsPreferencesSearchType.NotFound) {
    ...
    public int getItemCount() {
        if (mLuxxleNewsPreferencesType.equalsIgnoreCase(
            ...
            if (mLuxxleNewsPreferencesSearchType == LuxxleNewsPreferencesSearchType.SearchUrl
                    || mLuxxleNewsPreferencesSearchType == LuxxleNewsPreferencesSearchType.GettingFeed
                    || mLuxxleNewsPreferencesSearchType == LuxxleNewsPreferencesSearchType.NotFound) {
