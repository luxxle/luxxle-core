...
import org.chromium.chrome.browser.sync.LuxxleSyncDevices;
import org.chromium.chrome.browser.sync.SyncServiceFactory;
import org.chromium.chrome.browser.sync.settings.LuxxleManageSyncSettings;
...
public class LuxxleSyncScreensPreference extends LuxxlePreferenceFragment
        implements View.OnClickListener,
                BackPressHelper.ObsoleteBackPressedHandler,
                BarcodeTracker.BarcodeGraphicTrackerCallback,
                LuxxleSyncDevices.DeviceInfoChangedListener,
    ...
    private TextView mLuxxleSyncTextDevicesTitle;
    private TextView mLuxxleSyncWordCountTitle;
    private TextView mLuxxleSyncAddDeviceCodeWords;
    ...
    // Below enum is matching the values of GetDeviceTypeString() in luxxle_device_info.cc
    ...
    LuxxleSyncWorker getLuxxleSyncWorker() {
        return LuxxleSyncWorker.get();
    }
    ...
    public View onCreateView(
        ...
        return mInflater.inflate(R.layout.luxxle_sync_layout, container, false);
    }
    ...
    private void fillDevices() {
        try {
            ...
            ArrayList<LuxxleSyncDevices.SyncDeviceInfo> deviceInfos =
                    LuxxleSyncDevices.get().getSyncDeviceList();

            ViewGroup insertPoint = (ViewGroup) getView().findViewById(R.id.luxxle_sync_devices);
            insertPoint.removeAllViews();
            int index = 0;
            for (LuxxleSyncDevices.SyncDeviceInfo device : deviceInfos) {
                View separator = (View) mInflater.inflate(R.layout.menu_separator, null);
                View listItemView = (View) mInflater.inflate(R.layout.luxxle_sync_device, null);
                if (null != listItemView && null != separator && null != insertPoint) {
                    ImageView imageView =
                            (ImageView) listItemView.findViewById(R.id.luxxle_sync_device_image);
                    ...
                    TextView textView =
                            (TextView) listItemView.findViewById(R.id.luxxle_sync_device_text);
                    if (null != textView) {
                        if (device.mIsCurrentDevice) {
                            String currentDevice =
                                    device.mName
                                            ...
                                            + getResources()
                                                    .getString(
                                                            R.string.luxxle_sync_this_device_text);
                    ...
                    AppCompatImageView deleteButton =
                            (AppCompatImageView)
                                    listItemView.findViewById(R.id.luxxle_sync_remove_device);
                    if (device.mSupportsSelfDelete || device.mIsCurrentDevice) {
                        deleteButton.setTag(device);
                        deleteButton.setOnClickListener(
                                v -> {
                                    LuxxleSyncDevices.SyncDeviceInfo deviceToDelete =
                                            (LuxxleSyncDevices.SyncDeviceInfo) v.getTag();
            ...
            if (index > 0) {
                mLuxxleSyncTextDevicesTitle.setText(
                        getResources().getString(R.string.luxxle_sync_devices_title));
                View separator = (View) mInflater.inflate(R.layout.menu_separator, null);
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mScanChainCodeButton = getView().findViewById(R.id.luxxle_sync_btn_scan_chain_code);
        if (mScanChainCodeButton != null) {
            mScanChainCodeButton.setOnClickListener(this);
        }

        mStartNewChainButton = getView().findViewById(R.id.luxxle_sync_btn_start_new_chain);
        if (mStartNewChainButton != null) {
            mStartNewChainButton.setOnClickListener(this);
        }

        mEnterCodeWordsButton = getView().findViewById(R.id.luxxle_sync_btn_enter_code_words);
        if (mEnterCodeWordsButton != null) {
            mEnterCodeWordsButton.setOnClickListener(this);
        }

        mQRCodeImage = getView().findViewById(R.id.luxxle_sync_qr_code_image);

        mQRContainer = getView().findViewById(R.id.luxxle_sync_qr_containter);

        mDoneButton = getView().findViewById(R.id.luxxle_sync_btn_done);
        if (mDoneButton != null) {
            mDoneButton.setOnClickListener(this);
        }

        mDoneLaptopButton = getView().findViewById(R.id.luxxle_sync_btn_add_laptop_done);
        if (mDoneLaptopButton != null) {
            mDoneLaptopButton.setOnClickListener(this);
        }

        mUseCameraButton = getView().findViewById(R.id.luxxle_sync_btn_use_camera);
        if (mUseCameraButton != null) {
            mUseCameraButton.setOnClickListener(this);
        }

        mConfirmCodeWordsButton = getView().findViewById(R.id.luxxle_sync_confirm_code_words);
        if (mConfirmCodeWordsButton != null) {
            mConfirmCodeWordsButton.setOnClickListener(this);
        }

        mMobileButton = getView().findViewById(R.id.luxxle_sync_btn_mobile);
        if (mMobileButton != null) {
            mMobileButton.setOnClickListener(this);
        }

        mLaptopButton = getView().findViewById(R.id.luxxle_sync_btn_laptop);
        if (mLaptopButton != null) {
            mLaptopButton.setOnClickListener(this);
        }

        mPasteButton = getView().findViewById(R.id.luxxle_sync_paste_button);
        if (mPasteButton != null) {
            mPasteButton.setOnClickListener(this);
        }

        mCopyButton = getView().findViewById(R.id.luxxle_sync_copy_button);
        if (mCopyButton != null) {
            mCopyButton.setOnClickListener(this);
        }

        mLuxxleSyncTextDevicesTitle = getView().findViewById(R.id.luxxle_sync_devices_title);
        mLuxxleSyncWordCountTitle = getView().findViewById(R.id.luxxle_sync_text_word_count);
        mLuxxleSyncWordCountTitle.setText(getString(R.string.luxxle_sync_word_count_text, 0));
        mLuxxleSyncAddDeviceCodeWords =
                getView().findViewById(R.id.luxxle_sync_add_device_code_words);

        mCameraSourcePreview = getView().findViewById(R.id.preview);

        mAddDeviceButton = getView().findViewById(R.id.luxxle_sync_btn_add_device);
        if (null != mAddDeviceButton) {
            mAddDeviceButton.setOnClickListener(this);
        }

        mDeleteAccountButton = getView().findViewById(R.id.luxxle_sync_btn_delete_account);
        if (null != mDeleteAccountButton) {
            mDeleteAccountButton.setOnClickListener(this);
        }

        mShowCategoriesButton = getView().findViewById(R.id.luxxle_sync_btn_show_categories);
        ...
        mNewCodeWordsButton = getView().findViewById(R.id.luxxle_sync_btn_add_laptop_new_code);
        assert mNewCodeWordsButton != null;
        mNewCodeWordsButton.setOnClickListener(this);

        mNewQrCodeButton = getView().findViewById(R.id.luxxle_sync_btn_add_mobile_new_code);
        ...
        mLayoutMobile = getView().findViewById(R.id.luxxle_sync_frame_mobile);
        mLayoutLaptop = getView().findViewById(R.id.luxxle_sync_frame_laptop);
    ...
    private void setAppropriateView() {
        ...
        boolean firstSetupComplete = getLuxxleSyncWorker().isInitialSyncFeatureSetupComplete();
    ...
    private String getWordsValidationString(String words) {
        int validationResult = getLuxxleSyncWorker().getWordsValidationResult(words);
        ...
        switch (validationResult) {
            ...
            case 2:
                // kVersionDeprecated
                return getResources().getString(R.string.luxxle_sync_code_from_deprecated_version);
            case 3:
                // kExpired
                return getResources().getString(R.string.luxxle_sync_code_expired);
            case 4:
                // kValidForTooLong
                return getResources().getString(R.string.luxxle_sync_code_valid_for_too_long);

            default:
                // These three different types of errors have the same message
                // kWrongWordsNumber
                // kNotValidPureWords
                return getResources().getString(R.string.luxxle_sync_wrong_code_error);
        }
    }
    ...
    public void onClick(View v) {
        ...
        if (mScanChainCodeButton == v) {
            ...
            if (null != mLuxxleSyncAddDeviceCodeWords) {
                ClipboardManager clipboard = (ClipboardManager) getActivity().getSystemService(
                        Context.CLIPBOARD_SERVICE);
                ClipData clip = ClipData.newPlainText("", mLuxxleSyncAddDeviceCodeWords.getText());
                clipboard.setPrimaryClip(clip);
                Toast.makeText(getActivity().getApplicationContext(),
                             getResources().getString(R.string.luxxle_sync_copied_text),
                             Toast.LENGTH_LONG)
                        .show();
            }
            ...
            String codephraseCandidate = getLuxxleSyncWorker().getPureWordsFromTimeLimited(words);
            ...
            getActivity().setTitle(R.string.luxxle_sync_code_words_title);
            if (null != mCodeWords && null != mLuxxleSyncWordCountTitle) {
                mCodeWords.addTextChangedListener(
                        new TextWatcher() {
                            ...
                            public void onTextChanged(
                                ...
                                mLuxxleSyncWordCountTitle.setText(
                                        getString(R.string.luxxle_sync_word_count_text, wordCount));
                                mLuxxleSyncWordCountTitle.invalidate();
                            }
                        });
            }
            ...
            settingsLauncher.startSettings(getContext(), LuxxleManageSyncSettings.class);
    ...
    void generateNewCodeWords() {
        ...
        String timeLimitedWords = getLuxxleSyncWorker().getTimeLimitedWordsFromPure(codePhrase);
        assert timeLimitedWords != null && !timeLimitedWords.isEmpty();
        mLuxxleSyncAddDeviceCodeWords.setVisibility(View.VISIBLE);
        mLuxxleSyncAddDeviceCodeWords.setText(timeLimitedWords);
        mCopyButton.setVisibility(View.VISIBLE);
        mNewCodeWordsButton.setVisibility(View.GONE);

        LocalDateTime notAfterTime =
                getLuxxleSyncWorker().getNotAfterFromFromTimeLimitedWords(timeLimitedWords);
    ...
    void generateNewQrCode() {
        ...
        String seedHex = getLuxxleSyncWorker().getSeedHexFromWords(getPureWords());
        if (null == seedHex || seedHex.isEmpty()) {
            ...
            if (!isSeedHexValid(seedHex)) {
                ...
                String qrCodeString = getLuxxleSyncWorker().getQrDataJson(seedHex);
        ...
        getActivity()
                .runOnUiThread(
                        new Runnable() {
                            ...
                            public void run() {
                                String seedWords =
                                        getLuxxleSyncWorker().getWordsFromSeedHex(seedHex);
                            ...
                            if (result) {
                                ...
                                onSyncError(
                                        getResources()
                                                .getString(
                                                        R.string
                                                                .luxxle_sync_joining_deleted_account));
                                allowNewQrScan();
                            }
                        });

        getLuxxleSyncWorker().requestSync();
        getLuxxleSyncWorker().saveCodephrase(seedWords);
        getLuxxleSyncWorker().finalizeSyncSetup();
    }
    ...
    public void onDestroy() {
        ...
        if (mDeviceInfoObserverSet) {
            LuxxleSyncDevices.get().removeDeviceInfoChangedListener(this);
    ...
    private boolean isSeedHexValid(String barcode) {
        ...
        String seedWords = getLuxxleSyncWorker().getWordsFromSeedHex(barcode);
    ...
    private String getQrCodeValidationString(String jsonQr) {
        ...
        int validationResult = getLuxxleSyncWorker().getQrCodeValidationResult(jsonQr);
        Log.v(TAG, "validationResult is " + validationResult);
        switch (validationResult) {
            case 0:
                // kValid, empty string indicates there is no error
            ...
            default:
                ...
                return getResources().getString(R.string.luxxle_sync_wrong_qrcode_error);
        }
    }
    ...
    public void onDetectedQrCode(Barcode barcode) {
        if (barcode != null) {
            ...
            String seedHex = getLuxxleSyncWorker().getSeedHexFromQrJson(jsonQr);
    ...
    private void showEndDialog(String message, Runnable runWhenDismissed) {
        ...
        AlertDialog alertDialog =
                alertBuilder.setTitle(getResources().getString(R.string.luxxle_sync_device))
                        .setMessage(message)
    ...
    private void deleteDeviceDialog(LuxxleSyncDevices.SyncDeviceInfo device) {
        AlertDialog.Builder alertBuilder =
                new AlertDialog.Builder(getActivity(), R.style.ThemeOverlay_BrowserUI_AlertDialog);
                    ...
                    public void onClick(DialogInterface dialog, int button) {
                        if (button == AlertDialog.BUTTON_POSITIVE) {
                            if (device.mIsCurrentDevice) {
                                ...
                                getLuxxleSyncWorker().resetSync();
                                startLeaveSyncChainOperations();
                            } else {
                                LuxxleSyncDevices.get().deleteDevice(device.mGuid);
                            }
                        }
                    }
                };
        ...
        if (device.mIsCurrentDevice) {
            deviceNameToDisplay = deviceNameToDisplay + " "
                    + getResources().getString(R.string.luxxle_sync_this_device_text);
        }
        AlertDialog alertDialog =
                alertBuilder
                        .setTitle(getResources().getString(R.string.luxxle_sync_remove_device_text))
                        .setMessage(
                                getString(R.string.luxxle_sync_delete_device, deviceNameToDisplay))
                        .setPositiveButton(R.string.ok, onClickListener)
    ...
    private void permanentlyDeleteAccount() {
        ...
        AlertDialog alertDialog =
                alertBuilder
                        .setTitle(getResources().getString(R.string.luxxle_sync_delete_account_text))
                        .setMessage(getString(R.string.luxxle_sync_delete_account_message))
                        .setPositiveButton(
                                R.string.luxxle_sync_delete_account_button, onClickListener)
                        .setNegativeButton(R.string.cancel, onClickListener)
                        .create();
        alertDialog.getDelegate().setHandleNativeActionModesEnabled(false);
        alertDialog.show();
    ...
    private void permanentlyDeletedAccount(String result) {
        ...
        if (result == null || result.isEmpty()) {
            showToast(
                    activity,
                    getResources().getString(R.string.luxxle_sync_delete_account_succeeded_toast)
                            + " "
                            + result);
        } else {
            showToast(
                    activity,
                    getResources().getString(R.string.luxxle_sync_delete_account_failed_toast)
                            + " "
                            + result);
        }
    }
    ...
    private void startLeaveSyncChainOperations() {
        ...
        mLuxxleSyncTextDevicesTitle.setText(
                getResources().getString(R.string.luxxle_sync_leaving_sync_chain_title));

        mShowCategoriesButton.setVisibility(View.GONE);
        mAddDeviceButton.setVisibility(View.GONE);

        ViewGroup devicesUiGroup = (ViewGroup) getView().findViewById(R.id.luxxle_sync_devices);
    ...
    private void setJoinExistingChainLayout() {
        ...
        getActivity().setTitle(R.string.luxxle_sync_scan_chain_code);
        if (null != mScrollViewSyncChainCode) {
    ...
    private void setNewChainLayout() {
        getActivity().setTitle(R.string.luxxle_sync_start_new_chain);
        if (null != mScrollViewSyncInitial) {
            mScrollViewSyncInitial.setVisibility(View.GONE);
        }
        if (null != mScrollViewEnterCodeWords) {
    ...
    public String getPureWords() {
        if (mCodephrase == null || mCodephrase.isEmpty()) {
            mCodephrase = getLuxxleSyncWorker().getPureWords();
        }
        return mCodephrase;
    }
    ...
    private void setAddMobileDeviceLayout() {
        getActivity().setTitle(R.string.luxxle_sync_btn_mobile);
    ...
    private void setQrCountDown(LocalDateTime notAfterTime) {
        FragmentContainerView the_view =
                (FragmentContainerView) getView().findViewById(R.id.luxxle_sync_count_down_qr);
        LuxxleSyncCodeCountdownFragment countdown = the_view.getFragment();
    ...
    private void setAddLaptopLayout() {
        getActivity().setTitle(R.string.luxxle_sync_btn_laptop);
    ...
    private void setWordsCountDown(LocalDateTime notAfterTime) {
        FragmentContainerView containerView =
                (FragmentContainerView)
                        getView().findViewById(R.id.luxxle_sync_count_down_code_words);
        LuxxleSyncCodeCountdownFragment countdown = containerView.getFragment();
        countdown.setExpiredRunnable(
                () -> {
                    mNewCodeWordsButton.setVisibility(View.VISIBLE);
                    mLuxxleSyncAddDeviceCodeWords.setVisibility(View.GONE);
    ...
    private void setSyncDoneLayout() {
        if (!mDeviceInfoObserverSet) {
            LuxxleSyncDevices.get().addDeviceInfoChangedListener(this);
            mDeviceInfoObserverSet = true;
        }

        LuxxlePermissionUtils.showNotificationPermissionDialog(getActivity());
        ...
        mLuxxleSyncTextDevicesTitle.setText(
                getResources().getString(R.string.luxxle_sync_loading_devices_title));
