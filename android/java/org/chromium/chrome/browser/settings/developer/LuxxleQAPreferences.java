...
public class LuxxleQAPreferences extends LuxxlePreferenceFragment
    ...
    private ChromeSwitchPreference mLuxxleDormantFeatureEngagement;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mLuxxleDormantFeatureEngagement =
                (ChromeSwitchPreference)
                        findPreference(OnboardingPrefManager.PREF_DORMANT_USERS_ENGAGEMENT);
        if (mLuxxleDormantFeatureEngagement != null) {
            mLuxxleDormantFeatureEngagement.setOnPreferenceChangeListener(this);
        }
        ...
        mIsStagingServer.setChecked(
                UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                        .getBoolean(LuxxlePref.USE_REWARDS_STAGING_SERVER));
        ...
        mDbUtil = LuxxleDbUtil.getInstance();
    ...
    @Override
    public void onStart() {
        LuxxleRewardsNativeWorker.getInstance().addObserver(this);
        super.onStart();
    }

    @Override
    public void onStop() {
        LuxxleRewardsNativeWorker.getInstance().removeObserver(this);
        super.onStop();
    }
    ...
    private void checkQACode() {
        ...
        DialogInterface.OnClickListener onClickListener = new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int button) {
                if (button != AlertDialog.BUTTON_POSITIVE
                        || !input.getText().toString().equals(LuxxleConfig.DEVELOPER_OPTIONS_CODE)) {
                    getActivity().finish();
                }
            }
        };
    ...
    private void enableMaximumAdsNumber(boolean enable) {
        if (enable) {
            // Save current values
            int adsPerHour = LuxxleRewardsNativeWorker.getInstance().getAdsPerHour();
            SharedPreferences sharedPreferences = ContextUtils.getAppSharedPreferences();
            SharedPreferences.Editor sharedPreferencesEditor = sharedPreferences.edit();
            sharedPreferencesEditor.putInt(QA_ADS_PER_HOUR, adsPerHour);
            sharedPreferencesEditor.apply();
            // Set max value
            LuxxleRewardsNativeWorker.getInstance().setAdsPerHour(MAX_ADS);
            return;
        }
        // Set saved values
        int adsPerHour = ContextUtils.getAppSharedPreferences().getInt(
                             QA_ADS_PER_HOUR, DEFAULT_ADS_PER_HOUR);
        LuxxleRewardsNativeWorker.getInstance().setAdsPerHour(adsPerHour);
    }
    ...
    public void onResetTheWholeState(boolean success) {
        if (success) {
            ...
            UserPrefs.get(ProfileManager.getLastUsedRegularProfile())
                    .setBoolean(LuxxlePref.USE_REWARDS_STAGING_SERVER, mUseRewardsStagingServer);
            LuxxleRewardsHelper.setRewardsEnvChange(true);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CHOOSE_FILE_FOR_IMPORT_REQUEST_CODE && resultCode == Activity.RESULT_OK
            ...
            try {
                ...
                Log.e(LuxxleDbUtil.getTag(), "Error on preparing database file: " + e);
                return;
            }
            requestRestart(true);
        }
