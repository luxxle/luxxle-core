...
import org.chromium.chrome.browser.firstrun.LuxxleFirstRunFlowSequencer;
...
@NullMarked
public class LuxxleVpnSupportActivity extends AsyncInitializationActivity {
    private @Nullable LuxxleFirstRunFlowSequencer mFirstRunFlowSequencer;
    private final OneshotSupplier<ProfileProvider> mProfileSupplier;

    public LuxxleVpnSupportActivity() {
        mProfileSupplier = createProfileProvider();
    }

    private void initializeViews() {
        setContentView(R.layout.activity_luxxle_vpn_support);
                ...
                if (appReceiptSwitch.isChecked()) {
                    bodyText.append(
                            String.format(getResources().getString(R.string.playstore_token_text),
                                    LuxxleVpnPrefUtils.getPurchaseToken()));
                }
                if (appVersionSwitch.isChecked()) {
                    bodyText.append(String.format(
                            getResources().getString(R.string.app_version_text),
                            AboutChromeSettings.getApplicationVersion(LuxxleVpnSupportActivity.this,
    ...
    protected void triggerLayoutInflation() {
        mFirstRunFlowSequencer =
                new LuxxleFirstRunFlowSequencer(mProfileSupplier) {
                    @Override
    ...
    private void composeEmail(String body) {
        ...
        intent.putExtra(Intent.EXTRA_EMAIL, new String[] {"luxxleandroid@guardianapp.com"});
        intent.putExtra(Intent.EXTRA_SUBJECT, "Luxxle Firewall + VPN Issue");
