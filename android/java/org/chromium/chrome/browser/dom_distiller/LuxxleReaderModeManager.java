...
public class LuxxleReaderModeManager extends ReaderModeManager {
    // To be removed in bytecode, parent variable will be used instead.
    private Tab mTab;

    LuxxleReaderModeManager(Tab tab, Supplier<MessageDispatcher> messageDispatcherSupplier) {
    ...
    void spoofCustomTab(boolean spoof) {
        ...
        LuxxleActivity luxxleActivity =
                activity instanceof LuxxleActivity ? (LuxxleActivity) activity : null;
        if (luxxleActivity != null) {
            luxxleActivity.spoofCustomTab(spoof);
        }
    }
}
