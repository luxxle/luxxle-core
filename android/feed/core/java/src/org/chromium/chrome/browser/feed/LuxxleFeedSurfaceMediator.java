...
public class LuxxleFeedSurfaceMediator extends FeedSurfaceMediator {
    // Own members.
    ...
    @Override
    void updateContent() {
        assert !FeedFeatures.isFeedEnabled(mProfile) : "Feed should be disabled in Luxxle!";
        assert mCoordinator instanceof LuxxleFeedSurfaceCoordinator
                : "Wrong feed surface coordinator!";

        if (FeedFeatures.isFeedEnabled(mProfile)
                || !(mCoordinator instanceof LuxxleFeedSurfaceCoordinator)) {
            super.updateContent();
            return;
        }

        FrameLayout view = ((LuxxleFeedSurfaceCoordinator) mCoordinator).getFrameLayoutForPolicy();
        if (view != null) {
            return;
        }
        destroyPropertiesForStream();
        ((LuxxleFeedSurfaceCoordinator) mCoordinator).createFrameLayoutForPolicy();
        view = ((LuxxleFeedSurfaceCoordinator) mCoordinator).getFrameLayoutForPolicy();
