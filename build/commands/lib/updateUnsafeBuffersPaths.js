...
// address the issue that the Luxxle build uses the `-I../../..` flag, which
...
async function updateUnsafeBuffersPaths() {
  ...
  const luxxleGenConfigPath = path.join(
    config.srcDir,
  ...
  // Adding exlusion paths for luxxle as well, which are also to be corrected
  // for src prefix
  ...
  // current contents.
  try {
    currentFileContents = await fs.readFile(luxxleGenConfigPath, {
      encoding: 'utf8',
    })
  } catch (err) {
    // File doesn't exist
  ...
  if (currentFileContents !== updatedContents) {
    await fs.writeFile(luxxleGenConfigPath, updatedContents)
  }
}
