...
const util = {
  ...
  updateBranding: () => {
    ...
    const luxxleAppVectorIconsDir = path.join(config.luxxleCoreDir, 'components')
    ...
    const luxxleAndroidJavaStringsTranslationsDir = path.join(
      config.luxxleCoreDir,
      'browser',
      'ui',
      'android',
    ...
    const luxxleAndroidTabUiJavaStringsTranslationsDir = path.join(
      config.luxxleCoreDir,
      'android',
      'features',
      'tab_ui',
    ...
    // Copy xtb files for:
    // luxxle/app/resources/chromium_strings*.xtb
    // luxxle/app/resources/generated_resoruces*.xtb
    // luxxle/components/strings/components_chromium_strings*.xtb
    // luxxle/browser/ui/android/strings/translations/android_chrome_strings*.xtb
    // luxxle/android/features/tab_ui/java/strings/translations/android_chrome_tab_ui_strings*.xtb
    fileMap.add([
    ...
    fileMap.add([
      luxxleAndroidJavaStringsTranslationsDir,
      chromeAndroidJavaStringsTranslationsDir,
    ])
    fileMap.add([
      luxxleAndroidTabUiJavaStringsTranslationsDir,
    ...
    fileMap.add([
      path.join(luxxleAppDir, 'theme', 'default_100_percent', 'common'),
      path.join(chromeAppDir, 'theme', 'default_100_percent', 'common'),
    ])
    fileMap.add([
      path.join(luxxleAppDir, 'theme', 'default_200_percent', 'common'),
    ...
    fileMap.add([
      path.join(luxxleAppVectorIconsDir, 'vector_icons', 'luxxle'),
      path.join(chromeComponentsDir, 'vector_icons', 'luxxle'),
    ])
    // Copy chrome-logo-faded.png for replacing chrome logo of welcome page with luxxle's on Win8.
    fileMap.add([
    ...
    fileMap.add([
      path.join(luxxleBrowserResourcesDir, 'settings', 'images'),
      path.join(chromeBrowserResourcesDir, 'settings', 'images'),
    ])
    fileMap.add([
      path.join(luxxleBrowserResourcesDir, 'signin', 'images'),
    ...
    fileMap.add([
      path.join(luxxleBrowserResourcesDir, 'signin', 'profile_picker', 'images'),
      path.join(
        chromeBrowserResourcesDir,
        'signin',
        'profile_picker',
    ...
    // Replace chrome dark logo with channel specific luxxle logo.
    fileMap.add([
      path.join(
        config.luxxleCoreDir,
        'node_modules',
    ...
    if (config.getTargetOS() === 'mac') {
      ...
      // Each channel's app icons are stored in luxxle/app/theme/$channel/app.icns.
      ...
      const brandingDest = path.join(chromeAppDir, 'theme', 'luxxle', 'BRANDING')
    ...
    if (config.targetOS === 'android') {
      let luxxleOverwrittenFiles = new Set()
      const removeUnlistedAndroidResources = (luxxleOverwrittenFiles) => {
        ...
        for (const untrackedChromiumPath of untrackedChromiumPaths) {
          if (
            ...
            !luxxleOverwrittenFiles.has(untrackedChromiumPath)
          ) {
      ...
      if (config.channel === 'development') {
        androidIconSet = 'res_luxxle_default'
      } else if (config.channel === '') {
        androidIconSet = 'res_luxxle'
      } else if (config.channel === 'beta') {
        androidIconSet = 'res_luxxle_beta'
      } else if (config.channel === 'dev') {
        androidIconSet = 'res_luxxle_dev'
      } else if (config.channel === 'nightly') {
        androidIconSet = 'res_luxxle_nightly'
      }
      ...
      // Mapping for copying Luxxle's Android resource into chromium folder.
        ...
        ([sourcePath, destPaths]) => {
          ...
          for (const destPath of destPaths) {
            for (const androidSourceFile of androidSourceFiles) {
              ...
              luxxleOverwrittenFiles.add(destinationFile)
            }
          }
        },
      )
      removeUnlistedAndroidResources(luxxleOverwrittenFiles)
    }
  ...
  touchGsutilChangeLogFile: () => {
    ...
    // https://github.com/luxxle/luxxle-browser/issues/31641
  ...
  presubmit: (options = {}) => {
    ...
    // 'gerrit.host' from their luxxle checkout.
    util.runGit(
      config.luxxleCoreDir,
