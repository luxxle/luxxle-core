...
constexpr char kProductionValue[] = BUILDFLAG(LUXXLE_SERVICES_PRODUCTION_DOMAIN);
constexpr char kStagingValue[] = BUILDFLAG(LUXXLE_SERVICES_STAGING_DOMAIN);
constexpr char kDevValue[] = BUILDFLAG(LUXXLE_SERVICES_DEV_DOMAIN);
...
TEST(LuxxleServiceDomains, PrefixOverride) {
  ...
  auto prefixed_domain =
      GetServicesDomain(prefix, luxxle_domains::ServicesEnvironment::PROD, &cl);
  ...
  auto other_prefixed_domain = GetServicesDomain(
      other_prefix, luxxle_domains::ServicesEnvironment::PROD, &cl);
...
TEST(LuxxleServiceDomains, DefaultEnvValue) {
  ...
  result =
      GetServicesDomain(prefix, luxxle_domains::ServicesEnvironment::DEV, &cl);
  ...
  result = GetServicesDomain(prefix,
                             luxxle_domains::ServicesEnvironment::STAGING, &cl);
  EXPECT_EQ(result, base::StrCat({prefix, ".", kDevValue}));
