...
import {getTemplate} from './luxxle_leo_assistant_page.html.js'
import {LuxxleLeoAssistantBrowserProxy, LuxxleLeoAssistantBrowserProxyImpl, PremiumStatus, ModelWithSubtitle, PremiumInfo, ModelAccess, Model}
  from './luxxle_leo_assistant_browser_proxy.js'

const LuxxleLeoAssistantPageBase =
  WebUiListenerMixin(I18nMixin(PrefsMixin(PolymerElement)))

/**
 * 'settings-luxxle-leo-assistant-page' is the settings page containing
 * luxxle's Leo Assistant features.
 */
class LuxxleLeoAssistantPageElement extends LuxxleLeoAssistantPageBase {
    static get is() {
        return 'settings-luxxle-leo-assistant-page'
    }
    ...
    browserProxy_: LuxxleLeoAssistantBrowserProxy =
      LuxxleLeoAssistantBrowserProxyImpl.getInstance()
    manageUrl_: string | undefined = undefined

    onResetAssistantData_() {
      const message =
        this.i18n('luxxleLeoAssistantResetAndClearDataConfirmationText')
    ...
    override ready () {
      ...
      this.addWebUiListener('settings-luxxle-leo-assistant-changed',
    ...
    openTabOrganizationLearnMore_() {
      window.open(loadTimeData.getString('luxxleLeoAssistantTabOrganizationLearnMoreURL'), "_blank", "noopener noreferrer")
    }
    ...
    private onStorageEnabledChange_(event: Event) {
      ...
      // deleted.
      if (!target?.checked) {
        if (!confirm(this.i18n('luxxleLeoAssistantHistoryPreferenceConfirm'))) {
...
customElements.define(
  LuxxleLeoAssistantPageElement.is, LuxxleLeoAssistantPageElement)
