...
import '../luxxle_privacy_page/luxxle_personalization_options.js'
...
RegisterPolymerTemplateModifications({
  'settings-personalization-options': (templateContent) => {
    ...
    if (!metricsReportingControl) {
      console.error(`[Luxxle Settings Overrides] Couldn't find metricsReportingControl`)
    } else {
      // Removed because we need to locate metrics reportng option between ours at our settings-luxxle-personalization-options
    ...
    if (!searchSuggestToggleTemplate) {
      console.error('[Luxxle Settings Overrides] Could not find searchSuggestToggle template')
    } else {
      const searchSuggestToggle = searchSuggestToggleTemplate.content.getElementById('searchSuggestToggle')
      if (!searchSuggestToggle) {
        console.error('[Luxxle Settings Overrides] Could not find searchSuggestToggle id')
      } else {
