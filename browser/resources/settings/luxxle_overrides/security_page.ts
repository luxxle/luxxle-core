...
RegisterPolymerTemplateModifications({
  'settings-security-page': (templateContent) => {
    ...
    if (!safeBrowsingEnhanced) {
      console.error('[Luxxle Settings Overrides] Could not find ' +
    ...
    if (!safeBrowsingStandard) {
      console.error(
        '[Luxxle Settings Overrides] Could not find safeBrowsingStandard id ' +
        'on security page.')
    } else {
    ...
    if (!passwordsLeakToggle) {
      console.error(
        '[Luxxle Settings Overrides] Could not find passwordsLeakToggle id ' +
        'on security page.')
    } else {
    ...
    if (!enableCertManagementUIV2) {
      ...
      if (!chromeCertificates) {
        console.error(
          '[Luxxle Settings Overrides] Could not find chromeCertificates id ' +
          'on security page.')
      } else {
