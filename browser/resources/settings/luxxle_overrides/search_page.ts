...
import '../luxxle_search_engines_page/private_search_engine_list_dialog.js'
  ...
  'settings-search-page': (templateContent) => {
    ...
    if (!enginesSubpageTrigger) {
      ...
      enginesSubpageTrigger.insertAdjacentHTML(
        'beforebegin',
        getTrustedHTML`
          <settings-luxxle-search-page prefs="{{prefs}}">
          </settings-luxxle-search-page>
        `)
    }
...
RegisterPolymerComponentReplacement(
  ...
  class LuxxleSettingsSearchPageElement
