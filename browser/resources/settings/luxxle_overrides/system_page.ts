...
import '../luxxle_system_page/luxxle_performance_page.js'
// <if expr="enable_luxxle_vpn_wireguard">
import '../luxxle_system_page/luxxle_vpn_page.js'
// </if>
  ...
  'settings-system-page': (templateContent) => {
        ...
        <settings-animated-pages id="pages" section="system">
          <div route-path="default">
            <cr-link-row
              ...
              id="shortcutsButton"
              label=${loadTimeData.getString('luxxleShortcutsPage')}
              role-description=
          ...
          <template is="dom-if" route-path="/system/shortcuts">
            <settings-subpage
              ...
              page-title=${loadTimeData.getString('luxxleShortcutsPage')}>
    ...
    templateContent.appendChild(
      html`
        <settings-toggle-button
          class="cr-row"
          pref="{{prefs.luxxle.enable_closing_last_tab}}"
          label="${loadTimeData.getString('luxxleClosingLastTab')}"
        >
        </settings-toggle-button>
      `
    )
    ...
    templateContent.appendChild(
      html`
        <settings-toggle-button
          class="cr-row hr"
          pref="{{prefs.luxxle.enable_window_closing_confirm}}"
          label="${loadTimeData.getString('luxxleWarnBeforeClosingWindow')}">
        </settings-toggle-button>
      `
    )
    ...
    templateContent.appendChild(
      html`
        <settings-toggle-button
          class="cr-row"
          pref="{{prefs.luxxle.show_fullscreen_reminder}}"
          label="${loadTimeData.getString('luxxleShowFullscreenReminder')}"
        >
        </settings-toggle-button>
      `
    )
    ...
    // <if expr="enable_luxxle_vpn_wireguard">
    let showVpnPage = loadTimeData.getBoolean('isLuxxleVPNEnabled')
    // <if expr="is_macosx">
    showVpnPage = showVpnPage &&
                  loadTimeData.getBoolean('isLuxxleVPNWireguardEnabledOnMac')
    // </if>
    if (showVpnPage) {
      templateContent.appendChild(
        html`
          <settings-luxxle-vpn-page prefs="{{prefs}}">
          </settings-luxxle-vpn-page>
        `
      )
    }
    // </if>
    ...
    templateContent.appendChild(
      html`
        <settings-luxxle-performance-page prefs="{{prefs}}">
        </settings-luxxle-performance-page>
      `
    )
  }
})
