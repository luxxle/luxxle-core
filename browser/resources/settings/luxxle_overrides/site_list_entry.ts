...
RegisterPolymerTemplateModifications({
  'site-list-entry': (templateContent) => {
    ...
    if (!resetSite) {
      console.error(
        '[Luxxle Settings Overrides] Could not find reset site button'
      )
      return
    }
...
RegisterPolymerComponentReplacement(
  'site-list-entry',
  class LuxxleSiteListEntryElement extends SiteListEntryElement {
    private shouldDisableReset_() {
      return this.model.luxxleCookieType !== undefined
    }
    ...
    private onResetSiteShowTooltip_() {
      ...
      if (
        this.model.luxxleCookieType === LuxxleCookieTypes.SHIELDS_SETTINGS ||
        this.model.luxxleCookieType === LuxxleCookieTypes.SHIELDS_DOWN
      ) {
        const text = this.i18n('cookieControlledByShieldsTooltip')
        this.fire('show-tooltip', { target: button, text })
      } else if (
        this.model.luxxleCookieType === LuxxleCookieTypes.GOOGLE_SIGN_IN
      ) {
