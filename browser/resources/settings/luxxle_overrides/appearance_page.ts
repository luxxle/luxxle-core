...
import '../luxxle_appearance_page/super_referral.js'
import '../luxxle_appearance_page/luxxle_theme.js'
...
RegisterPolymerTemplateModifications({
  'settings-appearance-page': (templateContent) => {
    ...
    if (!theme) {
      ...
      // Insert Luxxle colors dropdown before theme.
      theme.setAttribute('class', 'settings-row hr')
      theme.insertAdjacentHTML(
        'beforebegin',
        getTrustedHTML`
          <settings-luxxle-appearance-theme prefs="{{prefs}}">
          </settings-luxxle-appearance-theme>
      `)
      ...
      if (!openTheme) {
        ...
        // LuxxleSettingsAppearancePageElement below.
          ...
          html`
            <template is="dom-if" route-path="/themes">
              <settings-subpage
                ...
                <settings-luxxle-appearance-super-referral prefs="{{prefs}}">
                </settings-luxxle-appearance-super-referral>
              </settings-subpage>
            </template>
          `)
      }
    ...
    if (!bookmarkBarToggle) {
      ...
      // settings-luxxle-appearance-bookmark-bar
...
RegisterPolymerComponentReplacement(
  ...
  class LuxxleSettingsAppearancePageElement
