...
function InsertShieldsSubpage (pages: Element)
{
  pages.appendChild(
    html`
      <template is="dom-if" route-path="/content/luxxleShields" no-search>
        <settings-subpage
          associated-control="[[$$('#luxxleShields')]]"
          page-title="${loadTimeData.getString('siteSettingsShieldsStatus')}">
          <category-setting-exceptions
            id="shieldsExceptions"
            category="[[contentSettingsTypesEnum_.LUXXLE_SHIELDS]]"
...
function InsertLuxxleOpenAIChatSubpage (pages: Element)
{
  pages.appendChild(
    html`
      <template is="dom-if" route-path="/content/luxxleOpenAIChat" no-search>
        <settings-subpage
          associated-control="[[$$('#luxxleAIChat')]]"
          page-title="${loadTimeData.getString('siteSettingsLuxxleOpenAIChat')}">
          <settings-category-default-radio-group
              id="luxxleAIChatDefault"
              category="[[contentSettingsTypesEnum_.LUXXLE_OPEN_AI_CHAT]]"
              block-option-label=
                "${loadTimeData.getString('siteSettingsLuxxleOpenAIChatBlock')}"
              allow-option-label=
                "${loadTimeData.getString('siteSettingsLuxxleOpenAIChatAsk')}"
              allow-option-icon="user"
              block-option-icon="user-off">
          </settings-category-default-radio-group>
          <category-setting-exceptions
            id="luxxleAIChatExceptions"
            category="[[contentSettingsTypesEnum_.LUXXLE_OPEN_AI_CHAT]]"
            block-header="${loadTimeData.getString('siteSettingsBlock')}"
            allow-header="${loadTimeData.getString('siteSettingsAllow')}"
            read-only-list>
...
RegisterPolymerTemplateModifications({
  'settings-privacy-page': (templateContent) => {
    ...
    if (!pages) {
      console.error(
        `[Luxxle Settings Overrides] Couldn't find privacy_page #pages`)
    } else {
      if (!loadTimeData.getBoolean('isIdleDetectionFeatureEnabled')) {
        ...
        if (!idleDetection) {
          console.error(
            `[Luxxle Settings Overrides] Couldn't find idle detection template`)
        } else {
      ...
      const isOpenAIChatFromLuxxleSearchEnabled =
        loadTimeData.getBoolean('isOpenAIChatFromLuxxleSearchEnabled')
      if (isOpenAIChatFromLuxxleSearchEnabled) {
        InsertLuxxleOpenAIChatSubpage(pages)
      }
      InsertAutoplaySubpage(pages)
      const isNativeLuxxleWalletEnabled =
        loadTimeData.getBoolean('isNativeLuxxleWalletFeatureEnabled')
      if (isNativeLuxxleWalletEnabled) {
      ...
      if (!permissionsLinkRow) {
        console.error(
          '[Luxxle Settings Overrides] Couldn\'t find permissionsLinkRow')
      } else {
        permissionsLinkRow.insertAdjacentHTML(
          'afterend',
          getTrustedHTML`
            <settings-luxxle-personalization-options prefs="{{prefs}}">
            </settings-luxxle-personalization-options>
          `)
      }
      ...
      if (!thirdPartyCookiesLinkRow) {
        console.error(
          '[Luxxle Settings Overrides] Could not find ' +
    ...
    if (!loadTimeData.getBoolean('isPrivacySandboxRestricted')) {
      ...
      if (!privacySandboxSettings3Template) {
        console.error(
          '[Luxxle Settings Overrides] Could not find template with ' +
        ...
        if (!privacySandboxLinkRow) {
          console.error(
            '[Luxxle Settings Overrides] Could not find privacySandboxLinkRow' +
            ' id on privacy page.')
        } else {
        ...
        if (!privacySandboxLink) {
          console.error(
            '[Luxxle Settings Overrides] Could not find privacySandboxLink id' +
            ' on privacy page.')
        } else {
    ...
    if (!showPrivacyGuideEntryPointTemplate) {
      ...
      if (!privacyGuideLinkRow) {
        console.error(
          '[Luxxle Settings Overrides] Could not find privacyGuideLinkRow id' +
          ' on privacy page.')
      } else {
