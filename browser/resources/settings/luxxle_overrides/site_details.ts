...
import { RegisterPolymerTemplateModifications, html } from 'chrome://resources/luxxle/polymer_overriding.js'
  ...
  'site-details': (templateContent: HTMLTemplateElement) => {
    ...
    if (!firstPermissionItem) {
      ...
      if (isOpenAIChatFromLuxxleSearchEnabled) {
        ...
        const luxxleAIChatSettings = templateContent.querySelector(
          `div.list-frame > site-details-permission:nth-child(${curChild})`)
        if (!luxxleAIChatSettings) {
          console.error('[Settings] Couldn\'t find Luxxle AI chat settings')
        } else {
          luxxleAIChatSettings.setAttribute(
            'label', loadTimeData.getString('siteSettingsLuxxleOpenAIChat'))
        }
        curChild++
      }
