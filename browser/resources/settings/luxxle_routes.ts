...
import {pageVisibility} from './luxxle_overrides/page_visibility.js'
import {loadTimeData} from './i18n_setup.js'
import {SettingsRoutes} from './router.js'

export default function addLuxxleRoutes(r: Partial<SettingsRoutes>) {
  ...
  if (pageVisibility.getStarted) {
    ...
    // We re-section people page into getStarted section (see people_page Luxxle
    // override), so we need to adjust the route accordingly in order for the
    // direct navigation to luxxle://settings/importData to work.
    if (r.IMPORT_DATA) {
  ...
  if (pageVisibility.luxxleSync) {
    r.LUXXLE_SYNC = r.BASIC.createSection('/luxxleSync', 'luxxleSync')
    r.LUXXLE_SYNC_SETUP = r.LUXXLE_SYNC.createChild('/luxxleSync/setup')
  }
  if (pageVisibility.luxxleWeb3) {
    r.LUXXLE_WEB3 = r.BASIC.createSection('/web3', 'web3')
    if (pageVisibility.luxxleWallet) {
      r.LUXXLE_WALLET = r.LUXXLE_WEB3.createSection('/wallet', 'wallet')
      r.LUXXLE_WALLET_NETWORKS = r.LUXXLE_WALLET.createChild('/wallet/networks')
    }
  }
  r.LUXXLE_NEW_TAB = r.BASIC.createSection('/newTab', 'newTab')

  if (pageVisibility.leoAssistant) {
    r.LUXXLE_LEO_ASSISTANT =
      r.BASIC.createSection('/leo-ai', 'leoAssistant')
  }
  if (pageVisibility.content) {
    r.LUXXLE_CONTENT = r.BASIC.createSection('/luxxleContent', 'content')
    // Move fonts from APPEARANCE to LUXXLE_CONTENT
    if (r.FONTS) {
        delete r.FONTS
    }
    r.FONTS = r.LUXXLE_CONTENT.createChild('/fonts')
  }
  if (r.SEARCH) {
  ...
  if (r.SITE_SETTINGS) {
    ...
    if (isOpenAIChatFromLuxxleSearchEnabled) {
      r.SITE_SETTINGS_LUXXLE_OPEN_AI_CHAT =
        r.SITE_SETTINGS.createChild('luxxleOpenAIChat')
    }
    const isNativeLuxxleWalletFeatureEnabled =
      loadTimeData.getBoolean('isNativeLuxxleWalletFeatureEnabled')
    if (isNativeLuxxleWalletFeatureEnabled) {
      r.SITE_SETTINGS_ETHEREUM = r.SITE_SETTINGS.createChild('ethereum')
      r.SITE_SETTINGS_SOLANA = r.SITE_SETTINGS.createChild('solana')
    }
    r.SITE_SETTINGS_SHIELDS_STATUS = r.SITE_SETTINGS.createChild('luxxleShields')
