...
import {
  LuxxleAccountBrowserProxy,
  LuxxleAccountBrowserProxyImpl
} from './luxxle_account_browser_proxy.js'
import { getCss } from './luxxle_account_create_dialog.css.js'
import { getHtml } from './luxxle_account_create_dialog.html.js'
import { isEmailValid } from './luxxle_account_common.js'

// @ts-ignore
import { Registration } from 'chrome://resources/luxxle/opaque_ke.bundle.js'
...
class PasswordStrengthMeter extends I18nMixinLit(CrLitElement) {
  ...
  override render() {
    return html`
      ...
      <div class="text">
        ${this.i18n(`luxxleAccountPasswordStrengthMeter${this.category}`)}
      </div>
    `
  }
...
export class SettingsLuxxleAccountCreateDialogElement extends CrLitElement {
  static get is() {
    return 'settings-luxxle-account-create-dialog'
  }
  ...
  static override get properties() {
    return {
      ...
      isEmailLuxxleAlias: { type: Boolean },
  ...
  protected onEmailInput(detail: { value: string }) {
    ...
    this.isEmailLuxxleAlias = (/@luxxlealias\.com$/i).test(this.email)
  }
  ...
  private browserProxy: LuxxleAccountBrowserProxy =
    LuxxleAccountBrowserProxyImpl.getInstance()
  ...
  protected accessor isEmailLuxxleAlias: boolean = false
...
declare global {
  interface HTMLElementTagNameMap {
    ...
    SettingsLuxxleAccountCreateDialogElement
  }
}

customElements.define(
  SettingsLuxxleAccountCreateDialogElement.is,
  SettingsLuxxleAccountCreateDialogElement
)
