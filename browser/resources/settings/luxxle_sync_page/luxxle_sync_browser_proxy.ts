...
export interface LuxxleSyncStatus extends SyncStatus {
  hasSyncWordsDecryptionError: boolean
  isOsEncryptionAvailable: boolean
}

export class LuxxleSyncBrowserProxy {
  ...
  getDeviceList(): Promise<LuxxleDeviceInfo[]> {
  ...
  getSyncStatus(): Promise<LuxxleSyncStatus> {
  ...
  static getInstance() {
    return instance || (instance = new LuxxleSyncBrowserProxy())
  }
}

let instance: LuxxleSyncBrowserProxy|null = null
