...
import './luxxle_sync_code_dialog.js';
import './luxxle_sync_delete_account_dialog.js';
...
import {LuxxleDeviceInfo, LuxxleSyncBrowserProxy} from './luxxle_sync_browser_proxy.js';
import {getTemplate} from './luxxle_sync_configure.html.js'

/**
 * @fileoverview
 * 'settings-luxxle-sync-configure' is the set of controls which fetches, displays
 * and updates the sync configuration.
 */

const SettingsLuxxleSyncConfigureElementBase =
...
export class SettingsLuxxleSyncConfigureElement extends SettingsLuxxleSyncConfigureElementBase {
  static get is() {
    return 'settings-luxxle-sync-configure'
  }
  ...
  private declare deviceList_: LuxxleDeviceInfo[];
  private declare syncCodeDialogType_: string | null;
  private declare syncDoingDeleteAccount_: Boolean | false;

  browserProxy_: LuxxleSyncBrowserProxy = LuxxleSyncBrowserProxy.getInstance();
  ...
  handleDeviceInfo_(deviceList: LuxxleDeviceInfo[]) {
    this.deviceList_ = deviceList
  }

  getDeviceDisplayDate(device: LuxxleDeviceInfo): string {
  ...
  async onResetSyncChain_() {
    const messageText = this.i18n('luxxleSyncResetConfirmation')
    ...
    router.navigateTo((router.getRoutes() as {LUXXLE_SYNC: Route}).LUXXLE_SYNC);
  }
  ...
  async onDeleteDevice_(e: DomRepeatEvent<LuxxleDeviceInfo>) {
    const messageText = this.i18n('luxxleSyncDeleteDeviceConfirmation')
    const shouldDeleteDevice = confirm(messageText)
    if (!shouldDeleteDevice) {
      return
    }
    const device: LuxxleDeviceInfo = e.model.item
    await this.browserProxy_.deleteDevice(device.guid);
  }
}

customElements.define(
  SettingsLuxxleSyncConfigureElement.is, SettingsLuxxleSyncConfigureElement)
