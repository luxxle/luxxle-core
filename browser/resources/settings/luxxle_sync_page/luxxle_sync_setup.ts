...
import {LuxxleSyncBrowserProxy} from './luxxle_sync_browser_proxy.js';
import {getTemplate} from './luxxle_sync_setup.html.js'

/**
 * @fileoverview
 * 'luxxle-sync-setup' is the UI for starting or joining a sync chain
 * settings.
 */

const SettingsLuxxleSyncSetupElementBase =
  I18nMixin(BaseMixin(PolymerElement)) as {
    new(): PolymerElement & I18nMixinInterface
  }

export class SettingsLuxxleSyncSetupElement extends SettingsLuxxleSyncSetupElementBase {
  static get is() {
    return 'settings-luxxle-sync-setup'
  }
  ...
  syncBrowserProxy_: LuxxleSyncBrowserProxy = LuxxleSyncBrowserProxy.getInstance();
  ...
  handleSyncCodeDialogDone_() {
    if (this.syncCodeDialogType === 'input') {
      const messageText = this.i18n('luxxleSyncFinalSecurityWarning')
...
customElements.define(
  SettingsLuxxleSyncSetupElement.is, SettingsLuxxleSyncSetupElement)
