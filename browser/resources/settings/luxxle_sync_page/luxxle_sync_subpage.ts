...
import './luxxle_sync_configure.js'
import './luxxle_sync_setup.js'
...
import {
  LuxxleSyncBrowserProxy,
  LuxxleSyncStatus
} from './luxxle_sync_browser_proxy.js'
import {SyncPrefs} from '/shared/settings/people_page/sync_browser_proxy.js';

import {getTemplate} from './luxxle_sync_subpage.html.js'

type PageStatus = 'configure' | 'setup' | 'spinner'

const SettingLuxxleSyncSubpageBase =
  I18nMixin(RouteObserverMixin(PolymerElement))

class SettingLuxxleSyncSubpage extends SettingLuxxleSyncSubpageBase {
  static get is() {
    return 'settings-luxxle-sync-subpage'
  }
  ...
  private declare syncStatus: LuxxleSyncStatus
  ...
  private luxxleSyncBrowserProxy_: LuxxleSyncBrowserProxy =
    LuxxleSyncBrowserProxy.getInstance()
  ...
  override currentRouteChanged() {
    ...
    if (router.getCurrentRoute() === router.getRoutes().LUXXLE_SYNC_SETUP) {
      this.onNavigateToPage_()
      return
    }

    if (router.getRoutes().LUXXLE_SYNC_SETUP.
  ...
  private onNavigateToPage_() {
    const router = Router.getInstance()
    assert(router.getCurrentRoute() === router.getRoutes().LUXXLE_SYNC_SETUP)
    if (this.beforeunloadCallback_) {
      return
    }
  ...
  private onSetupSuccess_() {
    ...
    const router = Router.getInstance()
    if (router.getCurrentRoute() === router.getRoutes().LUXXLE_SYNC_SETUP) {
      router.navigateTo(router.getRoutes().LUXXLE_SYNC)
    }
  }
  ...
  async onRejoin_(_e: Event) {
    ...
    await this.luxxleSyncBrowserProxy_.resetSyncChain()
...
customElements.define(
  SettingLuxxleSyncSubpage.is, SettingLuxxleSyncSubpage)
