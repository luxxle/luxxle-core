...
import {
  LuxxleWalletBrowserProxy,
  LuxxleWalletBrowserProxyImpl,
  type Option,
  type SolanaProvider
} from './luxxle_wallet_browser_proxy.js'

import {assert} from 'chrome://resources/js/assert.js'

import {getTemplate} from './luxxle_wallet_page.html.js'

interface SettingsLuxxleWalletPage {
  $: {
...
const SettingsLuxxleWalletPageBase =
  WebUiListenerMixin(PrefsMixin(I18nMixin(RouteObserverMixin(PolymerElement))))

class SettingsLuxxleWalletPage extends SettingsLuxxleWalletPageBase {
  static get is() {
    return 'settings-luxxle-wallet-page'
  }
  ...
  private browserProxy_: LuxxleWalletBrowserProxy =
    LuxxleWalletBrowserProxyImpl.getInstance()
  ...
  private isNetworkEditorRoute() {
    ...
    return (router.getCurrentRoute() ===
      router.getRoutes().LUXXLE_WALLET_NETWORKS)
  }
  ...
  private onInputAutoLockMinutes_() {
    ...
    this.setPrefValue('luxxle.wallet.auto_lock_minutes', value)
  }

  private onWalletNetworksEditorClick_() {
    const router = Router.getInstance()
    router.navigateTo(router.getRoutes().LUXXLE_WALLET_NETWORKS)
  }
...
customElements.define(
  SettingsLuxxleWalletPage.is, SettingsLuxxleWalletPage)
