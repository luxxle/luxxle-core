...
import { SponsoredRichMediaAdEventHandler } from 'gen/luxxle/components/ntp_background_images/browser/mojom/ntp_background_images.mojom.m.js'
...
export function createBackgroundHandler(
  ...
  async function updateLuxxleBackgrounds() {
    const { backgrounds } = await handler.getLuxxleBackgrounds()
    store.update({ luxxleBackgrounds: backgrounds })
  }
