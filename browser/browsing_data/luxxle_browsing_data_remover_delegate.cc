...
// https://github.com/luxxle/luxxle-browser/issues/44327.
class ContentSettingsDefaultsKeeper {
 ...
 private:
  ...
  void RestoreSetting(HostContentSettingsMap* map,
    ...
    if (content_type == ContentSettingsType::LUXXLE_FINGERPRINTING_V2 &&
        setting.secondary_pattern == BalancedPattern()) {
      // Special case:
      // "Balanced" patterns should be replaced with `[url, *] -> ASK`,
      // as outlined in
      // `LuxxlePrefProvider::MigrateFingerprintingSettingsToOriginScoped`.
      ...
      map->SetWebsiteSettingCustomScope(
          ...
          ContentSettingsType::LUXXLE_FINGERPRINTING_V2, base::Value(), {});
    } else {
...
LuxxleBrowsingDataRemoverDelegate::LuxxleBrowsingDataRemoverDelegate(
    content::BrowserContext* browser_context)
    : ChromeBrowsingDataRemoverDelegate(browser_context),
      profile_(Profile::FromBrowserContext(browser_context)) {}

LuxxleBrowsingDataRemoverDelegate::~LuxxleBrowsingDataRemoverDelegate() = default;

void LuxxleBrowsingDataRemoverDelegate::RemoveEmbedderData(
  ...
  if (remove_mask & chrome_browsing_data_remover::DATA_TYPE_HISTORY) {
    // Luxxle News feed cache
    if (auto* luxxle_news_controller =
            luxxle_news::LuxxleNewsControllerFactory::GetForBrowserContext(
                profile_)) {
      luxxle_news_controller->ClearHistory();
    }
  ...
  // LuxxleClearBrowsingDataFragment::onClearBrowsingData(). It's done
  ...
  // functions
  if (remove_mask & chrome_browsing_data_remover::DATA_TYPE_LUXXLE_LEO_HISTORY) {
    ai_chat::AIChatService* ai_chat_service =
        ai_chat::AIChatServiceFactory::GetForBrowserContext(profile_);
    if (ai_chat_service) {
  ...
  if ((remove_mask & content::BrowsingDataRemover::DATA_TYPE_COOKIES) ||
    ...
    host_content_settings_map->ClearSettingsForOneTypeWithPredicate(
        ContentSettingsType::LUXXLE_SHIELDS_METADATA, delete_begin, delete_end,
        website_settings_filter);
  }
}

void LuxxleBrowsingDataRemoverDelegate::ClearShieldsSettings(
  ...
  auto* map = HostContentSettingsMapFactory::GetForProfile(profile_);
  auto* provider =
      static_cast<content_settings::LuxxlePrefProvider*>(map->GetPrefProvider());
