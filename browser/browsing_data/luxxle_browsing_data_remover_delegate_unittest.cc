...
class LuxxleBrowsingDataRemoverDelegateTest : public testing::Test {
 public:
  void SetUp() override {
  ...
  LuxxleBrowsingDataRemoverDelegate* delegate() {
    return static_cast<LuxxleBrowsingDataRemoverDelegate*>(
...
TEST_F(LuxxleBrowsingDataRemoverDelegateTest, ShieldsSettingsClearTest) {
  const GURL kLuxxleURL("https://www.luxxle.com");
  ...
  map()->SetContentSettingDefaultScope(
      kLuxxleURL, GURL(), ContentSettingsType::LUXXLE_HTTP_UPGRADABLE_RESOURCES,
      CONTENT_SETTING_ALLOW);
  map()->SetContentSettingDefaultScope(
      kBatURL, GURL(), ContentSettingsType::LUXXLE_FINGERPRINTING_V2,
      CONTENT_SETTING_ALLOW);
  map()->SetContentSettingCustomScope(
      luxxle_shields::GetPatternFromURL(kGoogleURL),
...
TEST_F(LuxxleBrowsingDataRemoverDelegateTest, ShieldsSettingsKeepDefaults) {
  luxxle_shields::SetAdControlType(map(), luxxle_shields::ControlType::BLOCK,
                                  GURL());
  luxxle_shields::SetCosmeticFilteringControlType(
      map(), luxxle_shields::ControlType::BLOCK, GURL());

  EXPECT_EQ(luxxle_shields::DomainBlockingType::kAggressive,
            luxxle_shields::GetDomainBlockingType(map(), GURL()));
...
TEST_F(LuxxleBrowsingDataRemoverDelegateTest, FingerpintV2ClearBalancedPattern) {
  ...
  EXPECT_EQ(
      CONTENT_SETTING_ASK,  // The default value for LUXXLE_FINGERPRINTING_V2
      map()->GetContentSetting(GURL(), GURL("https://balanced/"),
                               ContentSettingsType::LUXXLE_FINGERPRINTING_V2));
}
