...
using content::LuxxleClearBrowsingData;
...
class LuxxleClearDataOnExitTest
    : public InProcessBrowserTest,
      public LuxxleClearBrowsingData::OnExitTestingCallback {
 public:
  LuxxleClearDataOnExitTest() = default;
  LuxxleClearDataOnExitTest(const LuxxleClearDataOnExitTest&) = delete;
  LuxxleClearDataOnExitTest& operator=(const LuxxleClearDataOnExitTest&) = delete;

  void SetUpOnMainThread() override {
    LuxxleClearBrowsingData::SetOnExitTestingCallback(this);
  }
  ...
  void TearDownInProcessBrowserTestFixture() override {
    ...
    LuxxleClearBrowsingData::SetOnExitTestingCallback(nullptr);
  }
  ...
  void SetClearAll(PrefService* prefService) {
    ...
    prefService->SetBoolean(browsing_data::prefs::kDeleteSiteSettingsOnExit,
                            true);
    prefService->SetBoolean(browsing_data::prefs::kDeleteLuxxleLeoHistoryOnExit,
                            true);
  }
  ...
  uint64_t GetRemoveMaskAll() {
    return chrome_browsing_data_remover::DATA_TYPE_HISTORY |
           ...
           chrome_browsing_data_remover::DATA_TYPE_LUXXLE_LEO_HISTORY |
  ...
  // LuxxleClearBrowsingData::OnExitTestingCallback implementation.
...
IN_PROC_BROWSER_TEST_F(LuxxleClearDataOnExitTest, NoPrefsSet) {
...
IN_PROC_BROWSER_TEST_F(LuxxleClearDataOnExitTest, VerifyRemovalMasks) {
...
class LuxxleClearDataOnExitTwoBrowsersTest : public LuxxleClearDataOnExitTest {
 public:
  LuxxleClearDataOnExitTwoBrowsersTest() { browsers_count_ = 2u; }

  LuxxleClearDataOnExitTwoBrowsersTest(
      const LuxxleClearDataOnExitTwoBrowsersTest&) = delete;
  LuxxleClearDataOnExitTwoBrowsersTest& operator=(
      const LuxxleClearDataOnExitTwoBrowsersTest&) = delete;
...
IN_PROC_BROWSER_TEST_F(LuxxleClearDataOnExitTwoBrowsersTest, SameProfile) {
...
IN_PROC_BROWSER_TEST_F(LuxxleClearDataOnExitTwoBrowsersTest, OneOTR) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
...
IN_PROC_BROWSER_TEST_F(LuxxleClearDataOnExitTwoBrowsersTest, OneOTRExitsLast) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
  // OTR sessions don't count, so expect a single call.
  SetExepectedRemoveDataCallCount(1);
...
IN_PROC_BROWSER_TEST_F(LuxxleClearDataOnExitTwoBrowsersTest, OneGuest) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
...
IN_PROC_BROWSER_TEST_F(LuxxleClearDataOnExitTwoBrowsersTest, OneGuestExitsLast) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
  // Guest sessions don't count, so expect a single call.
  SetExepectedRemoveDataCallCount(1);
...
IN_PROC_BROWSER_TEST_F(LuxxleClearDataOnExitTwoBrowsersTest, TwoProfiles) {
  // Delete browsing history on exit.
  SetDeleteBrowsingHistoryOnExit();
