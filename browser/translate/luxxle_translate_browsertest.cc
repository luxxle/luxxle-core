...
class LuxxleTranslateBrowserTest : public InProcessBrowserTest {
 public:
  LuxxleTranslateBrowserTest()
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_.RegisterRequestHandler(base::BindRepeating(
        &LuxxleTranslateBrowserTest::HandleRequest, base::Unretained(this)));
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    // Remap translate.luxxle.com requests to the https test server.
    const std::string host_port = https_server_.host_port_pair().ToString();
    command_line->AppendSwitchASCII(network::switches::kHostResolverRules,
                                    "MAP translate.luxxle.com:443 " + host_port +
  ...
  std::unique_ptr<net::test_server::HttpResponse> HandleRequest(
    ...
    if (request.GetURL().path() == "/translate") {
      ...
      EXPECT_NE(query.find(base::StringPrintf("&key=%s",
                ...
                std::string::npos)
          << "bad luxxle api key for request " << request.GetURL();
    }
...
IN_PROC_BROWSER_TEST_F(LuxxleTranslateBrowserTest, InternalTranslation) {
  ResetObserver();
...
IN_PROC_BROWSER_TEST_F(LuxxleTranslateBrowserTest, NoAutoTranslate) {
...
class LuxxleTranslateBrowserGoogleRedirectTest
    : public LuxxleTranslateBrowserTest {
 public:
  void SetUpCommandLine(base::CommandLine* command_line) override {
    LuxxleTranslateBrowserTest::SetUpCommandLine(command_line);
    const std::string host_port = https_server_.host_port_pair().ToString();
...
IN_PROC_BROWSER_TEST_F(LuxxleTranslateBrowserGoogleRedirectTest,
  ...
  EXPECT_EQ("https://translate.luxxle.com/something.svg",
