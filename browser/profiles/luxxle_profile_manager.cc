...
void RecordInitialP3AValues(Profile* profile) {
  ...
  if (profile->IsRegularProfile()) {
    luxxle_shields::MaybeRecordInitialShieldsSettings(
        profile->GetPrefs(),
...
LuxxleProfileManager::LuxxleProfileManager(const base::FilePath& user_data_dir)
    : ProfileManager(user_data_dir) {
  MigrateProfileNames();
}

void LuxxleProfileManager::InitProfileUserPrefs(Profile* profile) {
  ...
  // LuxxlePrefProvider
  content_settings::LuxxlePrefProvider::CopyPluginSettingsForMigration(
      profile->GetPrefs());
...
// Chromecast is enabled by default on Android.
  ...
  if (pref_service->FindPreference(kEnableMediaRouterOnRestart)
    ...
    // state of the media router switch in luxxle://settings/extensions. The
  ...
  perf::MaybeEnableLuxxleFeatureForPerfTesting(profile);
  MigrateHttpsUpgradeSettings(profile);
}

void LuxxleProfileManager::DoFinalInitForServices(Profile* profile,
...
#if !BUILDFLAG(USE_GCM_FROM_PLATFORM)
  gcm::LuxxleGCMChannelStatus* status =
...
bool LuxxleProfileManager::IsAllowedProfilePath(
...
bool LuxxleProfileManager::LoadProfileByPath(const base::FilePath& profile_path,
                                            bool incognito,
...
void LuxxleProfileManager::SetNonPersonalProfilePrefs(Profile* profile) {
...
void LuxxleProfileManager::MigrateProfileNames() {
...
LuxxleProfileManagerWithoutInit::LuxxleProfileManagerWithoutInit(
    const base::FilePath& user_data_dir)
    : LuxxleProfileManager(user_data_dir) {
