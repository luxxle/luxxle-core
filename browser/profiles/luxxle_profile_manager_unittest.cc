// Copyright 2021 The Luxxle Authors. All rights reserved.
...
class LuxxleProfileManagerTest : public testing::Test {
 public:
  LuxxleProfileManagerTest()
 ...
 protected:
  std::unique_ptr<LuxxleProfileManager> CreateProfileManagerForTest() {
    return std::make_unique<LuxxleProfileManagerWithoutInit>(
        temp_dir_.GetPath());
  }
...
TEST_F(LuxxleProfileManagerTest, EnableMediaRouterOnRestartDefaultValue) {
...
TEST_F(LuxxleProfileManagerTest, EnableMediaRouterOnRestartNonDefaultValue) {
  ProfileManager* profile_manager = g_browser_process->profile_manager();
  ASSERT_TRUE(profile_manager);
