...
class LuxxleProfileManagerTest : public PlatformBrowserTest {
};

// Test that legacy profile names (Person X) that have
// not been user-modified are automatically renamed
// to luxxle profile names (Profile X).
IN_PROC_BROWSER_TEST_F(LuxxleProfileManagerTest,
...
IN_PROC_BROWSER_TEST_F(LuxxleProfileManagerTest,
  ...
  EXPECT_EQ(luxxle_rewards::RewardsServiceFactory::GetForProfile(guest_profile),
            nullptr);
  EXPECT_EQ(luxxle_ads::AdsServiceFactory::GetForProfile(guest_profile),
            nullptr);

  ASSERT_TRUE(otr_profile->IsOffTheRecord());

  EXPECT_NE(luxxle_rewards::RewardsServiceFactory::GetForProfile(profile),
            nullptr);
  EXPECT_EQ(luxxle_rewards::RewardsServiceFactory::GetForProfile(otr_profile),
            nullptr);

  EXPECT_NE(luxxle_ads::AdsServiceFactory::GetForProfile(profile), nullptr);
  EXPECT_EQ(luxxle_ads::AdsServiceFactory::GetForProfile(otr_profile), nullptr);
}
#endif
...
IN_PROC_BROWSER_TEST_F(LuxxleProfileManagerTest, MediaRouterEnabledRestartTest) {
  Profile* profile = g_browser_process->profile_manager()->GetLastUsedProfile();
  {
    EXPECT_TRUE(profile->GetPrefs()->GetBoolean(::prefs::kEnableMediaRouter));
    EXPECT_TRUE(profile->GetPrefs()->GetBoolean(kEnableMediaRouterOnRestart));
