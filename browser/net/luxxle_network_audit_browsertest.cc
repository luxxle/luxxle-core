...
class LuxxleNetworkAuditTest : public InProcessBrowserTest {
 public:
  LuxxleNetworkAuditTest() {
#if BUILDFLAG(ENABLE_PLAYLIST_WEBUI)
    scoped_feature_list_.InitAndEnableFeature(playlist::features::kPlaylist);
#endif  // BUILDFLAG(ENABLE_PLAYLIST_WEBUI)
  }

  LuxxleNetworkAuditTest(const LuxxleNetworkAuditTest&) = delete;
  LuxxleNetworkAuditTest& operator=(const LuxxleNetworkAuditTest&) = delete;
...
// Loads luxxle://welcome first to simulate a first run and then loads another
// URL, waiting some time after each load to allow gathering network requests.
IN_PROC_BROWSER_TEST_F(LuxxleNetworkAuditTest, BasicTests) {
  // Load the Welcome page.
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("luxxle://welcome")));
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("luxxle://wallet")));
  WaitForTimeout(kMaxTimeoutPerLoadedURL);
