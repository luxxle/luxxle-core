...
TEST(LuxxleCommonStaticRedirectNetworkDelegateHelperTest,
  ...
  EXPECT_EQ(redirect.host(), kLuxxleRedirectorProxy);
...
TEST(LuxxleCommonStaticRedirectNetworkDelegateHelperTest,
  ...
  int rc = OnBeforeURLRequest_CommonStaticRedirectWork(ResponseCallback(),
                                                       request_info);
  const GURL redirect = GURL(request_info->new_url_spec);
  EXPECT_EQ(redirect.host(), kLuxxleClients4Proxy);
  EXPECT_TRUE(redirect.SchemeIs(url::kHttpsScheme));
  EXPECT_EQ(redirect.path(), url.path());
  EXPECT_EQ(rc, net::OK);
}
...
TEST(LuxxleCommonStaticRedirectNetworkDelegateHelperTest,
  ...
  EXPECT_TRUE(redirect.SchemeIs(url::kHttpsScheme));
  EXPECT_EQ(redirect.path(), "/luxxle/luxxle-browser/issues/new");
  EXPECT_EQ(redirect.query(),
            "title=Crash%20Report&labels=crash&body=IMPORTANT%20Luxxle");
  EXPECT_EQ(rc, net::OK);
  ...
  request_info = std::make_shared<luxxle::LuxxleRequestInfo>(url_fewer_keys);
  ...
  request_info = std::make_shared<luxxle::LuxxleRequestInfo>(url_wrong_keys);
  rc = OnBeforeURLRequest_CommonStaticRedirectWork(ResponseCallback(),
                                                   request_info);
  EXPECT_TRUE(request_info->new_url_spec.empty());
  EXPECT_EQ(rc, net::OK);
