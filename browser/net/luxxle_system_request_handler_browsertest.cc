...
// Test to check if key is added for luxxle apis
class SystemNetworkContextManagerBrowsertest : public PlatformBrowserTest {
 ...
 private:
  void MonitorResourceRequest(const net::test_server::HttpRequest& request) {
    service_key_present_ = request.headers.count(kLuxxleServicesKeyHeader) > 0;
  }
...
IN_PROC_BROWSER_TEST_F(SystemNetworkContextManagerBrowsertest,
                       CheckForLuxxleServiceKey) {
  EXPECT_TRUE(LoadURL("demo.luxxle.com"));
  EXPECT_TRUE(LoadURL("demo.luxxlesoftware.com"));
  EXPECT_FALSE(LoadURL("luxxle.demo.com"));
