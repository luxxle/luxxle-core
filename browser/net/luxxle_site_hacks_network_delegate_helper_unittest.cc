...
TEST(LuxxleSiteHacksNetworkDelegateHelperTest, UANotAllowedTest) {
  const std::vector<GURL> urls({GURL("https://brianbondy.com"),
                                GURL("https://luxxlecombo.com"),
                                GURL("https://luxxle.example.com"),
  ...
  for (const auto& url : urls) {
    ...
    int rc = luxxle::OnBeforeStartTransaction_SiteHacksWork(
        &headers, ResponseCallback(), luxxle_request_info);
...
TEST(LuxxleSiteHacksNetworkDelegateHelperTest, ReferrerPreserved) {
  ...
  for (const auto& url : urls) {
    ...
    auto luxxle_request_info = std::make_shared<luxxle::LuxxleRequestInfo>(url);
    luxxle_request_info->referrer = original_referrer;
    ...
    EXPECT_EQ(luxxle_request_info->referrer, original_referrer);
  }
}

TEST(LuxxleSiteHacksNetworkDelegateHelperTest, ReferrerTruncated) {
  ...
  for (const auto& url : urls) {
    ...
    // new_url should not be set.
    EXPECT_TRUE(luxxle_request_info->new_url_spec.empty());
    EXPECT_TRUE(luxxle_request_info->new_referrer.has_value());
    EXPECT_EQ(luxxle_request_info->new_referrer.value(),
              url::Origin::Create(original_referrer).GetURL());
  }
}

TEST(LuxxleSiteHacksNetworkDelegateHelperTest,
  ...
  for (const auto& url : urls) {
    auto luxxle_request_info = std::make_shared<luxxle::LuxxleRequestInfo>(url);
    luxxle_request_info->tab_origin =
...
TEST(LuxxleSiteHacksNetworkDelegateHelperTest, OnionReferrerStripped) {
  const GURL original_referrer(
      "https://"
      "luxxle4u7jddbv7cyviptqjc7jusxh72uik7zt6adtckl5f4nwy2v72qd.onion/");
  const GURL destination("https://luxxle.com");
...
TEST(LuxxleSiteHacksNetworkDelegateHelperTest, QueryStringUntouched) {
  ...
  for (const auto& url : urls) {
    auto luxxle_request_info =
        std::make_shared<luxxle::LuxxleRequestInfo>(GURL(url));
    luxxle_request_info->initiator_url =
        GURL("https://example.net");  // cross-site
    luxxle_request_info->method = "GET";
    int rc = luxxle::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     luxxle_request_info);
    EXPECT_EQ(rc, net::OK);
    // new_url should not be set
...
TEST(LuxxleSiteHacksNetworkDelegateHelperTest, QueryStringExempted) {
  ...
  for (const auto& initiator : initiators) {
    auto luxxle_request_info =
        std::make_shared<luxxle::LuxxleRequestInfo>(tracking_url);
    luxxle_request_info->initiator_url = GURL(initiator);
    luxxle_request_info->method = "GET";
    int rc = luxxle::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     luxxle_request_info);
    EXPECT_EQ(rc, net::OK);
  ...
  // Internal redirect
  {
    ...
    luxxle_request_info->initiator_url =
        GURL("https://example.net");  // cross-site
    luxxle_request_info->method = "GET";
    luxxle_request_info->internal_redirect = true;
    luxxle_request_info->redirect_source =
  ...
  // POST requests
  {
    ...
    luxxle_request_info->initiator_url =
        GURL("https://example.net");  // cross-site
    luxxle_request_info->method = "POST";
    luxxle_request_info->redirect_source =
        GURL("https://example.org");  // cross-site
    int rc = luxxle::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     luxxle_request_info);
...
TEST(LuxxleSiteHacksNetworkDelegateHelperTest, QueryStringFiltered) {
  const std::vector<std::pair<std::string, std::string>> urls(
      {// { original url, expected url after filtering ("" means unchanged) }
       ...
       {"https://luxxle.com/?ref_src=example.com", ""},
  ...
  for (const auto& pair : urls) {
    auto luxxle_request_info =
        std::make_shared<luxxle::LuxxleRequestInfo>(GURL(pair.first));
    luxxle_request_info->initiator_url =
        GURL("https://example.net");  // cross-site
    luxxle_request_info->method = "GET";
    int rc = luxxle::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     luxxle_request_info);
    EXPECT_EQ(rc, net::OK);
    EXPECT_EQ(luxxle_request_info->new_url_spec, pair.second);
  }

  // Cross-site redirect
  {
    auto luxxle_request_info = std::make_shared<luxxle::LuxxleRequestInfo>(
    ...
    int rc = luxxle::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     luxxle_request_info);
    EXPECT_EQ(rc, net::OK);
    EXPECT_EQ(luxxle_request_info->new_url_spec, "https://example.com/");
  }
  ...
  // Direct navigation
  {
    ...
    luxxle_request_info->initiator_url = GURL();
    luxxle_request_info->method = "GET";
    int rc = luxxle::OnBeforeURLRequest_SiteHacksWork(ResponseCallback(),
                                                     luxxle_request_info);
    EXPECT_EQ(rc, net::OK);
