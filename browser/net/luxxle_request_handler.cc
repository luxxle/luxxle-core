...
#include "luxxle/browser/net/luxxle_ad_block_csp_network_delegate_helper.h"
#include "luxxle/browser/net/luxxle_ad_block_tp_network_delegate_helper.h"
#include "luxxle/browser/net/luxxle_common_static_redirect_network_delegate_helper.h"
#include "luxxle/browser/net/luxxle_localhost_permission_network_delegate_helper.h"
#include "luxxle/browser/net/luxxle_reduce_language_network_delegate_helper.h"
...
#include "luxxle/browser/net/luxxle_stp_util.h"
...
#include "luxxle/browser/net/luxxle_torrent_redirect_network_delegate_helper.h"
#endif

static bool IsInternalScheme(std::shared_ptr<luxxle::LuxxleRequestInfo> ctx) {
  DCHECK(ctx);
...
LuxxleRequestHandler::LuxxleRequestHandler() {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  SetupCallbacks();
}

LuxxleRequestHandler::~LuxxleRequestHandler() = default;

void LuxxleRequestHandler::SetupCallbacks() {
  luxxle::OnBeforeURLRequestCallback callback =
      base::BindRepeating(luxxle::OnBeforeURLRequest_SiteHacksWork);
  before_url_request_callbacks_.push_back(callback);

  callback = base::BindRepeating(luxxle::OnBeforeURLRequest_AdBlockTPPreWork);
  before_url_request_callbacks_.push_back(callback);

  callback =
      base::BindRepeating(luxxle::OnBeforeURLRequest_CommonStaticRedirectWork);
  before_url_request_callbacks_.push_back(callback);
  ...
  if (base::FeatureList::IsEnabled(
    ...
    callback =
        base::BindRepeating(luxxle::OnBeforeURLRequest_LocalhostPermissionWork);
    before_url_request_callbacks_.push_back(callback);
  }

  luxxle::OnBeforeStartTransactionCallback start_transaction_callback =
      base::BindRepeating(luxxle::OnBeforeStartTransaction_SiteHacksWork);
  before_start_transaction_callbacks_.push_back(start_transaction_callback);

  if (base::FeatureList::IsEnabled(
          blink::features::kLuxxleGlobalPrivacyControl)) {
    start_transaction_callback = base::BindRepeating(
        luxxle::OnBeforeStartTransaction_GlobalPrivacyControlWork);
    before_start_transaction_callbacks_.push_back(start_transaction_callback);
  }

  start_transaction_callback =
      base::BindRepeating(luxxle::OnBeforeStartTransaction_LuxxleServiceKey);
  before_start_transaction_callbacks_.push_back(start_transaction_callback);

  if (base::FeatureList::IsEnabled(
          luxxle_shields::features::kLuxxleReduceLanguage)) {
    start_transaction_callback =
        base::BindRepeating(luxxle::OnBeforeStartTransaction_ReduceLanguageWork);
    before_start_transaction_callbacks_.push_back(start_transaction_callback);
  }

  start_transaction_callback =
      base::BindRepeating(luxxle::OnBeforeStartTransaction_SearchAdsHeader);
  before_start_transaction_callbacks_.push_back(start_transaction_callback);

#if BUILDFLAG(ENABLE_LUXXLE_WEBTORRENT)
  luxxle::OnHeadersReceivedCallback headers_received_callback =
  ...
  if (base::FeatureList::IsEnabled(
          ::luxxle_shields::features::kLuxxleAdblockCspRules)) {
    luxxle::OnHeadersReceivedCallback headers_received_callback2 =
        base::BindRepeating(luxxle::OnHeadersReceived_AdBlockCspWork);
    headers_received_callbacks_.push_back(headers_received_callback2);
  }
}

bool LuxxleRequestHandler::IsRequestIdentifierValid(
    uint64_t request_identifier) {
  return callbacks_.contains(request_identifier);
}

int LuxxleRequestHandler::OnBeforeURLRequest(
  ...
  ctx->new_url = new_url;
  ctx->event_type = luxxle::kOnBeforeRequest;
  callbacks_[ctx->request_identifier] = std::move(callback);
  RunNextCallback(ctx);
  return net::ERR_IO_PENDING;
}

int LuxxleRequestHandler::OnBeforeStartTransaction(
    std::shared_ptr<luxxle::LuxxleRequestInfo> ctx,
    net::CompletionOnceCallback callback,
  ...
  ctx->event_type = luxxle::kOnBeforeStartTransaction;
  ctx->headers = headers;
  callbacks_[ctx->request_identifier] = std::move(callback);
  RunNextCallback(ctx);
  return net::ERR_IO_PENDING;
...
int LuxxleRequestHandler::OnHeadersReceived(
    std::shared_ptr<luxxle::LuxxleRequestInfo> ctx,
    net::CompletionOnceCallback callback,
  ...
  if (!ctx->tab_origin.is_empty()) {
    luxxle::RemoveTrackableSecurityHeadersForThirdParty(
  ...
  if (headers_received_callbacks_.empty() &&
    ...
    // Extension scheme not excluded since luxxle_webtorrent needs it.
    return net::OK;
  }

  callbacks_[ctx->request_identifier] = std::move(callback);
  ctx->event_type = luxxle::kOnHeadersReceived;
...
void LuxxleRequestHandler::OnURLRequestDestroyed(
    std::shared_ptr<luxxle::LuxxleRequestInfo> ctx) {
...
void LuxxleRequestHandler::RunCallbackForRequestIdentifier(
    uint64_t request_identifier,
    int rv) {
...
void LuxxleRequestHandler::RunNextCallback(
    std::shared_ptr<luxxle::LuxxleRequestInfo> ctx) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  ...
  if (ctx->event_type == luxxle::kOnBeforeRequest) {
    while (before_url_request_callbacks_.size() !=
      ...
      luxxle::ResponseCallback next_callback =
          base::BindRepeating(&LuxxleRequestHandler::RunNextCallback,
  ...
  } else if (ctx->event_type == luxxle::kOnBeforeStartTransaction) {
    while (before_start_transaction_callbacks_.size() !=
           ctx->next_url_request_index) {
      luxxle::OnBeforeStartTransactionCallback callback =
  ...
  } else if (ctx->event_type == luxxle::kOnHeadersReceived) {
    while (headers_received_callbacks_.size() != ctx->next_url_request_index) {
      luxxle::OnHeadersReceivedCallback callback =
  ...
  if (ctx->event_type == luxxle::kOnBeforeRequest) {
    ...
    if (ctx->blocked_by == luxxle::kAdBlocked ||
        ctx->blocked_by == luxxle::kOtherBlocked) {
