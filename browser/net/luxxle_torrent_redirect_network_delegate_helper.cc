...
bool IsWebtorrentInitiated(std::shared_ptr<luxxle::LuxxleRequestInfo> ctx) {
  return ctx->initiator_url.scheme() == extensions::kExtensionScheme &&
         ctx->initiator_url.host() == luxxle_webtorrent_extension_id;
}
...
bool IsMainFrameResource(std::shared_ptr<luxxle::LuxxleRequestInfo> ctx) {
...
int OnHeadersReceived_TorrentRedirectWork(
  ...
  GURL url(base::StrCat(
      {extensions::kExtensionScheme, "://", luxxle_webtorrent_extension_id,
       "/extension/luxxle_webtorrent2.html?", ctx->request_url.spec()}));
