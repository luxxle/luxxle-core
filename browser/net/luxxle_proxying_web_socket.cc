...
LuxxleProxyingWebSocket::LuxxleProxyingWebSocket(
    WebSocketFactory factory,
    const network::ResourceRequest& request,
    content::FrameTreeNodeId frame_tree_node_id,
    content::BrowserContext* browser_context,
    scoped_refptr<RequestIDGenerator> request_id_generator,
    LuxxleRequestHandler& handler,
...
// static
LuxxleProxyingWebSocket* LuxxleProxyingWebSocket::ProxyWebSocket(
...
void LuxxleProxyingWebSocket::Start(
  ...
  forwarding_handshake_client_.set_disconnect_with_reason_handler(
      base::BindOnce(&LuxxleProxyingWebSocket::OnMojoConnectionError,
  ...
  if (proxy_has_extra_headers()) {
    continuation = base::BindRepeating(
        &LuxxleProxyingWebSocket::ContinueToStartRequest,
        weak_factory_.GetWeakPtr());
  } else {
    continuation = base::BindRepeating(
        &LuxxleProxyingWebSocket::OnBeforeRequestComplete,
        weak_factory_.GetWeakPtr());
  }
...
LuxxleProxyingWebSocket::CreateWebSocketFactory() {
  return base::BindOnce(&LuxxleProxyingWebSocket::WebSocketFactoryRun,
                        weak_factory_.GetWeakPtr());
}

bool LuxxleProxyingWebSocket::proxy_has_extra_headers() {
  return proxy_trusted_header_client_.is_bound();
}

void LuxxleProxyingWebSocket::WebSocketFactoryRun(
    const GURL& url,
...
void LuxxleProxyingWebSocket::OnOpeningHandshakeStarted(
...
void LuxxleProxyingWebSocket::ContinueToHeadersReceived() {
  auto continuation = base::BindRepeating(
      &LuxxleProxyingWebSocket::OnHeadersReceivedComplete,
      weak_factory_.GetWeakPtr());
  ctx_ = luxxle::LuxxleRequestInfo::MakeCTX(request_, frame_tree_node_id_,
                                          request_id_, browser_context_, ctx_);
...
void LuxxleProxyingWebSocket::OnFailure(const std::string& message,
                                       int32_t net_error,
                                       int32_t response_code) {}

void LuxxleProxyingWebSocket::OnConnectionEstablished(
...
void LuxxleProxyingWebSocket::OnAuthRequired(
...
void LuxxleProxyingWebSocket::OnBeforeSendHeaders(
...
void LuxxleProxyingWebSocket::OnHeadersReceived(
...
void LuxxleProxyingWebSocket::OnBeforeRequestComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnBeforeSendHeaders(
        request_.headers,
        base::BindOnce(
            &LuxxleProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void LuxxleProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy(
    int error_code,
  ...
  auto continuation = base::BindRepeating(
      &LuxxleProxyingWebSocket::OnBeforeSendHeadersComplete,
      weak_factory_.GetWeakPtr());
...
void LuxxleProxyingWebSocket::OnBeforeSendHeadersComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
         !receiver_as_handshake_client_.is_bound());
...
void LuxxleProxyingWebSocket::ContinueToStartRequest(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
...
void LuxxleProxyingWebSocket::OnHeadersReceivedCompleteFromProxy(
    int error_code,
...
void LuxxleProxyingWebSocket::OnHeadersReceivedComplete(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnHeadersReceived(
        ...
        remote_endpoint_,
        base::BindOnce(
            &LuxxleProxyingWebSocket::OnHeadersReceivedCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void LuxxleProxyingWebSocket::PauseIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Pause();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Pause();
}

void LuxxleProxyingWebSocket::ResumeIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Resume();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Resume();
}

void LuxxleProxyingWebSocket::OnError(int error_code) {
  if (!is_done_) {
    is_done_ = true;
  }
...
// ResetWithReason
void LuxxleProxyingWebSocket::OnMojoConnectionError(
