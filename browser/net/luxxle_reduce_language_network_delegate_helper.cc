...
#include "luxxle/browser/luxxle_shields/luxxle_farbling_service_factory.h"
...
static constexpr auto kFarbleAcceptLanguageExceptions =
    base::MakeFixedFlatSet<std::string_view>(
        base::sorted_unique,
        {
            // https://github.com/luxxle/luxxle-browser/issues/26325
            "aeroplan.rewardops.com",
            // https://github.com/luxxle/luxxle-browser/issues/31196
            "login.live.com",
            // https://github.com/luxxle/luxxle-browser/issues/25309
            "ulta.com",
            "www.ulta.com",
        });
}  // namespace
...
std::string FarbleAcceptLanguageHeader(
  ...
  auto* luxxle_farbling_service =
      LuxxleFarblingServiceFactory::GetForProfile(profile);
  if (luxxle_farbling_service &&
      luxxle_farbling_service->MakePseudoRandomGeneratorForURL(origin_url,
                                                              &prng)) {
...
int OnBeforeStartTransaction_ReduceLanguageWork(
  ...
  if (!luxxle_shields::ShouldDoReduceLanguage(content_settings, origin_url,
                                             profile->GetPrefs())) {
    return net::OK;
  }
  ...
  if (headers->HasHeader(net::HttpRequestHeaders::kAcceptLanguage)) {
    ...
    // https://github.com/luxxle/luxxle-browser/issues/28945
    return net::OK;
  }

  std::string accept_language_string;
  switch (luxxle_shields::GetFingerprintingControlType(content_settings,
                                                      origin_url)) {
