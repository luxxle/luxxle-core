...
#include "luxxle/browser/luxxle_shields/ad_block_pref_service_factory.h"
#include "luxxle/browser/luxxle_shields/luxxle_shields_web_contents_observer.h"
#include "luxxle/browser/net/url_context.h"
#include "luxxle/components/luxxle_shields/content/browser/ad_block_pref_service.h"
...
#include "luxxle/content/public/browser/devtools/adblock_devtools_instumentation.h"
...
class AdblockCnameResolveHostClient : public network::mojom::ResolveHostClient {
 ...
 public:
  ...
  void OnComplete(int32_t result,
    ...
    UMA_HISTOGRAM_TIMES("Luxxle.ShieldsCNAMEBlocking.TotalResolutionTime",
...
EngineFlags ShouldBlockRequestOnTaskRunner(
  ...
  SCOPED_UMA_HISTOGRAM_TIMER("Luxxle.Adblock.ShouldBlockRequest");
  auto adblock_result =
      g_luxxle_browser_process->ad_block_service()->ShouldStartRequest(
...
void OnShouldBlockRequestResult(
  ...
  if (ctx->blocked_by == kAdBlocked) {
    luxxle_shields::LuxxleShieldsWebContentsObserver::DispatchBlockedEvent(
        ctx->request_url, ctx->frame_tree_node_id, luxxle_shields::kAds);
...
bool ProxySettingsAllowUncloaking(content::BrowserContext* browser_context,
  ...
  auto* ad_block_pref_service =
      luxxle_shields::AdBlockPrefServiceFactory::GetForBrowserContext(
          browser_context);
...
void OnBeforeURLRequestAdBlockTP(const ResponseCallback& next_callback,
  ...
  scoped_refptr<base::SequencedTaskRunner> task_runner =
      g_luxxle_browser_process->ad_block_service()->GetTaskRunner();
  ...
  bool should_check_uncloaked =
      base::FeatureList::IsEnabled(
          luxxle_shields::features::kLuxxleAdblockCnameUncloaking) &&
  ...
  if (!base::FeatureList::IsEnabled(
          luxxle_shields::features::kLuxxleAdblockDefault1pBlocking) &&
...
int OnBeforeURLRequest_AdBlockTPPreWork(const ResponseCallback& next_callback,
  ...
  if (ctx->request_url.is_empty() ||
      ...
      !ctx->allow_luxxle_shields || ctx->allow_ads ||
      ctx->resource_type == LuxxleRequestInfo::kInvalidResourceType) {
    return net::OK;
  }
  ...
  if (ctx->initiator_url.SchemeIs(kChromeExtensionScheme) &&
      !base::FeatureList::IsEnabled(
          ::luxxle_shields::features::kLuxxleExtensionNetworkBlocking)) {
    return net::OK;
  }
  ...
  // https://github.com/luxxle/luxxle-browser/issues/26302).
