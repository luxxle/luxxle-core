...
#include "luxxle/components/luxxle_shields/core/browser/ad_block_service_helper.h"
...
std::optional<std::string> GetCspDirectivesOnTaskRunner(
  ...
  std::optional<std::string> csp_directives =
      g_luxxle_browser_process->ad_block_service()->GetCspDirectives(
          ctx->request_url, ctx->resource_type, source_host);

  luxxle_shields::MergeCspDirectiveInto(original_csp, &csp_directives);
...
int OnHeadersReceived_AdBlockCspWork(
  ...
  if (!response_headers || !ctx->allow_luxxle_shields || ctx->allow_ads) {
    return net::OK;
  }
