...
namespace ui {
// This resets dark mode to os theme when user changes luxxle theme from
// dark or light to Same as Windows by using the value of registry because
// we changed ui::NativeTheme::dark_mode_ explicitly for using luxxle theme
// like a system theme.
...
void SetSystemDarkMode(LuxxleDarkModeType type) {
  ...
  ui::IgnoreSystemDarkModeChange(
      type != dark_mode::LuxxleDarkModeType::LUXXLE_DARK_MODE_TYPE_DEFAULT);
  ...
  internal::SetSystemDarkModeForNonDefaultMode(
      type == LuxxleDarkModeType::LUXXLE_DARK_MODE_TYPE_DARK);
}
