...
class LuxxleThemeServiceTest : public InProcessBrowserTest {
 public:
  LuxxleThemeServiceTest() = default;
  ~LuxxleThemeServiceTest() override = default;
...
class LuxxleThemeServiceTestWithoutSystemTheme : public InProcessBrowserTest {
 public:
  LuxxleThemeServiceTestWithoutSystemTheme() {
    dark_mode::SetUseSystemDarkModeEnabledForTest(false);
  }
};

IN_PROC_BROWSER_TEST_F(LuxxleThemeServiceTestWithoutSystemTheme,
                       LuxxleThemeChangeTest) {
  ...
  EXPECT_EQ(dark_mode::LuxxleDarkModeType::LUXXLE_DARK_MODE_TYPE_DARK,
            dark_mode::GetActiveLuxxleDarkModeType());
...
// Test whether appropriate native/web theme observer is called when luxxle theme
// is changed.
IN_PROC_BROWSER_TEST_F(LuxxleThemeServiceTest, ThemeObserverTest) {
...
IN_PROC_BROWSER_TEST_F(LuxxleThemeServiceTest, SystemThemeChangeTest) {
  ...
  dark_mode::SetLuxxleDarkModeType(
      dark_mode::LuxxleDarkModeType::LUXXLE_DARK_MODE_TYPE_DEFAULT);
  EXPECT_EQ(initial_mode,
            ui::NativeTheme::GetInstanceForNativeUi()->ShouldUseDarkColors());
}

// Check some colors from color provider pipeline.
IN_PROC_BROWSER_TEST_F(LuxxleThemeServiceTest, ColorProviderTest) {
...
// See https://github.com/luxxle/luxxle-browser/issues/22767
...
#endif
IN_PROC_BROWSER_TEST_F(LuxxleThemeServiceTest, MAYBE_DarkModeChangeByRegTest) {
