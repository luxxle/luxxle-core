...
class TestRewardsServiceObserver
 ...
 public:
  void WaitForServiceInitialized(luxxle_rewards::RewardsService* service) {
 ...
 private:
  void OnRewardsInitialized(luxxle_rewards::RewardsService*) override {
    run_loop_.Quit();
  }
...
class LuxxleSpeedFeatureProcessorBrowserTest : public InProcessBrowserTest {
 protected:
  void SetUpCommandLine(base::CommandLine* command_line) override {
    InProcessBrowserTest::SetUpCommandLine(command_line);
    command_line->AppendSwitch(
        perf::switches::kEnableLuxxleFeaturesForPerfTesting);
  }
  ...
  bool LuxxleNewsAreEnabled() {
    return luxxle_news::IsEnabled(browser()->profile()->GetPrefs());
  }
  ...
  void WaitForRewardsServiceInitialized() {
    auto* rewards_service = luxxle_rewards::RewardsServiceFactory::GetForProfile(
        browser()->profile());
...
IN_PROC_BROWSER_TEST_F(LuxxleSpeedFeatureProcessorBrowserTest, PRE_Default) {
  WaitForRewardsServiceInitialized();
}

IN_PROC_BROWSER_TEST_F(LuxxleSpeedFeatureProcessorBrowserTest, Default) {
#if BUILDFLAG(ENABLE_SPEEDREADER)
  EXPECT_TRUE(SpeedreaderIsEnabled());
#endif
  EXPECT_TRUE(HasOptedInToNotificationAds());
  EXPECT_TRUE(LuxxleNewsAreEnabled());
  WaitForRewardsServiceInitialized();
