...
#include "chrome/android/chrome_jni_headers/LuxxleDappPermissionPromptDialog_jni.h"
...
void LuxxleDappPermissionPromptDialogController::ShowDialog() {
  if (!GetOrCreateJavaObject())
    return;

  JNIEnv* env = base::android::AttachCurrentThread();
  Java_LuxxleDappPermissionPromptDialog_show(env, GetOrCreateJavaObject());
}

void LuxxleDappPermissionPromptDialogController::OnPrimaryButtonClicked(
    JNIEnv* env,
...
void LuxxleDappPermissionPromptDialogController::OnNegativeButtonClicked(
    JNIEnv* env) {
  delegate_->CancelConnectToSite();
}

void LuxxleDappPermissionPromptDialogController::OnDialogDismissed(JNIEnv* env) {
  java_object_.Reset();
  delegate_->OnDialogDismissed();
}

void LuxxleDappPermissionPromptDialogController::DismissDialog() {
  if (java_object_) {
    Java_LuxxleDappPermissionPromptDialog_dismissDialog(
        base::android::AttachCurrentThread(), java_object_);
  }
}

base::android::ScopedJavaGlobalRef<jobject>
LuxxleDappPermissionPromptDialogController::GetOrCreateJavaObject() {
  if (java_object_)
    return java_object_;
  ...
  return java_object_ = Java_LuxxleDappPermissionPromptDialog_create(
