...
class LuxxleWalletPermissionContextBrowserTest : public InProcessBrowserTest {
 public:
  LuxxleWalletPermissionContextBrowserTest()
      : https_server_(net::EmbeddedTestServer::TYPE_HTTPS) {
    scoped_feature_list_.InitAndEnableFeature(
        luxxle_wallet::features::kNativeLuxxleWalletFeature);
  }

  ~LuxxleWalletPermissionContextBrowserTest() override = default;
...
IN_PROC_BROWSER_TEST_F(LuxxleWalletPermissionContextBrowserTest,
                       GetAllowedAccounts) {
  ...
  // Fail if rfh is null.
  auto allowed_accounts = LuxxleWalletPermissionContext::GetAllowedAccounts(
      blink::PermissionType::LUXXLE_ETHEREUM, nullptr /* rfh */, addresses);
  ...
  allowed_accounts = LuxxleWalletPermissionContext::GetAllowedAccounts(
  ...
  for (const auto& account : expected_allowed_accounts) {
    auto sub_request_origin = luxxle_wallet::GetSubRequestOrigin(
        permissions::RequestType::kLuxxleEthereum, GetLastCommitedOrigin(),
        account);
