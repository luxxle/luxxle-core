...
class MockBrowserClient : public content::ContentBrowserClient {
 public:
  ...
  // Only construct an LuxxleWebTorrentNavigationThrottle so that we can test it
  // in isolation.
  std::vector<std::unique_ptr<NavigationThrottle>> CreateThrottlesForNavigation(
      content::NavigationHandle* handle) override {
    std::vector<std::unique_ptr<NavigationThrottle>> throttles;
    throttles.push_back(
        std::make_unique<LuxxleWebTorrentNavigationThrottle>(handle));
    return throttles;
  }
};
...
class LuxxleWebTorrentNavigationThrottleUnitTest
    : public content::RenderViewHostTestHarness {
 public:
  LuxxleWebTorrentNavigationThrottleUnitTest()
      : local_state_(TestingBrowserProcess::GetGlobal()) {
  }

  LuxxleWebTorrentNavigationThrottleUnitTest(
      const LuxxleWebTorrentNavigationThrottleUnitTest&) = delete;

  LuxxleWebTorrentNavigationThrottleUnitTest& operator=(
      const LuxxleWebTorrentNavigationThrottleUnitTest&) = delete;
  ...
  void AddExtension() {
    extension_ = ExtensionBuilder()
                     ...
                     .SetID(luxxle_webtorrent_extension_id)
                     .Build();
...
TEST_F(LuxxleWebTorrentNavigationThrottleUnitTest, ExternalWebPage) {
  ...
  auto throttle =
      std::make_unique<LuxxleWebTorrentNavigationThrottle>(&test_handle);
...
TEST_F(LuxxleWebTorrentNavigationThrottleUnitTest,
...
TEST_F(LuxxleWebTorrentNavigationThrottleUnitTest, WebTorrentUrlInstalled) {
  AddExtension();
  web_contents_tester()->NavigateAndCommit(GURL("http://example.com"));
  content::RenderFrameHost* host =
      render_frame_host_tester(main_rfh())->AppendChild("child");
...
// disabled.
TEST_F(LuxxleWebTorrentNavigationThrottleUnitTest, WebTorrentDisabledByPref) {
