...
// accesses resources.
class LuxxleSpellingOptionsSubMenuObserverTest : public InProcessBrowserTest {
 public:
  LuxxleSpellingOptionsSubMenuObserverTest() {}
  LuxxleSpellingOptionsSubMenuObserverTest(
      const LuxxleSpellingOptionsSubMenuObserverTest&) = delete;
  LuxxleSpellingOptionsSubMenuObserverTest& operator=(
      const LuxxleSpellingOptionsSubMenuObserverTest&) = delete;
  ~LuxxleSpellingOptionsSubMenuObserverTest() override {}
  ...
  void Reset(bool incognito = false,
             LuxxleSpellingOptionsSubMenuObserver::GtestMode gtest_mode =
                 LuxxleSpellingOptionsSubMenuObserver::GTEST_MODE_NORMAL) {
    Clear();
    menu_ = std::make_unique<LuxxleMockRenderViewContextMenu>(
        incognito ? browser()->profile()->GetPrimaryOTRProfile(
                        /*create_if_needed=*/true)
                  : browser()->profile());
    std::unique_ptr<LuxxleSpellingOptionsSubMenuObserver> observer =
        std::make_unique<LuxxleSpellingOptionsSubMenuObserver>(menu_.get(),
                                                              menu_.get(), 1);
  ...
  void CheckExpected() {
    for (size_t i = 0; i < menu()->GetMenuSize(); ++i) {
      LuxxleMockRenderViewContextMenu::MockMenuItem item;
  ...
  LuxxleMockRenderViewContextMenu* menu() { return menu_.get(); }
  SpellingOptionsSubMenuObserver* observer() { return observer_.get(); }

 private:
  std::unique_ptr<LuxxleMockRenderViewContextMenu> menu_;
  std::unique_ptr<SpellingOptionsSubMenuObserver> observer_;
};

// Tests that "Ask Luxxle for suggestions" isn't shown in the menu and the menu
// doesn't end with a separator.

IN_PROC_BROWSER_TEST_F(LuxxleSpellingOptionsSubMenuObserverTest,
                       CheckAskLuxxleNotShown) {
  ...
  // Test empty submenu.
  Reset(false, LuxxleSpellingOptionsSubMenuObserver::GTEST_MODE_EMPTY_SUBMENU);
  InitMenu(false, "en-US", std::vector<std::string>());
...
IN_PROC_BROWSER_TEST_F(LuxxleSpellingOptionsSubMenuObserverTest,
                       CheckAskLuxxleNotShownIncognito) {
  // Test with spellcheck enabled.
  Reset(true);
  InitMenu(true, "en-US", std::vector<std::string>(1, "en-US"));
