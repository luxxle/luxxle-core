...
LuxxleDownloadItemModel::LuxxleDownloadItemModel(DownloadUIModel* model)
    : model_(model) {}

LuxxleDownloadItemModel::~LuxxleDownloadItemModel() = default;

// Adds origin url to the tooltip text and "Not secure", if needed.
std::u16string LuxxleDownloadItemModel::GetTooltipText() {
...
std::u16string LuxxleDownloadItemModel::GetOriginURLText(bool* is_secure) {
  *is_secure = false;
    ...
    *is_secure = network::IsUrlPotentiallyTrustworthy(gurl);
    if (gurl.SchemeIs(url::kAboutScheme)) {
      ...
      if (gurl.SchemeIsFile()) {
        ...
        // case, see the LuxxleDownloadItemModelTest.GetOriginUrlText unit test.
        origin = "file:///";
      }
    }
  } else {
