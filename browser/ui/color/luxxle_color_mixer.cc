...
#if BUILDFLAG(ENABLE_LUXXLE_VPN) || BUILDFLAG(ENABLE_SPEEDREADER)
...
void AddLuxxleVpnColorMixer(ui::ColorProvider* provider,
  ...
  if (key.custom_theme) {
    ...
    mixer[kColorLuxxleVpnButtonIconConnected] = {
        nala::kColorSystemfeedbackSuccessIcon};
    mixer[kColorLuxxleVpnButtonIconDisconnected] = {nala::kColorIconDefault};
    mixer[kColorLuxxleVpnButtonIconError] = {
        nala::kColorSystemfeedbackErrorIcon};
    mixer[kColorLuxxleVpnButtonErrorBackgroundNormal] = {
        nala::kColorSystemfeedbackErrorBackground};
  }

  mixer[kColorLuxxleVpnButtonBackgroundNormal] = {kColorToolbar};
}
#endif

#if BUILDFLAG(ENABLE_SPEEDREADER)
void AddLuxxleSpeedreaderColorMixer(ui::ColorProvider* provider,
                                   const ui::ColorProviderKey& key) {
  ui::ColorMixer& mixer = provider->AddMixer();
...
void AddChromeLightThemeColorMixer(ui::ColorProvider* provider,
  ...
  mixer[kColorNewTabPageBackground] = {kLuxxleNewTabBackgroundLight};
  ...
  mixer[kColorToolbarButtonIconInactive] = {
      ui::SetAlpha(kColorToolbarButtonIcon, kLuxxleDisabledControlAlpha)};
...
void AddChromeDarkThemeColorMixer(ui::ColorProvider* provider,
  ...
  mixer[kColorNewTabButtonBackgroundFrameActive] = {ui::kColorFrameActive};
  mixer[kColorNewTabButtonBackgroundFrameInactive] = {ui::kColorFrameInactive};
  mixer[kColorNewTabPageBackground] = {kLuxxleNewTabBackgroundDark};
...
void AddLuxxleColorMixerForAllThemes(ui::ColorProvider* provider,
                                    const ui::ColorProviderKey& key) {
  ui::ColorMixer& mixer = provider->AddMixer();
...
void AddLuxxleLightThemeColorMixer(ui::ColorProvider* provider,
                                  const ui::ColorProviderKey& key) {
  ui::ColorMixer& mixer = provider->AddMixer();
  ...
  // interaction.
  mixer[kColorLuxxleDownloadToolbarButtonActive] = {
  ...
  mixer[kColorLuxxleAppMenuAccentColor] = {SkColorSetRGB(0xDF, 0xE1, 0xFF)};
}

void AddLuxxleDarkThemeColorMixer(ui::ColorProvider* provider,
                                 const ui::ColorProviderKey& key) {
  ui::ColorMixer& mixer = provider->AddMixer();
  ...
  mixer[kColorLuxxleAppMenuAccentColor] = {SkColorSetRGB(0x37, 0x2C, 0xBF)};
}

// Handling dark or light theme on normal profile.
void AddLuxxleThemeColorMixer(ui::ColorProvider* provider,
                             const ui::ColorProviderKey& key) {
  AddLuxxleColorMixerForAllThemes(provider, key);

  key.color_mode == ui::ColorProviderKey::ColorMode::kDark
      ? AddLuxxleDarkThemeColorMixer(provider, key)
      : AddLuxxleLightThemeColorMixer(provider, key);
#if BUILDFLAG(ENABLE_LUXXLE_VPN)
  AddLuxxleVpnColorMixer(provider, key);
#endif
#if BUILDFLAG(ENABLE_SPEEDREADER)
  AddLuxxleSpeedreaderColorMixer(provider, key);
#endif
  ...
  mixer[kColorLuxxleExtensionMenuIcon] = {nala::kColorIconInteractive};
...
void AddLuxxlePrivateThemeColorMixer(ui::ColorProvider* provider,
                                    const ui::ColorProviderKey& key) {
  AddLuxxleDarkThemeColorMixer(provider, key);

  // Add private theme specific luxxle colors here.
  ui::ColorMixer& mixer = provider->AddMixer();
  ...
  // private/tor/guest profile. See LuxxleBrowserFrame::GetNativeTheme().
  // Exceptionally, below side panel header colors should be luxxle theme
  // specific because side panel header colors should be aligned with
  // side panel contents.
  const bool is_dark = dark_mode::GetActiveLuxxleDarkModeType() ==
...
void AddLuxxleTorThemeColorMixer(ui::ColorProvider* provider,
                                const ui::ColorProviderKey& key) {
  AddLuxxlePrivateThemeColorMixer(provider, key);
  AddChromeDarkThemeColorMixer(provider, key);

  // Add tor theme specific luxxle colors here.
}
...
void AddTorThemeColorMixer(ui::ColorProvider* provider,
                           const ui::ColorProviderKey& key) {
  AddLuxxleTorThemeColorMixer(provider, key);
...
void AddLuxxleOmniboxPrivateThemeColorMixer(ui::ColorProvider* provider,
                                           const ui::ColorProviderKey& key) {
  ui::ColorMixer& mixer = provider->AddMixer();
...
void AddLuxxleOmniboxColorMixer(ui::ColorProvider* provider,
                               const ui::ColorProviderKey& key) {
  ui::ColorMixer& mixer = provider->AddMixer();
  ...
  mixer[kColorLuxxleOmniboxResultViewSeparator] = {
