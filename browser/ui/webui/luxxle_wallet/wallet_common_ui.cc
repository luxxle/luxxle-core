...
void AddBlockchainTokenImageSource(Profile* profile) {
  ...
  content::URLDataSource::Add(
      profile, std::make_unique<luxxle_wallet::BlockchainImagesSource>(path));
}

bool IsLuxxleWalletOrigin(const url::Origin& origin) {
  return origin == url::Origin::Create(GURL(kLuxxleUIWalletPanelURL)) ||
         origin == url::Origin::Create(GURL(kLuxxleUIWalletPageURL));
}
