...
WalletHandler::WalletHandler(
    ...
    : receiver_(this, std::move(receiver)),
      luxxle_wallet_service_(
          LuxxleWalletServiceFactory::GetServiceForContext(profile)) {}

WalletHandler::~WalletHandler() = default;

// TODO(apaymyshev): this is the only method in WalletHandler. Should be merged
// into LuxxleWalletService.
void WalletHandler::GetWalletInfo(GetWalletInfoCallback callback) {
  if (!luxxle_wallet_service_) {
    std::move(callback).Run(nullptr);
    return;
  }

  auto* keyring_service = luxxle_wallet_service_->keyring_service();
