...
#include "luxxle/browser/ui/webui/luxxle_education/luxxle_education_server_checker.h"
...
constexpr auto kTrafficAnnotation =
    net::DefineNetworkTrafficAnnotation("luxxle_education_server_checker",
                                        R"(
      semantics {
        sender: "Luxxle Education"
        description: "Attempts to fetch the content for a Luxxle Education
          server page to ensure that it loads successfully."
        trigger:
          "Completing the Luxxle Welcome UX flow."
        data:
          "No data sent, other than URL of the education server page. "
          "Data does not contain PII."
        destination: LUXXLE_OWNED_SERVICE
      }
      policy {
        cookies_allowed: NO
        setting:
...
LuxxleEducationServerChecker::LuxxleEducationServerChecker(
    PrefService& pref_service,
    scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory)
    : pref_service_(pref_service), url_loader_factory_(url_loader_factory) {}

LuxxleEducationServerChecker::~LuxxleEducationServerChecker() = default;

void LuxxleEducationServerChecker::IsServerPageAvailable(
  ...
  auto download_callback = base::BindOnce(
      &LuxxleEducationServerChecker::OnURLResponse, weak_factory_.GetWeakPtr(),
...
void LuxxleEducationServerChecker::OnURLResponse(
    EducationPageType page_type,
