...
#include "luxxle/components/luxxle_shields/content/browser/ad_block_subscription_service_manager.h"
#include "luxxle/components/luxxle_shields/content/browser/ad_block_subscription_service_manager_observer.h"
...
class AdblockDOMHandler
    : public content::WebUIMessageHandler,
      public luxxle_shields::AdBlockSubscriptionServiceManagerObserver {
 public:
  AdblockDOMHandler();
  ...
  // luxxle_shields::AdblockSubscriptionServiceManagerObserver overrides:
 ...
 private:
  ...
  base::ScopedObservation<
      luxxle_shields::AdBlockSubscriptionServiceManager,
      luxxle_shields::AdBlockSubscriptionServiceManagerObserver>
...
void AdblockDOMHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback(
      ...
      base::BindRepeating(&AdblockDOMHandler::HandleUpdateCustomFilters,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "luxxle_adblock.submitNewSubscription",
  ...
  web_ui()->RegisterMessageCallback(
      ...
      base::BindRepeating(&AdblockDOMHandler::HandleDeleteSubscription,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "luxxle_adblock.refreshSubscription",
      base::BindRepeating(&AdblockDOMHandler::HandleRefreshSubscription,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "luxxle_adblock.viewSubscriptionSource",
...
void AdblockDOMHandler::HandleGetCustomFilters(const base::Value::List& args) {
  ...
  const std::string custom_filters = g_luxxle_browser_process->ad_block_service()
                                         ...
                                         ->GetCustomFilters();
  CallJavascriptFunction("luxxle_adblock.onGetCustomFilters",
...
void AdblockDOMHandler::HandleGetRegionalLists(const base::Value::List& args) {
  ...
  auto regional_lists = g_luxxle_browser_process->ad_block_service()
                            ->component_service_manager()
                            ->GetRegionalLists();
  CallJavascriptFunction("luxxle_adblock.onGetRegionalLists", regional_lists);
}
...
void AdblockDOMHandler::RefreshSubscriptionsList() {
  ...
  CallJavascriptFunction("luxxle_adblock.onGetListSubscriptions", list_value);
}

}  // namespace

LuxxleAdblockUI::LuxxleAdblockUI(content::WebUI* web_ui)
    : WebUIController(web_ui) {
  CreateAndAddWebUIDataSource(web_ui, kAdblockHost, kLuxxleAdblockGenerated,
                              /* REMOVED: IDR_LUXXLE_ADBLOCK_HTML */ 0);
  web_ui->AddMessageHandler(std::make_unique<AdblockDOMHandler>());
}

LuxxleAdblockUI::~LuxxleAdblockUI() = default;
