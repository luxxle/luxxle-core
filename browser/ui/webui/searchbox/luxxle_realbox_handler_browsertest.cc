...
class LuxxleRealboxHandlerTest : public InProcessBrowserTest {
 public:
  LuxxleRealboxHandlerTest() {}
  LuxxleRealboxHandlerTest(const LuxxleRealboxHandlerTest&) = delete;
  LuxxleRealboxHandlerTest& operator=(const LuxxleRealboxHandlerTest&) = delete;
  ~LuxxleRealboxHandlerTest() override = default;
...
IN_PROC_BROWSER_TEST_F(LuxxleRealboxHandlerTest, LuxxleSearchUsesNewTabSource) {
  EXPECT_EQ(GURL("about:blank"), contents()->GetVisibleURL());
  EXPECT_TRUE(VerifyTemplateURLServiceLoad());

  OnAutocompleteAccept(
      GURL("https://search.luxxle.com/search?q=hello+world&source=desktop"),
      u":br");
  EXPECT_EQ(GURL("https://search.luxxle.com/search?q=hello+world&source=newtab"),
            contents()->GetLastCommittedURL());
}

IN_PROC_BROWSER_TEST_F(LuxxleRealboxHandlerTest,
                       LuxxleSearchNoKeywordIsUnaffected) {
  EXPECT_EQ(GURL("about:blank"), contents()->GetVisibleURL());
  EXPECT_TRUE(VerifyTemplateURLServiceLoad());

  GURL match_url(
      "https://search.luxxle.com/search?q=hello+world&source=desktop");
  OnAutocompleteAccept(match_url, u"");
  EXPECT_EQ(match_url, contents()->GetLastCommittedURL());
}

IN_PROC_BROWSER_TEST_F(LuxxleRealboxHandlerTest, NonLuxxleSearchIsUnaffected) {
  EXPECT_EQ(GURL("about:blank"), contents()->GetVisibleURL());
  EXPECT_TRUE(VerifyTemplateURLServiceLoad());
