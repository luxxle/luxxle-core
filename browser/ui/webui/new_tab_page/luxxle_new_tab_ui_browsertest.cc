...
#include "luxxle/browser/extensions/luxxle_extension_functional_test.h"
...
class LuxxleNewTabUIBrowserTest : public extensions::ExtensionFunctionalTest {
 protected:
...
IN_PROC_BROWSER_TEST_F(LuxxleNewTabUIBrowserTest, StartupURLTest) {
...
// This test simply checks that by default the Luxxle new tab page is used.
// It does this by loading the newtab page and then checking if
// window.luxxle_new_tab exists.
IN_PROC_BROWSER_TEST_F(LuxxleNewTabUIBrowserTest, LuxxleNewTabIsDefault) {
  content::WebContents* web_contents = GetActiveWebContents();
  SimulateOpenNewTabAndWaitForLoad(web_contents);
  VerifyNewTabPageLoadedExpectation(web_contents);
}
...
IN_PROC_BROWSER_TEST_F(LuxxleNewTabUIBrowserTest, NewTabPageLocationOverride) {
