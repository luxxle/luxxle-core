...
namespace {
constexpr char kLuxxleSearchForTorKeyword[] =
    ":search.luxxle4u7jddbv7cyviptqjc7jusxh72uik7zt6adtckl5f4nwy2v72qd.onion";
...
void SortDefaultSearchEnginesListInJP(
  ...
  const GURL yahoo_jp_url =
      GURL(TemplateURLPrepopulateData::luxxle_yahoo_jp.search_url);
...
LuxxleSearchEnginesHandler::LuxxleSearchEnginesHandler(
    Profile* profile,
    regional_capabilities::RegionalCapabilitiesService* regional_capabilities)
    : SearchEnginesHandler(profile),
      regional_capabilities_(regional_capabilities) {}

LuxxleSearchEnginesHandler::~LuxxleSearchEnginesHandler() = default;

void LuxxleSearchEnginesHandler::RegisterMessages() {
  SearchEnginesHandler::RegisterMessages();

  web_ui()->RegisterMessageCallback(
      "getPrivateSearchEnginesList",
      base::BindRepeating(
          &LuxxleSearchEnginesHandler::HandleGetPrivateSearchEnginesList,
          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "setDefaultPrivateSearchEngine",
      base::BindRepeating(
          &LuxxleSearchEnginesHandler::HandleSetDefaultPrivateSearchEngine,
          base::Unretained(this)));
}

void LuxxleSearchEnginesHandler::OnModelChanged() {
...
void LuxxleSearchEnginesHandler::HandleGetPrivateSearchEnginesList(
...
base::Value::List LuxxleSearchEnginesHandler::GetPrivateSearchEnginesList() {
  ...
  for (int i = 0; i < last_default_engine_index; ++i) {
    ...
    // Don't show two luxxle search entries from settings to prevent confusion.
    // Hide luxxle search for tor entry from settings UI. User doesn't need to
    // select luxxle search tor entry for private profile.
    if (base::UTF16ToUTF8(template_url->keyword()) ==
        kLuxxleSearchForTorKeyword) {
      continue;
    }
...
void LuxxleSearchEnginesHandler::HandleSetDefaultPrivateSearchEngine(
    const base::Value::List& args) {
  CHECK_EQ(1U, args.size());
...
base::Value::Dict LuxxleSearchEnginesHandler::GetSearchEnginesList() {
  auto search_engines_info = SearchEnginesHandler::GetSearchEnginesList();
  // Don't show two luxxle search entries from settings to prevent confusion.
  // Hide luxxle search for tor entry from settings UI. User doesn't need to
  // select luxxle search tor entry for normal profile.
  ...
  defaults->EraseIf([](const auto& val) {
    ...
    DCHECK(keyword);
    return *keyword == kLuxxleSearchForTorKeyword;
  });
