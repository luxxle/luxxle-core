...
LuxxleDefaultExtensionsHandler::LuxxleDefaultExtensionsHandler()
...
LuxxleDefaultExtensionsHandler::~LuxxleDefaultExtensionsHandler() = default;

void LuxxleDefaultExtensionsHandler::RegisterMessages() {
  profile_ = Profile::FromWebUI(web_ui());
  web_ui()->RegisterMessageCallback(
      "resetWallet",
      base::BindRepeating(&LuxxleDefaultExtensionsHandler::ResetWallet,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "resetTransactionInfo",
      base::BindRepeating(&LuxxleDefaultExtensionsHandler::ResetTransactionInfo,
                          base::Unretained(this)));

#if BUILDFLAG(ENABLE_ORCHARD)
  if (luxxle_wallet::IsZCashShieldedTransactionsEnabled()) {
    web_ui()->RegisterMessageCallback(
        "resetZCashSyncState",
        base::BindRepeating(&LuxxleDefaultExtensionsHandler::ResetZCashSyncState,
                            base::Unretained(this)));
  }
#endif
  ...
  web_ui()->RegisterMessageCallback(
      "getRestartNeeded",
      base::BindRepeating(&LuxxleDefaultExtensionsHandler::GetRestartNeeded,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "setWidevineEnabled",
      base::BindRepeating(&LuxxleDefaultExtensionsHandler::SetWidevineEnabled,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "isWidevineEnabled",
      base::BindRepeating(&LuxxleDefaultExtensionsHandler::IsWidevineEnabled,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "getDecentralizedDnsResolveMethodList",
      base::BindRepeating(
          &LuxxleDefaultExtensionsHandler::GetDecentralizedDnsResolveMethodList,
          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "getEnsOffchainResolveMethodList",
      base::BindRepeating(
          &LuxxleDefaultExtensionsHandler::GetEnsOffchainResolveMethodList,
          base::Unretained(this)));
...
void LuxxleDefaultExtensionsHandler::InitializePrefCallbacks() {
#if BUILDFLAG(ENABLE_WIDEVINE)
  local_state_change_registrar_.Init(g_browser_process->local_state());
  local_state_change_registrar_.Add(
      kWidevineEnabled,
      base::BindRepeating(
          &LuxxleDefaultExtensionsHandler::OnWidevineEnabledChanged,
          base::Unretained(this)));
#endif
  pref_change_registrar_.Init(profile_->GetPrefs());
}

bool LuxxleDefaultExtensionsHandler::IsRestartNeeded() {
#if BUILDFLAG(ENABLE_WIDEVINE)
...
void LuxxleDefaultExtensionsHandler::GetRestartNeeded(
...
#if BUILDFLAG(ENABLE_ORCHARD)
void LuxxleDefaultExtensionsHandler::ResetZCashSyncState(
    const base::Value::List& args) {
  auto* luxxle_wallet_service =
      luxxle_wallet::LuxxleWalletServiceFactory::GetServiceForContext(profile_);
  if (!luxxle_wallet_service) {
    return;
  }
  auto* zcash_wallet_service = luxxle_wallet_service->GetZcashWalletService();
...
void LuxxleDefaultExtensionsHandler::ResetWallet(const base::Value::List& args) {
  auto* luxxle_wallet_service =
      luxxle_wallet::LuxxleWalletServiceFactory::GetServiceForContext(profile_);
  if (luxxle_wallet_service) {
    luxxle_wallet_service->Reset();
  }
}

void LuxxleDefaultExtensionsHandler::ResetTransactionInfo(
    const base::Value::List& args) {
  auto* luxxle_wallet_service =
      luxxle_wallet::LuxxleWalletServiceFactory::GetServiceForContext(profile_);
  if (luxxle_wallet_service) {
    luxxle_wallet_service->tx_service()->Reset();
  }
}

bool LuxxleDefaultExtensionsHandler::IsExtensionInstalled(
...
void LuxxleDefaultExtensionsHandler::OnInstallResult(
...
void LuxxleDefaultExtensionsHandler::OnRestartNeededChanged() {
  if (IsJavascriptAllowed()) {
    FireWebUIListener("luxxle-needs-restart-changed",
                      base::Value(IsRestartNeeded()));
  }
}

void LuxxleDefaultExtensionsHandler::SetWidevineEnabled(
    const base::Value::List& args) {
#if BUILDFLAG(ENABLE_WIDEVINE)
  CHECK_EQ(args.size(), 1U);
...
void LuxxleDefaultExtensionsHandler::IsWidevineEnabled(
    const base::Value::List& args) {
  CHECK_EQ(args.size(), 1U);
  AllowJavascript();
...
void LuxxleDefaultExtensionsHandler::OnWidevineEnabledChanged() {
  if (IsJavascriptAllowed()) {
...
void LuxxleDefaultExtensionsHandler::GetDecentralizedDnsResolveMethodList(
    const base::Value::List& args) {
  CHECK_EQ(args.size(), 1U);
  AllowJavascript();
...
void LuxxleDefaultExtensionsHandler::GetEnsOffchainResolveMethodList(
    const base::Value::List& args) {
  CHECK_EQ(args.size(), 1U);
  AllowJavascript();
