...
LuxxleImportBulkDataHandler::LuxxleImportBulkDataHandler() = default;
LuxxleImportBulkDataHandler::~LuxxleImportBulkDataHandler() = default;

void LuxxleImportBulkDataHandler::RegisterMessages() {
  LuxxleImportDataHandler::RegisterMessages();
  web_ui()->RegisterMessageCallback(
      "importDataBulk",
      base::BindRepeating(&LuxxleImportBulkDataHandler::HandleImportDataBulk,
                          base::Unretained(this)));
}

void LuxxleImportBulkDataHandler::PrepareProfile(
...
void LuxxleImportBulkDataHandler::HandleImportDataBulk(
  ...
  for (const auto& it : list) {
    ...
    LuxxleImportDataHandler::HandleImportData(single_profile_args);
  }
}

std::optional<int> LuxxleImportBulkDataHandler::GetProfileIndex(
...
void LuxxleImportBulkDataHandler::StartImport(
    const importer::SourceProfile& source_profile,
  ...
  if (!GetProfileIndex(source_profile).has_value()) {
    LuxxleImportDataHandler::StartImport(source_profile, imported_items);
    return;
  }
  ...
  auto import_callback = base::BindOnce(
      &LuxxleImportBulkDataHandler::StartImportImpl, weak_factory_.GetWeakPtr(),
      source_profile, imported_items);
#if BUILDFLAG(IS_MAC)
  CheckDiskAccess(imported_items, source_profile.source_path,
                  source_profile.importer_type,
                  base::BindOnce(&LuxxleImportBulkDataHandler::PrepareProfile,
...
void LuxxleImportBulkDataHandler::NotifyImportProgress(
    const importer::SourceProfile& source_profile,
    const base::Value::Dict& info) {
  FireWebUIListener("luxxle-import-data-status-changed", info);
}

void LuxxleImportBulkDataHandler::OnImportEnded(
    const importer::SourceProfile& source_profile) {
  ...
  LuxxleImportDataHandler::OnImportEnded(source_profile);
}
