...
#include "luxxle/browser/ui/webui/settings/luxxle_full_disk_access_confirm_dialog_delegate.h"
...
LuxxleImportDataHandler::LuxxleImportDataHandler() = default;
LuxxleImportDataHandler::~LuxxleImportDataHandler() = default;

void LuxxleImportDataHandler::StartImport(
  ...
  CheckDiskAccess(imported_items, source_profile.source_path,
                  ...
                  base::BindOnce(&LuxxleImportDataHandler::StartImportImpl,
...
void LuxxleImportDataHandler::StartImportImpl(
    const importer::SourceProfile& source_profile,
    uint16_t imported_items,
    Profile* profile) {
  ...
  import_observers_[source_profile.source_path] =
      std::make_unique<LuxxleImporterObserver>(
          ...
          base::BindRepeating(&LuxxleImportDataHandler::NotifyImportProgress,
...
void LuxxleImportDataHandler::NotifyImportProgress(
    const importer::SourceProfile& source_profile,
  ...
  if (*event == "ImportItemEnded") {
    ...
    content::GetUIThreadTaskRunner({})->PostTask(
        FROM_HERE, base::BindOnce(&LuxxleImportDataHandler::OnImportEnded,
                                  weak_factory_.GetWeakPtr(), source_profile));
  }
}

void LuxxleImportDataHandler::HandleImportData(const base::Value::List& args) {
  ImportDataHandler::HandleImportData(args);
}

void LuxxleImportDataHandler::OnImportEnded(
    const importer::SourceProfile& source_profile) {
  import_observers_.erase(source_profile.source_path);
...
const importer::SourceProfile& LuxxleImportDataHandler::GetSourceProfileAt(
    int browser_index) {
  return importer_list_->GetSourceProfileAt(browser_index);
}

#if BUILDFLAG(IS_MAC)
void LuxxleImportDataHandler::CheckDiskAccess(
    uint16_t imported_items,
  ...
  if (importer_type == importer::TYPE_SAFARI) {
    // Start import if Luxxle has full disk access permission.
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&LuxxleImportDataHandler::OnGetDiskAccessPermission,
...
void LuxxleImportDataHandler::OnGetDiskAccessPermission(
    ContinueImportCallback callback,
    base::FilePath source_path,
    bool allowed) {
  if (!allowed) {
...
void LuxxleImportDataHandler::DidStopLoading() {
  Observe(nullptr);
