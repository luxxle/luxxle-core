...
void NewTabPageHandler::GetLuxxleBackgrounds(
    GetLuxxleBackgroundsCallback callback) {
  std::move(callback).Run(background_facade_->GetLuxxleBackgrounds());
}
...
void NewTabPageHandler::SetShowSearchBox(bool show_search_box,
  ...
  pref_service_->SetBoolean(luxxle_search_conversion::prefs::kShowNTPSearchBox,
                            show_search_box);
  std::move(callback).Run();
}
...
void NewTabPageHandler::GetSearchSuggestionsPromptDismissed(
  ...
  std::move(callback).Run(
      pref_service_->GetBoolean(luxxle_search_conversion::prefs::kDismissed));
}

void NewTabPageHandler::SetSearchSuggestionsPromptDismissed(
    bool dismissed,
    SetSearchSuggestionsPromptDismissedCallback callback) {
  pref_service_->SetBoolean(luxxle_search_conversion::prefs::kDismissed,
                            dismissed);
  std::move(callback).Run();
}
...
void NewTabPageHandler::SetLastUsedSearchEngine(
  ...
  pref_service_->SetString(
      luxxle_search_conversion::prefs::kLastUsedNTPSearchEngine, engine_host);
  std::move(callback).Run();
}
...
void NewTabPageHandler::GetShowTalkWidget(GetShowTalkWidgetCallback callback) {
  std::move(callback).Run(pref_service_->GetBoolean(kNewTabPageShowLuxxleTalk));
}

void NewTabPageHandler::SetShowTalkWidget(bool show_talk_widget,
                                          SetShowTalkWidgetCallback callback) {
  pref_service_->SetBoolean(kNewTabPageShowLuxxleTalk, show_talk_widget);
  std::move(callback).Run();
}
...
void NewTabPageHandler::OpenVPNAccountPage(
    luxxle_vpn::mojom::ManageURLType url_type,
