...
UpdateObserver::UpdateObserver(PrefService& pref_service,
  ...
  AddPrefListener(luxxle_search_conversion::prefs::kShowNTPSearchBox,
                  Source::kSearch);
  AddPrefListener(prefs::kSearchSuggestEnabled, Source::kSearch);
  AddPrefListener(luxxle_search_conversion::prefs::kDismissed, Source::kSearch);
  ...
  AddPrefListener(luxxle_perf_predictor::prefs::kBandwidthSavedBytes,
                  Source::kShieldsStats);

  AddPrefListener(kNewTabPageShowLuxxleTalk, Source::kTalk);

  AddPrefListener(kNewTabPageShowRewards, Source::kRewards);

#if BUILDFLAG(ENABLE_LUXXLE_VPN)
  AddPrefListener(kNewTabPageShowLuxxleVPN, Source::kVPN);
#endif
