...
#include "luxxle/browser/ui/webui/luxxle_shields/shields_panel_data_handler.h"
...
using luxxle_shields::mojom::SiteSettings;
...
ShieldsPanelDataHandler::ShieldsPanelDataHandler(
  ...
  active_shields_data_controller_ =
      LuxxleShieldsTabHelper::FromWebContents(web_contents);
...
void ShieldsPanelDataHandler::RegisterUIHandler(
    mojo::PendingRemote<luxxle_shields::mojom::UIHandler> ui_handler_receiver) {
...
void ShieldsPanelDataHandler::SetLuxxleShieldsEnabled(bool is_enabled) {
  if (!active_shields_data_controller_) {
    return;
  }

  active_shields_data_controller_->SetLuxxleShieldsEnabled(is_enabled);
}
...
void ShieldsPanelDataHandler::AreAnyBlockedElementsPresent(
  ...
  std::move(callback).Run(
      g_luxxle_browser_process->ad_block_service()->AreAnyBlockedElementsPresent(
...
void ShieldsPanelDataHandler::UpdateSiteBlockInfo() {
  ...
  site_block_info_.is_luxxle_shields_enabled =
      active_shields_data_controller_->GetLuxxleShieldsEnabled();
  site_block_info_.is_luxxle_shields_managed =
      active_shields_data_controller_->IsLuxxleShieldsManaged();
...
void ShieldsPanelDataHandler::OnTabStripModelChanged(
  ...
  if (selection.active_tab_changed()) {
    ...
    if (selection.new_contents) {
      active_shields_data_controller_ =
          LuxxleShieldsTabHelper::FromWebContents(selection.new_contents);
      active_shields_data_controller_->AddObserver(this);
