...
void CookieListOptInPageHandler::ShowUI() {
  base::UmaHistogramExactLinear(luxxle_shields::kCookieListPromptHistogram, 1,
                                4);
  if (embedder_) {
    embedder_->ShowUI();
  }
...
void CookieListOptInPageHandler::EnableFilter() {
  g_luxxle_browser_process->ad_block_service()
      ...
      ->EnableFilterList(luxxle_shields::kCookieListUuid, true);
}

void CookieListOptInPageHandler::OnUINoClicked() {
  base::UmaHistogramExactLinear(luxxle_shields::kCookieListPromptHistogram, 2,
                                4);
}

void CookieListOptInPageHandler::OnUIYesClicked() {
  base::UmaHistogramExactLinear(luxxle_shields::kCookieListPromptHistogram, 3,
                                4);
}
