...
class LuxxleWelcomeUIBrowserTest : public InProcessBrowserTest {
 public:
  LuxxleWelcomeUIBrowserTest() = default;
  ~LuxxleWelcomeUIBrowserTest() override = default;
...
IN_PROC_BROWSER_TEST_F(LuxxleWelcomeUIBrowserTest, PRE_StartupURLTest) {
...
IN_PROC_BROWSER_TEST_F(LuxxleWelcomeUIBrowserTest, StartupURLTest) {
  Browser* new_browser = OpenNewBrowser(browser()->profile());
  ASSERT_TRUE(new_browser);
  TabStripModel* tab_strip = new_browser->tab_strip_model();
  ASSERT_EQ(1, tab_strip->count());
