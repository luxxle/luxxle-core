...
std::optional<gfx::Insets> GetLuxxleLayoutInsets(LayoutInset inset) {
...
// Returns a |nullopt| if the UI color is not handled by Luxxle.
std::optional<int> GetLuxxleLayoutConstant(LayoutConstant constant) {
  ...
  switch (constant) {
    ...
    case TAB_STRIP_HEIGHT: {
      if (HorizontalTabsUpdateEnabled()) {
        return luxxle_tabs::GetHorizontalTabStripHeight() +
    ...
    case TAB_STRIP_PADDING: {
      if (HorizontalTabsUpdateEnabled()) {
        return luxxle_tabs::kHorizontalTabVerticalSpacing;
      }
      return std::nullopt;
    }
