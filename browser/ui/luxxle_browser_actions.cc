...
LuxxleBrowserActions::LuxxleBrowserActions(Browser& browser)
    : BrowserActions(browser) {}

LuxxleBrowserActions::~LuxxleBrowserActions() = default;

void LuxxleBrowserActions::InitializeBrowserActions() {
  ...
  if (ai_chat::IsAIChatEnabled(profile->GetPrefs())) {
    root_action_item_->AddChild(
        SidePanelAction(SidePanelEntryId::kChatUI, IDS_CHAT_UI_TITLE,
                        IDS_CHAT_UI_TITLE, kLeoProductLuxxleLeoIcon,
