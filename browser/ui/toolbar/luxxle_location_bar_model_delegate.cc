...
LuxxleLocationBarModelDelegate::LuxxleLocationBarModelDelegate(Browser* browser)
    : BrowserLocationBarModelDelegate(browser), browser_(browser) {}

LuxxleLocationBarModelDelegate::~LuxxleLocationBarModelDelegate() = default;

// static
void LuxxleLocationBarModelDelegate::FormattedStringFromURL(
    const GURL& url,
    std::u16string* new_formatted_url) {
  // Replace chrome:// with luxxle://
  luxxle_utils::ReplaceChromeToLuxxleScheme(*new_formatted_url);
}

std::u16string
LuxxleLocationBarModelDelegate::FormattedStringWithEquivalentMeaning(
    const GURL& url,
    const std::u16string& formatted_url) const {
  std::u16string new_formatted_url =
      BrowserLocationBarModelDelegate::FormattedStringWithEquivalentMeaning(
          url, formatted_url);
  LuxxleLocationBarModelDelegate::FormattedStringFromURL(url,
                                                        &new_formatted_url);
  return new_formatted_url;
}

bool LuxxleLocationBarModelDelegate::GetURL(GURL* url) const {
#if !BUILDFLAG(IS_ANDROID)
