...
class LuxxleAppMenuModelBrowserTest : public InProcessBrowserTest {
 public:
  LuxxleAppMenuModelBrowserTest() {
#if BUILDFLAG(ENABLE_LUXXLE_VPN)
  ...
  void RunCommandFromAppMenuModel(Browser* browser, int command_id) {
    ...
    LuxxleAppMenuModel model(browser_view->toolbar(), browser);
    model.Init();
...
IN_PROC_BROWSER_TEST_F(LuxxleAppMenuModelBrowserTest, CommandsExecutionTest) {
  RunCommandFromAppMenuModel(CreateIncognitoBrowser(),
                             IDC_EXTENSIONS_SUBMENU_MANAGE_EXTENSIONS);
}

// Test luxxle menu order test.
// Luxxle menu is inserted based on corresponding commands enable status.
// So, this doesn't test for each profiles(normal, private, tor and guest).
// Instead, LuxxleBrowserCommandControllerTest will do that.
IN_PROC_BROWSER_TEST_F(LuxxleAppMenuModelBrowserTest, MenuOrderTest) {
  ...
  if (!syncer::IsSyncAllowedByFlag()) {
    more_tools_in_order.erase(
        std::remove(more_tools_in_order.begin(), more_tools_in_order.end(),
                    IDC_SHOW_LUXXLE_SYNC),
        commands_in_order_for_normal_profile.end());
    more_tools_in_order.push_back(IDC_SHOW_LUXXLE_SYNC);
  }
...
IN_PROC_BROWSER_TEST_F(LuxxleAppMenuModelBrowserTest, LuxxleVPNMenuTest) {
...
IN_PROC_BROWSER_TEST_F(LuxxleAppMenuModelBrowserTest, MenuItemsHaveIcons) {
  auto* browser_view = BrowserView::GetBrowserViewForBrowser(browser());
  LuxxleAppMenuModel model(browser_view->toolbar(), browser());
  model.Init();
