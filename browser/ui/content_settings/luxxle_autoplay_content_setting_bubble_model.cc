...
#include "luxxle/browser/ui/content_settings/luxxle_autoplay_content_setting_bubble_model.h"
...
LuxxleAutoplayContentSettingBubbleModel::LuxxleAutoplayContentSettingBubbleModel(
    Delegate* delegate,
...
LuxxleAutoplayContentSettingBubbleModel::
    ~LuxxleAutoplayContentSettingBubbleModel() = default;

void LuxxleAutoplayContentSettingBubbleModel::CommitChanges() {
  if (settings_changed()) {
...
bool LuxxleAutoplayContentSettingBubbleModel::settings_changed() const {
  return selected_item() != bubble_content().radio_group.default_item;
}

void LuxxleAutoplayContentSettingBubbleModel::SetTitle() {
  set_title(l10n_util::GetStringUTF16(IDS_BLOCKED_AUTOPLAY_TITLE));
}

void LuxxleAutoplayContentSettingBubbleModel::SetRadioGroup() {
...
void LuxxleAutoplayContentSettingBubbleModel::SetNarrowestContentSetting(
    ContentSetting setting) {
  if (!GetProfile())
    return;
