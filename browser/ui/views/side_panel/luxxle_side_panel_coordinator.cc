...
LuxxleSidePanelCoordinator::~LuxxleSidePanelCoordinator() = default;

void LuxxleSidePanelCoordinator::Show(
...
void LuxxleSidePanelCoordinator::OnTabStripModelChanged(
    TabStripModel* tab_strip_model,
    const TabStripModelChange& change,
    const TabStripSelectionChange& selection) {
  auto* luxxle_browser_view = static_cast<LuxxleBrowserView*>(browser_view_);
  const bool active_tab_changed = selection.active_tab_changed();
  if (active_tab_changed) {
    luxxle_browser_view->SetSidePanelOperationByActiveTabChange(true);
  }
  ...
  if (active_tab_changed) {
    luxxle_browser_view->SetSidePanelOperationByActiveTabChange(false);
  }
}

std::unique_ptr<views::View> LuxxleSidePanelCoordinator::CreateHeader() {
  auto header = SidePanelCoordinator::CreateHeader();

  // Luxxle has its own side panel navigation in the form of the SideBar, so
  // hide the Chromium combobox-style header.
  header->SetVisible(false);
  return header;
}

void LuxxleSidePanelCoordinator::Toggle() {
...
void LuxxleSidePanelCoordinator::OnViewVisibilityChanged(
  ...
  if (update_items_state) {
    GetLuxxleBrowserView()->sidebar_container_view()->UpdateActiveItemState();
  }
}

std::optional<SidePanelEntry::Key>
LuxxleSidePanelCoordinator::GetLastActiveEntryKey() const {
...
void LuxxleSidePanelCoordinator::UpdateToolbarButtonHighlight(
    bool side_panel_visible) {
  // Workaround to prevent crashing while window closing.
  // See https://github.com/luxxle/luxxle-browser/issues/34334
  ...
  auto* luxxle_toolbar =
      static_cast<LuxxleToolbarView*>(browser_view_->toolbar());
  if (auto* side_panel_button = luxxle_toolbar->side_panel_button()) {
...
void LuxxleSidePanelCoordinator::PopulateSidePanel(
  ...
  GetLuxxleBrowserView()->sidebar_container_view()->WillShowSidePanel();
  SidePanelCoordinator::PopulateSidePanel(supress_animations, unique_key, entry,
                                          std::move(content_view));
}

void LuxxleSidePanelCoordinator::NotifyPinnedContainerOfActiveStateChange(
    SidePanelEntryKey key,
    bool is_active) {
...
LuxxleBrowserView* LuxxleSidePanelCoordinator::GetLuxxleBrowserView() {
  return static_cast<LuxxleBrowserView*>(browser_view_);
}
