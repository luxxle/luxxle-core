...
LuxxleSidePanel::LuxxleSidePanel(BrowserView* browser_view,
  ...
  if (prefs->FindPreference(sidebar::kSidePanelWidth)) {
    side_panel_width_.Init(
        ...
        base::BindRepeating(&LuxxleSidePanel::OnSidePanelWidthChanged,
  ...
  if (LuxxleBrowser::ShouldUseLuxxleWebViewRoundedCorners(
          browser_view_->browser())) {
    shadow_ = LuxxleContentsViewUtil::CreateShadow(this);
    SetBackground(
...
LuxxleSidePanel::~LuxxleSidePanel() {
  scoped_observation_.RemoveObservation(this);
}

void LuxxleSidePanel::UpdateWidthOnEntryChanged() {
  // Do nothing.
}

bool LuxxleSidePanel::ShouldRestrictMaxWidth() const {
  return false;
}

void LuxxleSidePanel::SetHorizontalAlignment(HorizontalAlignment alignment) {
  horizontal_alignment_ = alignment;
  UpdateBorder();
}

LuxxleSidePanel::HorizontalAlignment LuxxleSidePanel::GetHorizontalAlignment() {
  return horizontal_alignment_;
}

bool LuxxleSidePanel::IsRightAligned() {
  return horizontal_alignment_ == HorizontalAlignment::kRight;
}

void LuxxleSidePanel::UpdateBorder() {
  if (LuxxleBrowser::ShouldUseLuxxleWebViewRoundedCorners(
          browser_view_->browser())) {
...
void LuxxleSidePanel::OnSidePanelWidthChanged() {
  SetPanelWidth(side_panel_width_.GetValue());
}

void LuxxleSidePanel::OnThemeChanged() {
  View::OnThemeChanged();
  UpdateBorder();
}

gfx::Size LuxxleSidePanel::GetMinimumSize() const {
  // Use default width as a minimum width.
  return gfx::Size(sidebar::kDefaultSidePanelWidth, 0);
}

bool LuxxleSidePanel::IsClosing() {
  return false;
}

void LuxxleSidePanel::AddedToWidget() {
  resize_widget_ = std::make_unique<SidePanelResizeWidget>(
      this, static_cast<LuxxleBrowserView*>(browser_view_), this);
}

void LuxxleSidePanel::Layout(PassKey) {
  if (children().empty()) {
    return;
  }
...
double LuxxleSidePanel::GetAnimationValue() const {
  return 1;
}

void LuxxleSidePanel::SetPanelWidth(int width) {
  // Only the width is used by BrowserViewLayout.
  SetPreferredSize(gfx::Size(width, 0));
}

void LuxxleSidePanel::OnResize(int resize_amount, bool done_resizing) {
...
void LuxxleSidePanel::AddHeaderView(std::unique_ptr<views::View> view) {
...
void LuxxleSidePanel::OnChildViewAdded(View* observed_view, View* child) {
...
void LuxxleSidePanel::OnChildViewRemoved(View* observed_view, View* child) {
  if (observed_view != this) {
    return;
  }
  if (scoped_observation_.IsObservingSource(child)) {
...
void LuxxleSidePanel::Open(bool animated) {
  UpdateVisibility(/*should_be_open=*/true);
}

void LuxxleSidePanel::Close(bool animated) {
  UpdateVisibility(/*should_be_open=*/false);
}

void LuxxleSidePanel::UpdateVisibility(bool should_be_open) {
  state_ = should_be_open ? State::kOpen : State::kClosed;
  SetVisible(should_be_open);
}

views::View* LuxxleSidePanel::GetContentParentView() {
  return content_parent_view_;
}

BEGIN_METADATA(LuxxleSidePanel)
END_METADATA
