...
sk_sp<cc::PaintShader> kLuxxleGradient =
...
LuxxleHelpBubbleHostView::LuxxleHelpBubbleHostView() {
...
LuxxleHelpBubbleHostView::~LuxxleHelpBubbleHostView() = default;

bool LuxxleHelpBubbleHostView::Show() {
  if (help_bubble_) {
    return false;
  }
  ...
  auto* luxxle_help_bubble_delegate_view =
      new LuxxleHelpBubbleDelegateView(this, text_);
  help_bubble_ = views::BubbleDialogDelegateView::CreateBubble(
      luxxle_help_bubble_delegate_view);
  auto* frame_view = luxxle_help_bubble_delegate_view->GetBubbleFrameView();
  ...
  activated_subscription_ =
      ui::ElementTracker::GetElementTracker()->AddElementActivatedCallback(
          ...
          base::BindRepeating(
              &LuxxleHelpBubbleHostView::OnTrackedElementActivated,
  ...
  bubble_auto_closing_timer_.Start(
      ...
      base::BindOnce(&LuxxleHelpBubbleHostView::Hide,
                     weak_factory_.GetWeakPtr()));
...
void LuxxleHelpBubbleHostView::Hide() {
  if (!help_bubble_) {
    return;
  }
...
void LuxxleHelpBubbleHostView::UpdatePosition() {
...
void LuxxleHelpBubbleHostView::OnPaint(gfx::Canvas* canvas) {
  cc::PaintFlags flags;
  flags.setAntiAlias(true);
  flags.setStyle(cc::PaintFlags::kStroke_Style);
  flags.setShader(kLuxxleGradient);
...
void LuxxleHelpBubbleHostView::OnViewBoundsChanged(views::View* observed_view) {
  // Update host view position when |tracked_element_| bounds is changed.
  CHECK(tracked_element_ == observed_view);
  UpdatePosition();
}

void LuxxleHelpBubbleHostView::OnViewIsDeleting(views::View* observed_view) {
  CHECK(tracked_element_ == observed_view);
  tracked_element_ = nullptr;
  tracked_view_observation_.Reset();
}

void LuxxleHelpBubbleHostView::OnViewVisibilityChanged(
...
void LuxxleHelpBubbleHostView::OnWidgetBoundsChanged(views::Widget* widget,
                                                    const gfx::Rect&) {
...
void LuxxleHelpBubbleHostView::OnWidgetDestroying(views::Widget* widget) {
  if (help_bubble_ == widget) {
...
void LuxxleHelpBubbleHostView::OnWidgetActivationChanged(views::Widget* widget,
                                                        bool active) {
  if (help_bubble_ != widget) {
    return;
  }
...
void LuxxleHelpBubbleHostView::OnTrackedElementActivated(
    ui::TrackedElement* element) {
  Hide();
}

BEGIN_METADATA(LuxxleHelpBubbleHostView)
END_METADATA
