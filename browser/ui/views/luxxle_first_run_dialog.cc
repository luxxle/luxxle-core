...
#include "luxxle/browser/ui/views/luxxle_first_run_dialog.h"
...
void ShowLuxxleFirstRunDialogViews() {
  base::RunLoop run_loop(base::RunLoop::Type::kNestableTasksAllowed);
  LuxxleFirstRunDialog::Show(run_loop.QuitClosure());
  run_loop.Run();
}
...
// static
void LuxxleFirstRunDialog::Show(base::RepeatingClosure quit_runloop) {
  LuxxleFirstRunDialog* dialog =
      new LuxxleFirstRunDialog(std::move(quit_runloop));
  views::DialogDelegate::CreateDialogWidget(dialog, gfx::NativeWindow(),
                                            gfx::NativeView())
      ->Show();
}

LuxxleFirstRunDialog::LuxxleFirstRunDialog(base::RepeatingClosure quit_runloop)
...
LuxxleFirstRunDialog::~LuxxleFirstRunDialog() = default;

void LuxxleFirstRunDialog::Done() {
  CHECK(!quit_runloop_.is_null());
  quit_runloop_.Run();
}

bool LuxxleFirstRunDialog::Accept() {
  GetWidget()->Hide();

#if BUILDFLAG(ENABLE_PIN_SHORTCUT)
  base::MakeRefCounted<shell_integration::LuxxleDefaultBrowserWorker>()
      ->StartSetAsDefault(base::BindOnce(
          [](bool pin_to_shortcut,
             shell_integration::DefaultWebClientState state) {
            if (pin_to_shortcut &&
                state == shell_integration::DefaultWebClientState::IS_DEFAULT) {
              // Try to pin to taskbar when Luxxle is set as a default browser.
...
void LuxxleFirstRunDialog::WindowClosing() {
  Done();
}

BEGIN_METADATA(LuxxleFirstRunDialog)
END_METADATA
