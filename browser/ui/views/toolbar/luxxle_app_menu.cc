...
#include "luxxle/browser/ui/views/toolbar/luxxle_app_menu.h"
...
#include "luxxle/browser/ui/views/toolbar/luxxle_vpn_status_label.h"
...
class SidebarShowOptionInMenuButton : public views::LabelButton,
 ...
 public:
  SidebarShowOptionInMenuButton(LuxxleAppMenu* app_menu,
...
SidebarShowOptionInMenuButton::SidebarShowOptionInMenuButton(
    ...
    : LabelButton(
      ...
      show_option_(LuxxleAppMenuModel::ConvertIDCToSidebarShowOptions(
...
void SidebarShowOptionInMenuButton::PaintButtonContents(gfx::Canvas* canvas) {
  if (is_active_option_) {
    ...
    bounds.Inset(2);
    cc::PaintFlags flags;
    flags.setColor(cp->GetColor(kColorLuxxleAppMenuAccentColor));
...
class SidebarShowOptionMenu : public views::BoxLayoutView {
 ...
 public:
  SidebarShowOptionMenu(LuxxleAppMenu* app_menu, ui::ButtonMenuItemModel* model);
...
SidebarShowOptionMenu::SidebarShowOptionMenu(LuxxleAppMenu* app_menu,
                                             ui::ButtonMenuItemModel* model)
    : model_(model) {
...
LuxxleAppMenu::LuxxleAppMenu(Browser* browser,
                           ui::MenuModel* model,
                           int run_types)
    : AppMenu(browser, model, run_types),
      menu_metrics_(
          g_luxxle_browser_process->process_misc_metrics()->menu_metrics()) {
  DCHECK(menu_metrics_);
  UpdateMenuItemView();
}

LuxxleAppMenu::~LuxxleAppMenu() = default;

void LuxxleAppMenu::RunMenu(views::MenuButtonController* host) {
  AppMenu::RunMenu(host);
  menu_metrics_->RecordMenuShown();
}

void LuxxleAppMenu::ExecuteCommand(int command_id, int mouse_event_flags) {
  // Suspect that the entry is null but can't imagine which command causes it.
  // See
  // https://github.com/luxxle/luxxle-browser/issues/37862#issuecomment-2078553575
  if (!IsBookmarkCommand(command_id) && !IsTabGroupsCommand(command_id) &&
    ...
    SCOPED_CRASH_KEY_NUMBER("LuxxleAppMenu", "command_id", command_id);
...
void LuxxleAppMenu::OnMenuClosed(views::MenuItemView* menu) {
...
void LuxxleAppMenu::RecordMenuUsage(int command_id) {
  ...
  switch (command_id) {
    ...
    case IDC_SHOW_LUXXLE_REWARDS:
      group = misc_metrics::MenuGroup::kLuxxleFeatures;
      break;
...
void LuxxleAppMenu::UpdateMenuItemView() {
#if BUILDFLAG(ENABLE_LUXXLE_VPN)
  CHECK(root_menu_item());
  if (auto* menu_item =
          root_menu_item()->GetMenuItemByID(IDC_TOGGLE_LUXXLE_VPN)) {
    menu_item->AddChildView(std::make_unique<LuxxleVPNStatusLabel>(browser_));
    menu_item->AddChildView(std::make_unique<LuxxleVPNToggleButton>(browser_));
  }
#endif
