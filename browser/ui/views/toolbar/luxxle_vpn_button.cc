 ...
 private:
  ...
  void Build(bool purchased) {
    ...
    AddItemWithStringId(IDC_TOGGLE_LUXXLE_VPN_TOOLBAR_BUTTON,
                        IDS_LUXXLE_VPN_HIDE_VPN_BUTTON_MENU_ITEM);
    if (purchased) {
      AddItemWithStringId(IDC_SEND_LUXXLE_VPN_FEEDBACK,
                          IDS_LUXXLE_VPN_SHOW_FEEDBACK_MENU_ITEM);
      AddItemWithStringId(IDC_ABOUT_LUXXLE_VPN,
                          IDS_LUXXLE_VPN_ABOUT_VPN_MENU_ITEM);
...
const ui::ColorProvider* GetColorProviderForView(
    base::WeakPtr<LuxxleVPNButton> view) {
  if (view) {
...
class LuxxleVPNBadgeImageSource : public luxxle::LuxxleIconWithBadgeImageSource {
 public:
  LuxxleVPNBadgeImageSource(const gfx::Size& image_size,
                           int icon_size,
                           GetColorProviderCallback get_color_provider_callback,
                           const gfx::VectorIcon& badge_icon,
                           SkColor badge_icon_color)
      : LuxxleIconWithBadgeImageSource(image_size,
...
LuxxleVPNButton::LuxxleVPNButton(Browser* browser)
    : ToolbarButton(base::BindRepeating(&LuxxleVPNButton::OnButtonPressed,
  ...
  auto menu_button_controller = std::make_unique<views::MenuButtonController>(
      this,
      base::BindRepeating(&LuxxleVPNButton::OnButtonPressed,
                          base::Unretained(this)),
      std::make_unique<views::Button::DefaultButtonControllerDelegate>(this));
  menu_button_controller_ = menu_button_controller.get();
  SetButtonController(std::move(menu_button_controller));
  SetAccessibleName(
      l10n_util::GetStringUTF16(IDS_LUXXLE_VPN_TOOLBAR_BUTTON_TEXT));
}

LuxxleVPNButton::~LuxxleVPNButton() = default;

void LuxxleVPNButton::OnConnectionStateChanged(ConnectionState state) {
...
void LuxxleVPNButton::UpdateButtonState() {
  is_error_state_ = IsConnectError();
  is_connected_ = IsConnected();
}

void LuxxleVPNButton::OnPurchasedStateChanged(
    luxxle_vpn::mojom::PurchasedState state,
...
std::unique_ptr<views::Border> LuxxleVPNButton::GetBorder(
...
void LuxxleVPNButton::UpdateColorsAndInsets() {
  ...
  const auto bg_color =
      cp->GetColor(is_error_state_ ? kColorLuxxleVpnButtonErrorBackgroundNormal
                                   : kColorLuxxleVpnButtonBackgroundNormal);
  const int radius = ChromeLayoutProvider::Get()->GetCornerRadiusMetric(
      views::Emphasis::kMaximum, {});
  ...
  auto image_source = std::make_unique<LuxxleVPNBadgeImageSource>(
...
SkColor LuxxleVPNButton::GetIconColor() {
  ui::ColorProvider* cp = GetColorProvider();
  CHECK(cp);

  if (is_error_state_) {
    return cp->GetColor(kColorLuxxleVpnButtonIconError);
  }
...
SkColor LuxxleVPNButton::GetBadgeColor() {
  ui::ColorProvider* cp = GetColorProvider();
  CHECK(cp);
  ...
  return cp->GetColor(is_connected_ ? kColorLuxxleVpnButtonIconConnected
                                    : kColorLuxxleVpnButtonIconDisconnected);
}

const gfx::VectorIcon& LuxxleVPNButton::GetBadgeIcon() {
  if (is_error_state_) {
...
std::u16string LuxxleVPNButton::GetRenderedTooltipText(
    const gfx::Point& p) const {
  if (!IsPurchased()) {
    return l10n_util::GetStringUTF16(IDS_LUXXLE_VPN);
  }

  return l10n_util::GetStringUTF16(IsConnected()
                                       ? IDS_LUXXLE_VPN_CONNECTED_TOOLTIP
                                       : IDS_LUXXLE_VPN_DISCONNECTED_TOOLTIP);
}

void LuxxleVPNButton::OnThemeChanged() {
...
void LuxxleVPNButton::InkDropRippleAnimationEnded(views::InkDropState state) {
  // To use different icon color when activated.
  UpdateColorsAndInsets();
}

bool LuxxleVPNButton::IsConnected() const {
  return service_->IsConnected();
}

ConnectionState LuxxleVPNButton::GetVpnConnectionState() const {
...
bool LuxxleVPNButton::IsConnectError() const {
...
bool LuxxleVPNButton::IsPurchased() const {
  return service_->is_purchased_user();
}
void LuxxleVPNButton::OnButtonPressed(const ui::Event& event) {
  chrome::ExecuteCommand(browser_, IDC_SHOW_LUXXLE_VPN_PANEL);
}

BEGIN_METADATA(LuxxleVPNButton)
END_METADATA
