...
class LuxxleBookmarkContextMenuTest : public testing::Test {
 public:
  LuxxleBookmarkContextMenuTest() : model_(nullptr) {}
 ...
 protected:
  std::unique_ptr<LuxxleBookmarkContextMenu> CreateBookmarkContextMenu() {
    std::vector<raw_ptr<const BookmarkNode, VectorExperimental>> nodes = {
        model_->bookmark_bar_node()};
    auto menu = std::make_unique<LuxxleBookmarkContextMenu>(
...
TEST_F(LuxxleBookmarkContextMenuTest, OverrideBookmarkAlwaysShowMenu) {
  ...
  EXPECT_EQ(submenu->GetMenuItemAt(2)->GetCommand(),
            IDC_LUXXLE_BOOKMARK_BAR_NTP);
  EXPECT_EQ(submenu->GetMenuItems().size(), 3u);
}

TEST_F(LuxxleBookmarkContextMenuTest, AddShowAllBookmarksButtonMenu) {
  auto bookmark_context_menu = CreateBookmarkContextMenu();
  auto* menu = bookmark_context_menu->menu()->GetMenuItemByID(
...
TEST_F(LuxxleBookmarkContextMenuTest, ShowAllBookmarksButtonMenuCheckedState) {
  auto bookmark_context_menu = CreateBookmarkContextMenu();
...
TEST_F(LuxxleBookmarkContextMenuTest,
