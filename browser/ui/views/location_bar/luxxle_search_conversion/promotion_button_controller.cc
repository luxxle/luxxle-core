...
bool PromotionButtonController::PromotionEnabled(PrefService* prefs) {
  ...
  if (prefs->GetBoolean(luxxle_search_conversion::prefs::kDismissed)) {
    return false;
  }
...
PromotionButtonController::PromotionButtonController(
  ...
  button_->SetMakeDefaultCallback(base::BindOnce(
      &PromotionButtonController::SetDefaultAndLoadLuxxleSearchWithCurrentInput,
      weak_factory_.GetWeakPtr()));
  template_url_service_ =
      TemplateURLServiceFactory::GetForProfile(browser_->profile());
  is_luxxle_search_default_ = IsLuxxleSearchDefault();
...
bool PromotionButtonController::ShouldShowSearchPromotionButton() {
  ...
  if (is_luxxle_search_default_) {
    return false;
  }

  if (prefs_->GetBoolean(luxxle_search_conversion::prefs::kDismissed)) {
    return false;
  }

  // No popup means no suggestions for current input.
  // Promotion button will be shown for current search provider's
  // suggestion entries to make users search with luxxle search with that
  // suggestion.
  ...
  return !IsLuxxleSearchPromotionMatch(match) &&
...
void PromotionButtonController::SetDefaultAndLoadLuxxleSearchWithCurrentInput() {
  CHECK(omnibox_view_);

  // Set luxxle search as default.
  ...
  // Load luxxle search with current input.
  const auto url =
  ...
  // Once user set luxxle as a default, we'll not show this button again.
  Dismissed();
}
...
void PromotionButtonController::Dismissed() {
  ...
  luxxle_search_conversion::SetDismissed(&*prefs_);
...
bool PromotionButtonController::IsLuxxleSearchDefault() {
  const auto* template_url = template_url_service_->GetDefaultSearchProvider();
