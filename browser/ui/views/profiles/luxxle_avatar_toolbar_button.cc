...
class LuxxleAvatarButtonHighlightPathGenerator
    : public views::HighlightPathGenerator {
 public:
  LuxxleAvatarButtonHighlightPathGenerator(
      const base::WeakPtr<LuxxleAvatarToolbarButton>& avatar_button,
      int radius)
      : avatar_button_(avatar_button), radius_(radius) {}
  LuxxleAvatarButtonHighlightPathGenerator(
      const LuxxleAvatarButtonHighlightPathGenerator&) = delete;
  LuxxleAvatarButtonHighlightPathGenerator& operator=(
      const LuxxleAvatarButtonHighlightPathGenerator&) = delete;
  ~LuxxleAvatarButtonHighlightPathGenerator() override = default;
 ...
 private:
  base::WeakPtr<LuxxleAvatarToolbarButton> avatar_button_;
  int radius_ = 0;
};

}  // namespace

LuxxleAvatarToolbarButton::LuxxleAvatarToolbarButton(BrowserView* browser_view)
...
LuxxleAvatarToolbarButton::~LuxxleAvatarToolbarButton() = default;

void LuxxleAvatarToolbarButton::SetHighlight(
...
void LuxxleAvatarToolbarButton::OnThemeChanged() {
  ...
  views::HighlightPathGenerator::Install(
      this, std::make_unique<LuxxleAvatarButtonHighlightPathGenerator>(
                weak_ptr_factory_.GetWeakPtr(), radius));
}

void LuxxleAvatarToolbarButton::UpdateColorsAndInsets() {
  ...
  if (browser_->profile()->IsOffTheRecord()) {
    ...
    constexpr int kLuxxleAvatarButtonHorizontalSpacing = 8;
    gfx::Insets target_insets = ::GetLayoutInsets(TOOLBAR_BUTTON);
    target_insets.set_left_right(kLuxxleAvatarButtonHorizontalSpacing,
                                 kLuxxleAvatarButtonHorizontalSpacing);
    if (!is_tor) {
      // Use smaller vertical margins as we user more larger icon.
      constexpr int kLuxxleAvatarButtonVerticalSpacing = 3;
      target_insets.set_top_bottom(kLuxxleAvatarButtonVerticalSpacing,
                                   kLuxxleAvatarButtonVerticalSpacing);
    }
    ...
    constexpr int kLuxxleAvatarImageLabelSpacing = 8;
    SetImageLabelSpacing(kLuxxleAvatarImageLabelSpacing);
    return;
  }
...
BEGIN_METADATA(LuxxleAvatarToolbarButton)
END_METADATA
