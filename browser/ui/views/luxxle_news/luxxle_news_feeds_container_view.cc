...
LuxxleNewsFeedsContainerView::LuxxleNewsFeedsContainerView(
    content::WebContents* contents) {
  auto* tab_helper = LuxxleNewsTabHelper::FromWebContents(contents);

  auto available_feeds = tab_helper->GetAvailableFeedUrls();
  for (auto& feed_item : available_feeds) {
    auto* child = AddChildView(
        std::make_unique<LuxxleNewsFeedItemView>(feed_item, contents));
...
LuxxleNewsFeedsContainerView::~LuxxleNewsFeedsContainerView() = default;

void LuxxleNewsFeedsContainerView::OnThemeChanged() {
...
BEGIN_METADATA(LuxxleNewsFeedsContainerView)
END_METADATA
