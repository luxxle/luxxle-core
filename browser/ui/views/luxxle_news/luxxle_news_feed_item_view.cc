...
LuxxleNewsFeedItemView::LuxxleNewsFeedItemView(const GURL& feed_url,
                                             content::WebContents* contents)
    : feed_url_(feed_url), contents_(contents) {
  DCHECK(contents_);
  tab_helper_ = LuxxleNewsTabHelper::FromWebContents(contents);
  ...
  subscribe_button_ = AddChildView(std::make_unique<views::MdTextButton>(
      base::BindRepeating(&LuxxleNewsFeedItemView::OnPressed,
...
LuxxleNewsFeedItemView::~LuxxleNewsFeedItemView() = default;

void LuxxleNewsFeedItemView::Update() {
  ...
  subscribe_button_->SetText(l10n_util::GetStringUTF16(
      is_subscribed ? IDS_LUXXLE_NEWS_BUBBLE_FEED_ITEM_UNSUBSCRIBE
                    : IDS_LUXXLE_NEWS_BUBBLE_FEED_ITEM_SUBSCRIBE));
...
void LuxxleNewsFeedItemView::OnPressed() {
...
void LuxxleNewsFeedItemView::OnAvailableFeedsChanged(
    const std::vector<GURL>& feed_urls) {
  loading_ = false;
  Update();
}

BEGIN_METADATA(LuxxleNewsFeedItemView)
END_METADATA
