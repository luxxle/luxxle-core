...
#include "luxxle/browser/ui/views/luxxle_news/luxxle_news_feed_item_view.h"
#include "luxxle/browser/ui/views/luxxle_news/luxxle_news_feeds_container_view.h"
...
// static
base::WeakPtr<views::Widget> LuxxleNewsBubbleView::Show(
    views::View* anchor,
    content::WebContents* contents) {
  auto* widget = views::BubbleDialogDelegateView::CreateBubble(
      std::make_unique<LuxxleNewsBubbleView>(anchor, contents));
  widget->Show();
  return widget->GetWeakPtr();
}

LuxxleNewsBubbleView::LuxxleNewsBubbleView(views::View* action_view,
                                         content::WebContents* contents)
  ...
  auto* controller =
      luxxle_news::LuxxleNewsBubbleController::FromWebContents(contents);
  CHECK(controller);
  ...
  title_label_ = AddChildView(std::make_unique<views::Label>(
      l10n_util::GetStringUTF16(IDS_LUXXLE_NEWS_BUBBLE_TITLE), custom_font));
  ...
  subtitle_label_ = AddChildView(std::make_unique<views::Label>(
      l10n_util::GetStringUTF16(IDS_LUXXLE_NEWS_BUBBLE_SUBTITLE),
  ...
  feeds_container_ =
      AddChildView(std::make_unique<LuxxleNewsFeedsContainerView>(contents));
  ...
  auto* manage_feeds_button =
      AddChildView(std::make_unique<views::MdTextButton>(
          base::BindRepeating(&LuxxleNewsBubbleView::OpenManageFeeds,
                              base::Unretained(this)),
          l10n_util::GetStringUTF16(IDS_LUXXLE_NEWS_BUBBLE_MANAGE_FEEDS)));
...
LuxxleNewsBubbleView::~LuxxleNewsBubbleView() = default;

void LuxxleNewsBubbleView::OpenManageFeeds() {
  auto* browser = chrome::FindBrowserWithTab(contents_);
  browser->OpenURL(
      {GURL("luxxle://newtab/?openSettings=LuxxleNews"), content::Referrer(),
       WindowOpenDisposition::NEW_FOREGROUND_TAB, ui::PAGE_TRANSITION_LINK,
       false},
      /*navigation_handle_callback=*/{});
}

void LuxxleNewsBubbleView::OnWidgetDestroyed(views::Widget*) {
  if (!controller_) {
    return;
  }
...
void LuxxleNewsBubbleView::OnThemeChanged() {
  views::BubbleDialogDelegateView::OnThemeChanged();

  auto is_dark = dark_mode::GetActiveLuxxleDarkModeType() ==
...
BEGIN_METADATA(LuxxleNewsBubbleView)
END_METADATA
