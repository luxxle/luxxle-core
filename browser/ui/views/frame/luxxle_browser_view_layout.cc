...
LuxxleBrowserViewLayout::LuxxleBrowserViewLayout(
...
LuxxleBrowserViewLayout::~LuxxleBrowserViewLayout() = default;

int LuxxleBrowserViewLayout::GetIdealSideBarWidth() const {
...
int LuxxleBrowserViewLayout::GetIdealSideBarWidth(int available_width) const {
  if (!sidebar_container_) {
    return 0;
  }
...
void LuxxleBrowserViewLayout::Layout(views::View* host) {
  BrowserViewLayout::Layout(host);
  LayoutVerticalTabs();
}

void LuxxleBrowserViewLayout::LayoutVerticalTabs() {
...
void LuxxleBrowserViewLayout::LayoutSidePanelView(
...
int LuxxleBrowserViewLayout::LayoutTabStripRegion(int top) {
  if (tabs::utils::ShouldShowVerticalTabs(browser_view_->browser())) {
...
int LuxxleBrowserViewLayout::LayoutBookmarkAndInfoBars(int top,
                                                      int browser_view_y) {
...
int LuxxleBrowserViewLayout::LayoutInfoBar(int top) {
...
void LuxxleBrowserViewLayout::LayoutContentsContainerView(int top, int bottom) {
  if (!vertical_tab_strip_host_) {
...
void LuxxleBrowserViewLayout::LayoutSideBar(gfx::Rect& contents_bounds) {
  if (!sidebar_container_) {
    return;
  }
...
void LuxxleBrowserViewLayout::UpdateContentsContainerInsets(
    gfx::Rect& contents_container_bounds) {
  ...
  int contents_margin_for_rounded_corners =
      LuxxleContentsViewUtil::GetRoundedCornersWebViewMargin(
          browser_view_->browser());
...
gfx::Insets LuxxleBrowserViewLayout::GetContentsMargins() const {
  if (!LuxxleBrowser::ShouldUseLuxxleWebViewRoundedCorners(
          browser_view_->browser())) {
    return {};
  }
  ...
  gfx::Insets margins(LuxxleContentsViewUtil::kMarginThickness);
...
bool LuxxleBrowserViewLayout::IsFullscreenForBrowser() const {
...
bool LuxxleBrowserViewLayout::IsFullscreenForTab() const {
  auto* exclusive_access_manager = browser_view_->GetExclusiveAccessManager();
  if (!exclusive_access_manager) {
    return false;
  }
...
bool LuxxleBrowserViewLayout::ShouldPushBookmarkBarForVerticalTabs() {
...
gfx::Insets LuxxleBrowserViewLayout::GetInsetsConsideringVerticalTabHost() {
  CHECK(vertical_tab_strip_host_)
      << "This method is used only when vertical tab strip host is set";
  gfx::Insets insets;
  if (tabs::utils::IsVerticalTabOnRight(browser_view_->browser())) {
...
#if BUILDFLAG(IS_MAC)
gfx::Insets LuxxleBrowserViewLayout::AdjustInsetsConsideringFrameBorder(
