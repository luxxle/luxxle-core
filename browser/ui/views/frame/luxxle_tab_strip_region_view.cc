...
LuxxleTabStripRegionView::~LuxxleTabStripRegionView() = default;

void LuxxleTabStripRegionView::Layout(PassKey) {
...
void LuxxleTabStripRegionView::UpdateTabStripMargin() {
  ...
  if (tabs::features::HorizontalTabsUpdateEnabled()) {
    if (!tab_strip_->controller()->IsFrameCondensed() && !vertical_tabs) {
      margins.set_left(luxxle_tabs::kHorizontalTabStripLeftMargin);
    } else {
      margins.set_left(0);
    }
  }
...
void LuxxleTabStripRegionView::Initialize() {
...
BEGIN_METADATA(LuxxleTabStripRegionView)
END_METADATA
