...
using LuxxleNonClientHitTestHelperBrowserTest = InProcessBrowserTest;

// TODO(sko) It might be good to have resizable area tests. But testing it
// is pretty flaky depending on platforms.
IN_PROC_BROWSER_TEST_F(LuxxleNonClientHitTestHelperBrowserTest, Toolbar) {
  ...
  for (views::View* view : toolbar_container->GetChildrenInZOrder()) {
    ...
    // LuxxleToolbarView::ViewHierarchyChanged where we set children that way).
