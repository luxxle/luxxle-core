...
LuxxleOpaqueBrowserFrameView::LuxxleOpaqueBrowserFrameView(
    BrowserFrame* frame,
    BrowserView* browser_view,
    OpaqueBrowserFrameViewLayout* layout)
    : OpaqueBrowserFrameView(frame, browser_view, layout) {
  frame_graphic_ = std::make_unique<LuxxleWindowFrameGraphic>(
      browser_view->browser()->profile());
}

LuxxleOpaqueBrowserFrameView::~LuxxleOpaqueBrowserFrameView() = default;

void LuxxleOpaqueBrowserFrameView::OnPaint(gfx::Canvas* canvas) {
...
int LuxxleOpaqueBrowserFrameView::NonClientHitTest(const gfx::Point& point) {
...
void LuxxleOpaqueBrowserFrameView::
  ...
  static_cast<LuxxleToolbarView*>(browser_view()->toolbar())
      ->UpdateHorizontalPadding();
}

void LuxxleOpaqueBrowserFrameView::PaintClientEdge(gfx::Canvas* canvas) const {
...
int LuxxleOpaqueBrowserFrameView::GetTopInset(bool restored) const {
  if (ShouldShowVerticalTabs()) {
...
int LuxxleOpaqueBrowserFrameView::GetTopAreaHeight() const {
  if (ShouldShowVerticalTabs()) {
    // In order to ignore horizontal tab strip's height, we by pass the base
    // class's implementation.
...
bool LuxxleOpaqueBrowserFrameView::ShouldShowVerticalTabs() const {
  DCHECK(browser_view());
  auto* browser = browser_view()->browser();
  DCHECK(browser);
...
BEGIN_METADATA(LuxxleOpaqueBrowserFrameView)
END_METADATA
