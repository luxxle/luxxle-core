...
LuxxleBrowserFrameViewWin::LuxxleBrowserFrameViewWin(BrowserFrame* frame,
                                                   BrowserView* browser_view)
    : BrowserFrameViewWin(frame, browser_view) {
  frame_graphic_.reset(
      new LuxxleWindowFrameGraphic(browser_view->browser()->profile()));
  ...
  using_vertical_tabs_.Init(
      ...
      base::BindRepeating(&LuxxleBrowserFrameViewWin::OnVerticalTabsPrefsChanged,
...
LuxxleBrowserFrameViewWin::~LuxxleBrowserFrameViewWin() = default;

bool LuxxleBrowserFrameViewWin::ShouldCaptionButtonsBeDrawnOverToolbar() const {
  auto* browser = browser_view()->browser();
  return tabs::utils::ShouldShowVerticalTabs(browser) &&
         !tabs::utils::ShouldShowWindowTitleForVerticalTabs(browser);
}

void LuxxleBrowserFrameViewWin::OnVerticalTabsPrefsChanged() {
  caption_button_container_->UpdateButtons();
  caption_button_container_->InvalidateLayout();
  LayoutCaptionButtons();
}

void LuxxleBrowserFrameViewWin::OnPaint(gfx::Canvas* canvas) {
...
int LuxxleBrowserFrameViewWin::GetTopInset(bool restored) const {
...
int LuxxleBrowserFrameViewWin::NonClientHitTest(const gfx::Point& point) {
  ...
  if (auto overridden_result = luxxle::NonClientHitTest(browser_view(), point);
...
bool LuxxleBrowserFrameViewWin::ShouldShowWindowTitle(TitlebarType type) const {
  if (auto* browser = browser_view()->browser();
      tabs::utils::ShouldShowVerticalTabs(browser) &&
      tabs::utils::ShouldShowWindowTitleForVerticalTabs(browser) &&
...
void LuxxleBrowserFrameViewWin::LayoutCaptionButtons() {
