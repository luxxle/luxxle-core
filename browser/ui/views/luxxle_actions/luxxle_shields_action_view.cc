...
class LuxxleShieldsActionViewHighlightPathGenerator
    : public views::HighlightPathGenerator {
 public:
  LuxxleShieldsActionViewHighlightPathGenerator() = default;
  LuxxleShieldsActionViewHighlightPathGenerator(
      const LuxxleShieldsActionViewHighlightPathGenerator&) = delete;
  LuxxleShieldsActionViewHighlightPathGenerator& operator=(
      const LuxxleShieldsActionViewHighlightPathGenerator&) = delete;
  ~LuxxleShieldsActionViewHighlightPathGenerator() override = default;

  SkPath GetHighlightPath(const views::View* view) override {
    return static_cast<const LuxxleShieldsActionView*>(view)->GetHighlightPath();
  }
};
}  // namespace

DEFINE_CLASS_ELEMENT_IDENTIFIER_VALUE(LuxxleShieldsActionView,
                                      kShieldsActionIcon);

LuxxleShieldsActionView::LuxxleShieldsActionView(
    BrowserWindowInterface* browser_window_interface)
    : LabelButton(base::BindRepeating(&LuxxleShieldsActionView::ButtonPressed,
  ...
  SetAccessibleName(l10n_util::GetStringUTF16(IDS_LUXXLE_SHIELDS));
  ...
  auto menu_button_controller = std::make_unique<views::MenuButtonController>(
      this,
      base::BindRepeating(&LuxxleShieldsActionView::ButtonPressed,
...
void LuxxleShieldsActionView::Init() {
  UpdateIconState();
  views::HighlightPathGenerator::Install(
      this, std::make_unique<LuxxleShieldsActionViewHighlightPathGenerator>());
}

SkPath LuxxleShieldsActionView::GetHighlightPath() const {
  ...
  auto highlight_insets =
      gfx::Insets::TLBR(0, 0, 0, -1 * kLuxxleActionLeftMarginExtra);
...
LuxxleShieldsActionView::GetImageSource() {
  ...
  std::unique_ptr<IconWithBadgeImageSource> image_source(
      new luxxle::LuxxleIconWithBadgeImageSource(
  ...
  if (web_contents) {
    ...
    is_enabled = shields_data_controller->GetLuxxleShieldsEnabled() &&
...
gfx::ImageSkia LuxxleShieldsActionView::GetIconImage(bool is_enabled) {
  ui::ResourceBundle& rb = ui::ResourceBundle::GetSharedInstance();
  gfx::ImageSkia image;
  const SkBitmap bitmap =
      rb.GetImageNamed(is_enabled ? /* REMOVED: IDR_LUXXLE_SHIELDS_ICON_64 */ 0
                                  : /* REMOVED: IDR_LUXXLE_SHIELDS_ICON_64_DISABLED */ 0)
          .AsBitmap();
...
void LuxxleShieldsActionView::UpdateIconState() {
...
void LuxxleShieldsActionView::ButtonPressed(
    BrowserWindowInterface* browser_window_interface) {
...
bool LuxxleShieldsActionView::IsPageInReaderMode(
...
bool LuxxleShieldsActionView::ShouldShowBubble(
    content::WebContents* web_contents) {
  if (!web_contents) {
    return false;
  }
  ...
  if (url.SchemeIs(url::kAboutScheme) || url.SchemeIs(url::kBlobScheme) ||
      ...
      url.SchemeIs(kMagnetScheme) || url.SchemeIs(kLuxxleUIScheme) ||
...
LuxxleShieldsActionView::CreateDefaultBorder() const {
...
std::u16string LuxxleShieldsActionView::GetRenderedTooltipText(
  ...
  if (web_contents) {
    ...
    if (count > 0) {
      return l10n_util::GetStringFUTF16Int(IDS_LUXXLE_SHIELDS_ICON_TOOLTIP,
                                           count);
    }
  }

  return l10n_util::GetStringUTF16(IDS_LUXXLE_SHIELDS);
}

void LuxxleShieldsActionView::OnThemeChanged() {
...
void LuxxleShieldsActionView::Update() {
  UpdateIconState();
}

void LuxxleShieldsActionView::OnResourcesChanged() {
  UpdateIconState();
}

void LuxxleShieldsActionView::OnShieldsEnabledChanged() {
  UpdateIconState();
}

void LuxxleShieldsActionView::OnTabStripModelChanged(
    TabStripModel* tab_strip_model,
    const TabStripModelChange& change,
    const TabStripSelectionChange& selection) {
  if (selection.active_tab_changed()) {
    if (selection.new_contents) {
      luxxle_shields::LuxxleShieldsTabHelper::FromWebContents(
...
BEGIN_METADATA(LuxxleShieldsActionView)
END_METADATA
