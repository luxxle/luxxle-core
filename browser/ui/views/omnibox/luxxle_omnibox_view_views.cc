...
namespace {
void LuxxleUpdateContextMenu(ui::SimpleMenuModel* menu_contents, GURL url) {
...
LuxxleOmniboxViewViews::~LuxxleOmniboxViewViews() = default;

std::optional<GURL> LuxxleOmniboxViewViews::GetURLToCopy() {
  GURL url;
...
bool LuxxleOmniboxViewViews::SelectedTextIsURL() {
  return GetURLToCopy().has_value();
}

void LuxxleOmniboxViewViews::CleanAndCopySelectedURL() {
...
void LuxxleOmniboxViewViews::CopySanitizedURL(const GURL& url) {
...
#if BUILDFLAG(IS_WIN)
bool LuxxleOmniboxViewViews::AcceleratorPressed(
    const ui::Accelerator& accelerator) {
  if (!base::FeatureList::IsEnabled(features::kLuxxleCopyCleanLinkByDefault)) {
...
bool LuxxleOmniboxViewViews::GetAcceleratorForCommandId(
    int command_id,
    ui::Accelerator* accelerator) const {
  if (base::FeatureList::IsEnabled(features::kLuxxleCopyCleanLinkByDefault)) {
    bool is_url = const_cast<LuxxleOmniboxViewViews*>(this)->SelectedTextIsURL();
    if (is_url) {
      if (command_id == kCopy) {
        return false;
      }
...
void LuxxleOmniboxViewViews::ExecuteTextEditCommand(
...
void LuxxleOmniboxViewViews::ExecuteCommand(int command_id, int event_flags) {
...
LuxxleOmniboxViewViews::GetClipboardTextForPasteAndSearch() {
...
void LuxxleOmniboxViewViews::UpdateContextMenu(
    ui::SimpleMenuModel* menu_contents) {
  OmniboxViewViews::UpdateContextMenu(menu_contents);
  auto url_to_copy = GetURLToCopy();
  if (url_to_copy.has_value()) {
    LuxxleUpdateContextMenu(menu_contents, url_to_copy.value());
  }
}

BEGIN_METADATA(LuxxleOmniboxViewViews)
END_METADATA
