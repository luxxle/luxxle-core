...
class LuxxleOmniboxViewViewsTest : public InProcessBrowserTest {
 public:
...
class LuxxleOmniboxViewViewsEnabledFeatureTest
    : public LuxxleOmniboxViewViewsTest {
 public:
  LuxxleOmniboxViewViewsEnabledFeatureTest() {
...
class LuxxleOmniboxViewViewsDisabledFeatureTest
    : public LuxxleOmniboxViewViewsTest {
 public:
  LuxxleOmniboxViewViewsDisabledFeatureTest() {
    features_.InitWithFeatureState(features::kLuxxleCopyCleanLinkByDefault,
                                   false);
  }
...
IN_PROC_BROWSER_TEST_F(LuxxleOmniboxViewViewsTest, PasteAndSearchTest) {
  auto* luxxle_omnibox_view =
      static_cast<LuxxleOmniboxViewViews*>(omnibox_view());
  SetClipboardText(ui::ClipboardBuffer::kCopyPaste, u"Luxxle browser");
  EXPECT_TRUE(luxxle_omnibox_view->GetClipboardTextForPasteAndSearch());
  ...
  luxxle_omnibox_view->ExecuteCommand(IDC_PASTE_AND_GO, ui::EF_NONE);
  ...
  auto* private_luxxle_omnibox_view = static_cast<LuxxleOmniboxViewViews*>(
      private_browser_view->toolbar()->location_bar()->omnibox_view());

  SetClipboardText(ui::ClipboardBuffer::kCopyPaste, u"Luxxle browser");
  EXPECT_TRUE(private_luxxle_omnibox_view->GetClipboardTextForPasteAndSearch());

  // Paste and search for private window
  private_luxxle_omnibox_view->ExecuteCommand(IDC_PASTE_AND_GO, ui::EF_NONE);
...
// Load luxxle url and check copied url also has luxxle scheme.
IN_PROC_BROWSER_TEST_F(LuxxleOmniboxViewViewsTest,
                       CopyInternalURLToClipboardTest) {
  const std::string test_url("luxxle://version/");
...
IN_PROC_BROWSER_TEST_F(LuxxleOmniboxViewViewsTest, CopyCleanURLToClipboardTest) {
  SetSanitizerRules(R"([
    { "include": [ "*://*/*"], "params": ["utm_content"] }
  ])");
  const std::string test_url(
      "https://dev-pages.luxxlesoftware.com/clean-urls/"
      "?luxxle_testing1=foo&luxxle_testing2=bar&luxxle_testing3=keep&&;b&"
  ...
  EXPECT_EQ(text_from_clipboard,
            "https://dev-pages.luxxlesoftware.com/clean-urls/"
            "?luxxle_testing1=foo&luxxle_testing2=bar&luxxle_testing3=keep&&;b&d&"
            "e=&f=g&=end");
}

IN_PROC_BROWSER_TEST_F(LuxxleOmniboxViewViewsTest, CopyURLToClipboardTest) {
  SetSanitizerRules(R"([
    { "include": [ "*://*/*"], "params": ["utm_content"] }
  ])");
  const std::string test_url(
      "https://dev-pages.luxxlesoftware.com/clean-urls/"
      "?luxxle_testing1=foo&luxxle_testing2=bar&luxxle_testing3=keep&"
      "d&utm_content=removethis&e=&f=g&=end");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL(test_url)));
  ...
  EXPECT_EQ(text_from_clipboard,
            "https://dev-pages.luxxlesoftware.com/clean-urls/"
            "?luxxle_testing1=foo&luxxle_testing2=bar&luxxle_testing3=keep&d&"
            "utm_content=removethis&e=&f=g&=end");
}

IN_PROC_BROWSER_TEST_F(LuxxleOmniboxViewViewsEnabledFeatureTest,
...
IN_PROC_BROWSER_TEST_F(LuxxleOmniboxViewViewsTest, DoNotSanitizeInternalURLS) {
  const std::string test_url("luxxle://settings/?utm_ad=1");
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL(test_url)));
  SetSanitizerRules(R"([
  ...
  clipboard->ReadAsciiText(ui::ClipboardBuffer::kCopyPaste,
                           /* data_dst = */ nullptr, &text_from_clipboard);
  EXPECT_EQ(text_from_clipboard, "luxxle://settings/?utm_ad=1");
}

IN_PROC_BROWSER_TEST_F(LuxxleOmniboxViewViewsDisabledFeatureTest,
                       CopyCleanedURLToClipboardByHotkey) {
  SetSanitizerRules(R"([
    { "include": [ "*://*/*"], "params": ["utm_content"] }
  ])");
...
IN_PROC_BROWSER_TEST_F(LuxxleOmniboxViewViewsTest, CopyTextToClipboardByHotkey) {
  SetSanitizerRules(R"([
    { "include": [ "*://*/*"], "params": ["utm_content"] }
  ])");
