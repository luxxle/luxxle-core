...
LuxxleTabStrip::LuxxleTabStrip(std::unique_ptr<TabStripController> controller)
    : TabStrip(std::move(controller)) {}

LuxxleTabStrip::~LuxxleTabStrip() = default;

bool LuxxleTabStrip::IsVerticalTabsFloating() const {
...
bool LuxxleTabStrip::ShouldDrawStrokes() const {
  if (ShouldShowVerticalTabs()) {
  ...
  return contrast_ratio < kLuxxleMinimumContrastRatioForOutlines;
}

void LuxxleTabStrip::UpdateHoverCard(Tab* tab, HoverCardUpdateType update_type) {
  if (luxxle_tabs::AreTooltipsEnabled(controller_->GetProfile()->GetPrefs())) {
    return;
  }
  TabStrip::UpdateHoverCard(tab, update_type);
}

void LuxxleTabStrip::MaybeStartDrag(
    TabSlotView* source,
...
void LuxxleTabStrip::AddedToWidget() {
  ...
  if (BrowserView::GetBrowserViewForBrowser(GetBrowser())) {
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&LuxxleTabStrip::UpdateTabContainer,
                                  weak_factory_.GetWeakPtr()));
  }
}

std::optional<int> LuxxleTabStrip::GetCustomBackgroundId(
...
bool LuxxleTabStrip::IsTabTiled(const Tab* tab) const {
  return GetTileForTab(tab).has_value();
}

bool LuxxleTabStrip::IsFirstTabInTile(const Tab* tab) const {
  auto* browser = GetBrowser();
...
TabTiledState LuxxleTabStrip::GetTiledStateForTab(int index) const {
...
std::optional<TabTile> LuxxleTabStrip::GetTileForTab(const Tab* tab) const {
...
void LuxxleTabStrip::UpdateTabContainer() {
  ...
  const bool is_using_compound_tab_container =
      views::IsViewClass<LuxxleCompoundTabContainer>(
  ...
  if (should_use_compound_tab_container != is_using_compound_tab_container) {
    ...
    if (should_use_compound_tab_container) {
      ...
      auto* luxxle_tab_container = AddChildViewAt(
          std::make_unique<LuxxleCompoundTabContainer>(
              *this, hover_card_controller_.get(), drag_context, *this, this),
          0);
      tab_container_ = *luxxle_tab_container;
      layout_lock =
          base::ScopedClosureRunner(luxxle_tab_container->LockLayout());

      luxxle_tab_container->SetScrollEnabled(using_vertical_tabs);
      ...
      auto* luxxle_tab_container =
          AddChildViewAt(std::make_unique<LuxxleTabContainer>(
...
bool LuxxleTabStrip::ShouldShowVerticalTabs() const {
  return tabs::utils::ShouldShowVerticalTabs(GetBrowser());
}

TabContainer* LuxxleTabStrip::GetTabContainerForTesting() {
  return &tab_container_.get();  // IN-TEST
}

void LuxxleTabStrip::Layout(PassKey) {
  if (ShouldShowVerticalTabs()) {
