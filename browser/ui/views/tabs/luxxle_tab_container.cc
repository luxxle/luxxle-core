...
LuxxleTabContainer::LuxxleTabContainer(
  ...
  show_vertical_tabs_.Init(
      ...
      base::BindRepeating(&LuxxleTabContainer::UpdateLayoutOrientation,
...
base::OnceClosure LuxxleTabContainer::LockLayout() {
  DCHECK(!layout_locked_) << "LockLayout() doesn't allow reentrance";
  layout_locked_ = true;
  return base::BindOnce(&LuxxleTabContainer::OnUnlockLayout,
                        base::Unretained(this));
}

void LuxxleTabContainer::AddedToWidget() {
...
gfx::Size LuxxleTabContainer::CalculatePreferredSize(
  ...
  if (tab_count) {
    if (Tab* last_tab = tabs_view_model_.view_at(tab_count - 1);
      ...
      height += LuxxleTabGroupHeader::kPaddingForGroup;
    }
...
void LuxxleTabContainer::UpdateClosingModeOnRemovedTab(int model_index,
                                                      bool was_active) {
...
gfx::Rect LuxxleTabContainer::GetTargetBoundsForClosingTab(
    Tab* tab,
...
void LuxxleTabContainer::EnterTabClosingMode(std::optional<int> override_width,
...
bool LuxxleTabContainer::ShouldTabBeVisible(const Tab* tab) const {
...
void LuxxleTabContainer::StartInsertTabAnimation(int model_index) {
  // Note that we check this before checking currently we're in vertical tab
  // strip mode. We might be in the middle of changing orientation.
  if (layout_locked_) {
    return;
...
void LuxxleTabContainer::RemoveTab(int index, bool was_active) {
  if (tabs::utils::ShouldShowVerticalTabs(tab_slot_controller_->GetBrowser())) {
...
void LuxxleTabContainer::OnTabCloseAnimationCompleted(Tab* tab) {
  if (tabs::utils::ShouldShowVerticalTabs(tab_slot_controller_->GetBrowser())) {
...
void LuxxleTabContainer::UpdateLayoutOrientation() {
...
void LuxxleTabContainer::PaintBoundingBoxForTiles(
    gfx::Canvas& canvas,
...
void LuxxleTabContainer::PaintBoundingBoxForTile(gfx::Canvas& canvas,
                                                const TabTile& tile) {
  if (!GetTabCount()) {
    return;
  }
  ...
  if (!is_vertical_tab) {
    ...
    bounding_rects.Inset(gfx::Insets::TLBR(
        vertical_margin, luxxle_tabs::kHorizontalTabInset,
  ...
  flags.setColor(cp->GetColor(
      is_vertical_tab ? kColorLuxxleSplitViewTileBackgroundVertical
  ...
  if (!is_vertical_tab && active_tab_handle != tile.first &&
    ...
    canvas.DrawLine(separator_top, separator_bottom,
                    cp->GetColor(kColorLuxxleSplitViewTileDivider));
  }

  bounding_rects.Outset(1);
  flags.setStyle(cc::PaintFlags::kStroke_Style);
  flags.setColor(cp->GetColor(kColorLuxxleSplitViewTileBackgroundBorder));
  canvas.DrawRoundRect(bounding_rects, kRadius, flags);
}

void LuxxleTabContainer::OnUnlockLayout() {
...
void LuxxleTabContainer::CompleteAnimationAndLayout() {
  // Note that we check this before checking currently we're in vertical tab
  // strip mode. We might be in the middle of changing orientation.
  if (layout_locked_) {
    return;
...
void LuxxleTabContainer::PaintChildren(const views::PaintInfo& paint_info) {
...
void LuxxleTabContainer::SetTabSlotVisibility() {
  ...
  // in that case.
  // https://github.com/luxxle/luxxle-browser/issues/39298
...
void LuxxleTabContainer::InvalidateIdealBounds() {
  if (tabs::utils::ShouldShowVerticalTabs(tab_slot_controller_->GetBrowser())) {
...
void LuxxleTabContainer::Layout(PassKey) {
  if (tabs::utils::ShouldShowVerticalTabs(tab_slot_controller_->GetBrowser()) &&
      last_layout_size_ == size()) {
    return;
  }
...
std::optional<BrowserRootView::DropIndex> LuxxleTabContainer::GetDropIndex(
...
// LuxxleTabContainer::DropArrow:
// ----------------------------------------------------------
LuxxleTabContainer::DropArrow::DropArrow(const BrowserRootView::DropIndex& index,
                                        Position position,
                                        bool beneath,
...
LuxxleTabContainer::DropArrow::~DropArrow() = default;

void LuxxleTabContainer::DropArrow::SetBeneath(bool beneath) {
  if (beneath_ == beneath) {
    return;
  }
...
void LuxxleTabContainer::DropArrow::SetWindowBounds(const gfx::Rect& bounds) {
  arrow_window_->SetBounds(bounds);
}

void LuxxleTabContainer::HandleDragUpdate(
...
void LuxxleTabContainer::HandleDragExited() {
  if (!tabs::utils::ShouldShowVerticalTabs(
          tab_slot_controller_->GetBrowser())) {
...
void LuxxleTabContainer::OnTileTabs(const TabTile& tile) {
  UpdateTabsBorderInTile(tile);
  SchedulePaint();
}

void LuxxleTabContainer::OnDidBreakTile(const TabTile& tile) {
  UpdateTabsBorderInTile(tile);
  SchedulePaint();
}

void LuxxleTabContainer::OnSwapTabsInTile(const TabTile& tile) {
  UpdateTabsBorderInTile(tile);
}

gfx::Rect LuxxleTabContainer::GetDropBounds(int drop_index,
                                           bool drop_before,
                                           bool drop_in_group,
                                           bool* is_beneath) {
  ...
  const gfx::ImageSkia* drop_image = GetDropArrowImage(
      LuxxleTabContainer::DropArrow::Position::Horizontal, false);
...
gfx::ImageSkia* LuxxleTabContainer::GetDropArrowImage(
    LuxxleTabContainer::DropArrow::Position pos,
    bool beneath) {
  using Position = LuxxleTabContainer::DropArrow::Position;
...
void LuxxleTabContainer::SetDropArrow(
    const std::optional<BrowserRootView::DropIndex>& index) {
  if (!index) {
...
bool LuxxleTabContainer::IsPinnedTabContainer() const {
  return tabs_view_model_.view_size() > 0 &&
         tabs_view_model_.view_at(0)->data().pinned;
}

void LuxxleTabContainer::UpdateTabsBorderInTile(const TabTile& tile) {
  auto* tab_strip_model = tab_slot_controller_->GetBrowser()->tab_strip_model();
  const int offset =
      IsPinnedTabContainer() ? 0 : tab_strip_model->IndexOfFirstNonPinnedTab();
  ...
  // See LuxxleVerticalTabStyle::GetContentsInsets().
  tab1->SetBorder(
...
BEGIN_METADATA(LuxxleTabContainer)
END_METADATA
