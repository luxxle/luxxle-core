...
class LuxxleTabContextMenuContentsTest : public InProcessBrowserTest {
 public:
  LuxxleTabContextMenuContentsTest() = default;
  ~LuxxleTabContextMenuContentsTest() override = default;

 protected:
  std::unique_ptr<LuxxleTabContextMenuContents> CreateMenu() {
    TabStrip* tabstrip =
...
IN_PROC_BROWSER_TEST_F(LuxxleTabContextMenuContentsTest, Basics) {
  auto menu = CreateMenu();

  // All items are disable state when there is only one tab.
  EXPECT_FALSE(menu->IsCommandIdEnabled(LuxxleTabMenuModel::CommandRestoreTab));
  EXPECT_FALSE(
      menu->IsCommandIdEnabled(LuxxleTabMenuModel::CommandBookmarkAllTabs));
  ...
  ASSERT_TRUE(
      ui_test_utils::NavigateToURL(browser(), GURL("luxxle://version/")));
  chrome::CloseTab(browser());
  EXPECT_TRUE(menu->IsCommandIdEnabled(LuxxleTabMenuModel::CommandRestoreTab));
  EXPECT_FALSE(
      menu->IsCommandIdEnabled(LuxxleTabMenuModel::CommandBookmarkAllTabs));
}

IN_PROC_BROWSER_TEST_F(LuxxleTabContextMenuContentsTest,
                       BringAllTabsToThisWindow_VisibleWhenOtherBrowserExists) {
  auto menu = CreateMenu();
  auto is_command_visible = [&]() {
    return menu->IsCommandIdVisible(
        LuxxleTabMenuModel::CommandBringAllTabsToThisWindow);
  };
...
IN_PROC_BROWSER_TEST_F(LuxxleTabContextMenuContentsTest,
  ...
  ASSERT_TRUE(menu->IsCommandIdVisible(
      LuxxleTabMenuModel::CommandBringAllTabsToThisWindow));
  menu->ExecuteCommand(LuxxleTabMenuModel::CommandBringAllTabsToThisWindow,
                       /*event_flags=*/0);
