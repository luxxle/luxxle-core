...
SkPath GetBorderPath(const gfx::Point& origin,
  ...
  // Overridden to use Luxxle's non-circular shape
  SkPath path;
...
// static
gfx::Size LuxxleNewTabButton::GetButtonSize() {
...
gfx::Size LuxxleNewTabButton::CalculatePreferredSize(
    const views::SizeBounds& available_size) const {
  // Overridden so that we use Luxxle's custom button size
...
SkPath LuxxleNewTabButton::GetBorderPath(const gfx::Point& origin,
...
LuxxleNewTabButton::LuxxleNewTabButton(TabStrip* tab_strip,
                                     PressedCallback callback)
    : NewTabButton(tab_strip, std::move(callback)) {}

LuxxleNewTabButton::~LuxxleNewTabButton() = default;

void LuxxleNewTabButton::PaintIcon(gfx::Canvas* canvas) {
...
void LuxxleNewTabButton::PaintFill(gfx::Canvas* canvas) const {
  OnPaintFill(canvas);
}

void LuxxleNewTabButton::OnPaintFill(gfx::Canvas* canvas) const {
  NewTabButton::PaintFill(canvas);
}

gfx::Insets LuxxleNewTabButton::GetInsets() const {
...
BEGIN_METADATA(LuxxleNewTabButton)
END_METADATA
