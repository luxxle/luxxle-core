...
class ContentsView : public views::View {
 public:
  explicit ContentsView(LuxxleCompoundTabContainer* container)
  ...
  void ChildPreferredSizeChanged(views::View* child) override {
    // Bypass ScrollView and notify the LuxxleCompoundTabContainer directly.
    container_->ChildPreferredSizeChanged(child);
  }

  base::raw_ptr<LuxxleCompoundTabContainer> container_;
};
 ...
 public:
  explicit CustomScrollView(PrefService* prefs)
    ...
    should_show_scroll_bar_.Init(
        luxxle_tabs::kVerticalTabsShowScrollbar, prefs,
...
LuxxleCompoundTabContainer::LuxxleCompoundTabContainer(
...
void LuxxleCompoundTabContainer::SetAvailableWidthCallback(
  ...
  if (tabs::utils::ShouldShowVerticalTabs(tab_slot_controller_->GetBrowser()) &&
    ...
    if (base::FeatureList::IsEnabled(
            tabs::features::kLuxxleVerticalTabScrollBar)) {
      unpinned_tab_container_->SetAvailableWidthCallback(base::BindRepeating(
          &LuxxleCompoundTabContainer::GetAvailableWidthConsideringScrollBar,
          base::Unretained(this)));
    } else {
...
base::OnceClosure LuxxleCompoundTabContainer::LockLayout() {
  std::vector<base::OnceClosure> closures;
  for (const auto& tab_container :
       {unpinned_tab_container_, pinned_tab_container_}) {
    closures.push_back(
        static_cast<LuxxleTabContainer*>(base::to_address(tab_container))
            ->LockLayout());
  }
...
void LuxxleCompoundTabContainer::SetScrollEnabled(bool enabled) {
...
void LuxxleCompoundTabContainer::TransferTabBetweenContainers(
...
void LuxxleCompoundTabContainer::Layout(PassKey) {
  if (!ShouldShowVerticalTabs()) {
...
gfx::Size LuxxleCompoundTabContainer::CalculatePreferredSize(
...
gfx::Size LuxxleCompoundTabContainer::GetMinimumSize() const {
  if (!ShouldShowVerticalTabs()) {
...
views::SizeBounds LuxxleCompoundTabContainer::GetAvailableSize(
...
std::vector<Tab*> LuxxleCompoundTabContainer::AddTabs(
...
void LuxxleCompoundTabContainer::MoveTab(int from_model_index,
                                        int to_model_index) {
  CompoundTabContainer::MoveTab(from_model_index, to_model_index);
  UpdatePinnedTabContainerBorder();
}

void LuxxleCompoundTabContainer::RemoveTab(int index, bool was_active) {
  CompoundTabContainer::RemoveTab(index, was_active);
  UpdatePinnedTabContainerBorder();
}

void LuxxleCompoundTabContainer::SetTabPinned(int model_index,
                                             TabPinned pinned) {
  CompoundTabContainer::SetTabPinned(model_index, pinned);
  UpdatePinnedTabContainerBorder();
}

int LuxxleCompoundTabContainer::GetUnpinnedContainerIdealLeadingX() const {
  if (!ShouldShowVerticalTabs()) {
...
LuxxleCompoundTabContainer::GetDropIndex(const ui::DropTargetEvent& event) {
  if (!ShouldShowVerticalTabs()) {
...
BrowserRootView::DropTarget* LuxxleCompoundTabContainer::GetDropTarget(
...
void LuxxleCompoundTabContainer::PaintChildren(const views::PaintInfo& info) {
  if (ShouldShowVerticalTabs()) {
...
void LuxxleCompoundTabContainer::ChildPreferredSizeChanged(views::View* child) {
...
void LuxxleCompoundTabContainer::SetActiveTab(
...
views::View* LuxxleCompoundTabContainer::TargetForRect(views::View* root,
                                                      const gfx::Rect& rect) {
  if (base::FeatureList::IsEnabled(
          tabs::features::kLuxxleVerticalTabScrollBar) &&
      scroll_view_) {
...
TabContainer* LuxxleCompoundTabContainer::GetTabContainerAt(
...
gfx::Rect LuxxleCompoundTabContainer::ConvertUnpinnedContainerIdealBoundsToLocal(
...
bool LuxxleCompoundTabContainer::ShouldShowVerticalTabs() const {
  return tabs::utils::ShouldShowVerticalTabs(
      tab_slot_controller_->GetBrowser());
}

void LuxxleCompoundTabContainer::UpdatePinnedTabContainerBorder() {
  ...
  if (should_have_separator_between_pinned_and_unpinned) {
    pinned_tab_container_->SetBorder(views::CreateSolidSidedBorder(
        gfx::Insets().set_bottom(1), kColorLuxxleVerticalTabSeparator));
  } else {
    pinned_tab_container_->SetBorder(nullptr);
  }
}

void LuxxleCompoundTabContainer::UpdateUnpinnedContainerSize() {
  DCHECK(scroll_view_);
...
void LuxxleCompoundTabContainer::ScrollTabToBeVisible(Tab* tab) {
  CHECK(tab);
  if (!scroll_view_) {
    return;
  }
...
int LuxxleCompoundTabContainer::GetAvailableWidthConsideringScrollBar() {
  CHECK(
      base::FeatureList::IsEnabled(tabs::features::kLuxxleVerticalTabScrollBar));
  if (scroll_view_) {
    auto* scroll_bar = scroll_view_->vertical_scroll_bar();
...
BEGIN_METADATA(LuxxleCompoundTabContainer)
END_METADATA
