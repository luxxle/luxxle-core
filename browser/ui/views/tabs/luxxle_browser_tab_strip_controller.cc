...
LuxxleBrowserTabStripController::LuxxleBrowserTabStripController(
    TabStripModel* model,
...
const std::optional<int> LuxxleBrowserTabStripController::GetModelIndexOf(
    Tab* tab) {
  return tabstrip_->GetModelIndexOf(tab);
}

void LuxxleBrowserTabStripController::ShowContextMenuForTab(
    Tab* tab,
    const gfx::Point& p,
  ...
  context_menu_contents_ =
      std::make_unique<LuxxleTabContextMenuContents>(tab, this, *tab_index);
  context_menu_contents_->RunMenuAt(p, source_type);
}

int LuxxleBrowserTabStripController::GetIndexOfLastFocusedTabInSplit(
    int model_index) {
