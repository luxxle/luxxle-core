...
class LuxxleTabTest : public ChromeViewsTestBase {
 public:
  LuxxleTabTest() = default;
  ~LuxxleTabTest() override = default;

  void LayoutAndCheckBorder(LuxxleTab* tab, const gfx::Rect& bounds) {
    ...
    left_inset += LuxxleTab::kExtraLeftPadding;
    EXPECT_EQ(left_inset, tab->GetInsets().left());
  }
};

TEST_F(LuxxleTabTest, ExtraPaddingLayoutTest) {
  FakeTabSlotController tab_slot_controller;
  LuxxleTab tab(&tab_slot_controller);

  // Our tab should have extra padding always.
  // See the comment at LuxxleTab::GetInsets().
...
TEST_F(LuxxleTabTest, TabHeightTest) {
  FakeTabSlotController tab_slot_controller;
  LuxxleTab tab(&tab_slot_controller);
