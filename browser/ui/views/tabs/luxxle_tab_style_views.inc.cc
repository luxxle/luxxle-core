...
// LuxxleTabStyleViews
//
class LuxxleTabStyleViews : public TabStyleViewsImpl {
 public:
  explicit LuxxleTabStyleViews(Tab* tab);
  LuxxleTabStyleViews(const LuxxleTabStyleViews&) = delete;
  LuxxleTabStyleViews& operator=(const LuxxleTabStyleViews&) = delete;
  ~LuxxleTabStyleViews() override = default;
...
LuxxleTabStyleViews::LuxxleTabStyleViews(Tab* tab)
    : TabStyleViewsImpl(tab), tab_(tab) {}

TabStyle::TabColors LuxxleTabStyleViews::CalculateTargetColors() const {
...
////////////////////////////////////////////////////////////////////////////////
// LuxxleVerticalTabStyle
//
// This class deals with tab styling when vertical tab strip feature flag is
// enabled.
//
class LuxxleVerticalTabStyle : public LuxxleTabStyleViews {
 public:
  explicit LuxxleVerticalTabStyle(Tab* tab);
  LuxxleVerticalTabStyle(const LuxxleVerticalTabStyle&) = delete;
  LuxxleVerticalTabStyle& operator=(const LuxxleVerticalTabStyle&) = delete;
  ~LuxxleVerticalTabStyle() override = default;
...
LuxxleVerticalTabStyle::LuxxleVerticalTabStyle(Tab* tab)
    : LuxxleTabStyleViews(tab) {}

SkPath LuxxleVerticalTabStyle::GetPath(
    TabStyle::PathType path_type,
    float scale,
    bool force_active,
    TabStyle::RenderUnits render_units) const {
  if (!HorizontalTabsUpdateEnabled() && !ShouldShowVerticalTabs()) {
    return LuxxleTabStyleViews::GetPath(path_type, scale, force_active,
                                       render_units);
  }
  ...
  if (!ShouldShowVerticalTabs()) {
    ...
    aligned_bounds.Inset(
        gfx::InsetsF::VH(luxxle_tabs::kHorizontalTabVerticalSpacing * scale,
                         luxxle_tabs::kHorizontalTabInset * scale));
    ...
    if (path_type == TabStyle::PathType::kHitTest) {
      auto hit_test_outsets =
          gfx::OutsetsF::VH(luxxle_tabs::kHorizontalTabVerticalSpacing * scale,
                            luxxle_tabs::kHorizontalTabGap / 2 * scale);
      ...
      if (frame_condensed && tab()->controller()->IsTabFirst(tab())) {
        if (tab()->GetMirrored()) {
          hit_test_outsets.set_right(luxxle_tabs::kHorizontalTabInset * scale);
        } else {
          hit_test_outsets.set_left(luxxle_tabs::kHorizontalTabInset * scale);
        }
      }
  ...
  if (!is_pinned && IsTabTiled(tab()) &&
    ...
    if (ShouldShowVerticalTabs()) {
      ...
      constexpr int kAdditionalVerticalPadding =
          luxxle_tabs::kHorizontalSplitViewTabVerticalSpacing -
          luxxle_tabs::kHorizontalTabGap;
...
gfx::Insets LuxxleVerticalTabStyle::GetContentsInsets() const {
  const bool is_pinned = tab()->data().pinned;
  ...
  return LuxxleTabStyleViews::GetContentsInsets();
}

TabStyle::SeparatorBounds LuxxleVerticalTabStyle::GetSeparatorBounds(
    float scale) const {
  if (!HorizontalTabsUpdateEnabled()) {
    return LuxxleTabStyleViews::GetSeparatorBounds(scale);
  }
...
float LuxxleVerticalTabStyle::GetSeparatorOpacity(bool for_layout,
                                                 bool leading) const {
  if (ShouldShowVerticalTabs()) {
    return 0;
  }
  ...
  if (!HorizontalTabsUpdateEnabled()) {
    return LuxxleTabStyleViews::GetSeparatorOpacity(for_layout, leading);
  }
...
int LuxxleVerticalTabStyle::GetStrokeThickness(
    bool should_paint_as_active) const {
  if (!HorizontalTabsUpdateEnabled() && !ShouldShowVerticalTabs()) {
    return LuxxleTabStyleViews::GetStrokeThickness(should_paint_as_active);
  }
  return 0;
}

void LuxxleVerticalTabStyle::PaintTab(gfx::Canvas* canvas) const {
  if (ShouldShowVerticalTabs()) {
    ...
    LuxxleTabStyleViews::PaintTab(canvas);
  }
  ...
  if (tab()->data().pinned) {
    ...
    if (ShouldShowVerticalTabs()) {
      color_id = kColorLuxxleVerticalTabSeparator;
    }
...
SkColor LuxxleVerticalTabStyle::GetTargetTabBackgroundColor(
    TabStyle::TabSelectionState selection_state,
    bool hovered) const {
  ...
  if (!ShouldShowVerticalTabs()) {
    return LuxxleTabStyleViews::GetTargetTabBackgroundColor(selection_state,
                                                           hovered);
  }

  if (tab()->IsActive()) {
    return cp->GetColor(kColorLuxxleVerticalTabActiveBackground);
  }

  if (hovered) {
    return cp->GetColor(kColorLuxxleVerticalTabHoveredBackground);
  }
  ...
  return cp->GetColor(kColorLuxxleVerticalTabInactiveBackground);
}

bool LuxxleVerticalTabStyle::ShouldShowVerticalTabs() const {
  return tabs::utils::ShouldShowVerticalTabs(tab()->controller()->GetBrowser());
}

bool LuxxleVerticalTabStyle::IsTabTiled(const Tab* tab) const {
  if (!tab) {
    return false;
  }
...
std::unique_ptr<TabStyleViews> TabStyleViews::CreateForTab(Tab* tab) {
  return std::make_unique<LuxxleVerticalTabStyle>(tab);
}
