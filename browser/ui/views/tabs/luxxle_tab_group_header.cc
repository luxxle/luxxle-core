...
LuxxleTabGroupHeader::~LuxxleTabGroupHeader() = default;

void LuxxleTabGroupHeader::AddedToWidget() {
...
void LuxxleTabGroupHeader::VisualsChanged() {
  ...
  if (!ShouldShowVerticalTabs()) {
    title_->SetLineHeight(luxxle_tabs::kTabGroupLineHeight);
  }
...
int LuxxleTabGroupHeader::GetDesiredWidth() const {
  if (!tabs::features::HorizontalTabsUpdateEnabled() ||
      ShouldShowVerticalTabs()) {
    return TabGroupHeader::GetDesiredWidth();
  }
  return luxxle_tabs::kHorizontalTabInset * 2 + title_chip_->width();
}

void LuxxleTabGroupHeader::Layout(PassKey) {
...
bool LuxxleTabGroupHeader::ShouldShowVerticalTabs() const {
  return tabs::utils::ShouldShowVerticalTabs(
      tab_slot_controller_->GetBrowser());
}

void LuxxleTabGroupHeader::LayoutTitleChipForVerticalTabs() {
...
SkColor LuxxleTabGroupHeader::GetGroupColor() const {
...
std::optional<SkColor> LuxxleTabGroupHeader::GetChipBackgroundColor() const {
  if (ShouldShowVerticalTabs()) {
    return {};
  }
...
BEGIN_METADATA(LuxxleTabGroupHeader)
END_METADATA
