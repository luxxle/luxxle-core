...
LuxxleTab::~LuxxleTab() = default;

std::u16string LuxxleTab::GetRenderedTooltipText(const gfx::Point& p) const {
  auto* browser = controller_->GetBrowser();
  if (browser &&
      luxxle_tabs::AreTooltipsEnabled(browser->profile()->GetPrefs())) {
...
int LuxxleTab::GetWidthOfLargestSelectableRegion() const {
...
void LuxxleTab::ActiveStateChanged() {
  Tab::ActiveStateChanged();

  // This should be called whenever the active state changes
  // see comment on UpdateEnabledForMuteToggle();
  // https://github.com/luxxle/luxxle-browser/issues/23476/
  alert_indicator_button_->UpdateEnabledForMuteToggle();
}

std::optional<SkColor> LuxxleTab::GetGroupColor() const {
...
void LuxxleTab::UpdateIconVisibility() {
...
gfx::Insets LuxxleTab::GetInsets() const {
  ...
  // See https://www.github.com/luxxle/luxxle-browser/issues/30469.
  auto insets = Tab::GetInsets();
  insets.set_left(insets.left() + kExtraLeftPadding);
  return insets;
}

void LuxxleTab::MaybeAdjustLeftForPinnedTab(gfx::Rect* bounds,
...
bool LuxxleTab::ShouldRenderAsNormalTab() const {
  if (IsAtMinWidthForVerticalTabStrip()) {
...
bool LuxxleTab::IsAtMinWidthForVerticalTabStrip() const {
  return tabs::utils::ShouldShowVerticalTabs(controller()->GetBrowser()) &&
         width() <= tabs::kVerticalTabMinWidth;
}

void LuxxleTab::SetData(TabRendererData data) {
