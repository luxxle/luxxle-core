...
#include "luxxle/browser/ui/luxxle_tooltips/bounds_util.h"
...
LuxxleTooltipPopup::LuxxleTooltipPopup(std::unique_ptr<LuxxleTooltip> tooltip)
  ...
  GetViewAccessibility().SetRoleDescription(l10n_util::GetStringUTF8(
      IDS_LUXXLE_TOOLTIPS_LUXXLE_TOOLTIP_ACCESSIBLE_NAME));
...
LuxxleTooltipPopup::~LuxxleTooltipPopup() {
...
void LuxxleTooltipPopup::Show() {
  GetWidget()->ShowInactive();

  LuxxleTooltipDelegate* delegate = tooltip_->delegate();
  if (delegate) {
    delegate->OnTooltipShow(tooltip_->id());
  }
}

void LuxxleTooltipPopup::Close() {
  LuxxleTooltipDelegate* delegate = tooltip_->delegate();
  if (delegate) {
...
void LuxxleTooltipPopup::CloseWidget() {
  CloseWidgetView();
}

void LuxxleTooltipPopup::OnOkButtonPressed() {
...
void LuxxleTooltipPopup::OnCancelButtonPressed() {
...
void LuxxleTooltipPopup::set_normalized_display_coordinates(double x, double y) {
  normalized_display_coordinate_x_ = x;
  normalized_display_coordinate_y_ = y;
}

void LuxxleTooltipPopup::set_display_work_area_insets(int x, int y) {
  display_work_area_inset_x_ = x;
  display_work_area_inset_y_ = y;
}

void LuxxleTooltipPopup::OnDisplaysRemoved(
    const display::Displays& old_displays) {
  // Called when |old_display| has been removed
  RecomputeAlignment();
}

void LuxxleTooltipPopup::OnDisplayMetricsChanged(const display::Display& display,
                                                uint32_t changed_metrics) {
  // Called when the metrics of a display change
  RecomputeAlignment();
}

void LuxxleTooltipPopup::OnDisplayChanged() {
  // Called with the display changes (color depth or resolution)
  RecomputeAlignment();
}

void LuxxleTooltipPopup::OnWorkAreaChanged() {
  // Called when the work area (the desktop area minus task bars, menu bars,
  // etc.) changes in size
  RecomputeAlignment();
}

void LuxxleTooltipPopup::OnPaintBackground(gfx::Canvas* canvas) {
  DCHECK(canvas);
...
void LuxxleTooltipPopup::OnThemeChanged() {
  views::View::OnThemeChanged();

  SchedulePaint();
}

void LuxxleTooltipPopup::OnWidgetCreated(views::Widget* widget) {
  DCHECK(widget);
...
void LuxxleTooltipPopup::OnWidgetDestroyed(views::Widget* widget) {
  DCHECK(widget);
...
void LuxxleTooltipPopup::OnWidgetBoundsChanged(views::Widget* widget,
...
void LuxxleTooltipPopup::CreatePopup() {
  ...
  tooltip_view_ =
      AddChildView(new LuxxleTooltipView(this, tooltip_->attributes()));

  CreateWidgetView();
}

gfx::Point LuxxleTooltipPopup::GetDefaultOriginForSize(const gfx::Size& size) {
...
gfx::Rect LuxxleTooltipPopup::CalculateBounds(bool use_default_origin) {
  DCHECK(tooltip_view_);
...
void LuxxleTooltipPopup::RecomputeAlignment() {
  if (!IsWidgetValid()) {
    return;
  }
...
const gfx::ShadowDetails& LuxxleTooltipPopup::GetShadowDetails() const {
  return gfx::ShadowDetails::Get(kShadowElevation, kCornerRadius);
}

gfx::Insets LuxxleTooltipPopup::GetShadowMargin() const {
  const gfx::ShadowDetails& shadow_details = GetShadowDetails();
...
void LuxxleTooltipPopup::CreateWidgetView() {
...
void LuxxleTooltipPopup::CloseWidgetView() {
  if (!GetWidget()) {
    DeleteDelegate();
    return;
  }
...
bool LuxxleTooltipPopup::IsWidgetValid() const {
  return GetWidget() && !GetWidget()->IsClosed();
}

BEGIN_METADATA(LuxxleTooltipPopup)
END_METADATA
