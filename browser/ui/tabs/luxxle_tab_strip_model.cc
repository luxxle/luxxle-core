...
LuxxleTabStripModel::LuxxleTabStripModel(
    TabStripModelDelegate* delegate,
    Profile* profile,
    TabGroupModelFactory* group_model_factory)
    : TabStripModel(delegate, profile, group_model_factory) {}
LuxxleTabStripModel::~LuxxleTabStripModel() = default;

void LuxxleTabStripModel::SelectRelativeTab(TabRelativeDirection direction,
...
void LuxxleTabStripModel::SelectMRUTab(TabRelativeDirection direction,
                                      TabStripUserGestureDetails detail) {
  if (mru_cycle_list_.empty()) {
    ...
    static_cast<LuxxleBrowserWindow*>(browser->window())->StartTabCycling();
  }
...
void LuxxleTabStripModel::StopMRUCycling() {
  mru_cycle_list_.clear();
}

std::vector<int> LuxxleTabStripModel::GetTabIndicesForCommandAt(int tab_index) {
  return TabStripModel::GetIndicesForCommand(tab_index);
}

void LuxxleTabStripModel::CloseTabs(base::span<int> indices,
