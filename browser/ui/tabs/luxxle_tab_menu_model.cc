...
LuxxleTabMenuModel::LuxxleTabMenuModel(
  ...
  auto indices = static_cast<LuxxleTabStripModel*>(tab_strip_model)
...
LuxxleTabMenuModel::~LuxxleTabMenuModel() = default;

int LuxxleTabMenuModel::GetRestoreTabCommandStringId() const {
...
std::u16string LuxxleTabMenuModel::GetLabelAt(size_t index) const {
  if (!is_vertical_tab_) {
...
void LuxxleTabMenuModel::Build(Browser* browser,
                              TabStripModel* tab_strip_model,
...
void LuxxleTabMenuModel::BuildItemsForSplitView(
    Browser* browser,
    TabStripModel* tab_strip_model,
    const std::vector<int>& indices) {
