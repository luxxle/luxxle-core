...
public class LuxxleSearchBannerViewBinder {
    public static void bind(PropertyModel model, View contentView, PropertyKey propertyKey) {
        ...
        if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
            ...
            textViewParams.topMargin = contentView.getResources().getDimensionPixelSize(
                    R.dimen.omnibox_luxxle_search_banner_text_desc_top_margin);
            textViewParams.rightMargin = contentView.getResources().getDimensionPixelSize(
                    R.dimen.omnibox_luxxle_search_banner_text_desc_right_margin);
        ...
        boolean isMaybeLaterPressed =
                OmniboxPrefManager.getInstance().isLuxxleSearchPromoBannerMaybeLater();
        ...
        if (propertyKey == LuxxleSearchBannerProperties.DELEGATE) {
            LuxxleSearchBannerProperties.Delegate delegate =
                    model.get(LuxxleSearchBannerProperties.DELEGATE);
            if (delegate != null) {
                btnPositive.setOnClickListener(view -> { delegate.onPositiveClicked(); });

                btnNegative.setOnClickListener(view -> {
                    if (!isMaybeLaterPressed) {
                        OmniboxPrefManager.getInstance().setLuxxleSearchPromoBannerMaybeLater();
                    } else {
                        OmniboxPrefManager.getInstance().setLuxxleSearchPromoBannerDismissed();
                    }
