...
class LuxxleExternalProcessImporterHostUnitTest : public testing::Test {
 public:
  LuxxleExternalProcessImporterHostUnitTest() = default;

  void SetUp() override {
    TestingProfile::Builder profile_builder;
    EXPECT_TRUE(luxxle_profile_dir_.CreateUniqueTempDir());
    profile_builder.SetPath(GetProductProfilePath("Luxxle"));
  ...
  base::FilePath GetProductProfilePath(const std::string& product) {
    return luxxle_profile_dir_.GetPath()
  ...
  void LaunchExtensionsImportAndWait(importer::SourceProfile source_profile) {
    ...
    // LuxxleExternalProcessImporterHost uses `delete this`.
    auto* external_process_host = new LuxxleExternalProcessImporterHost();
  ...
  std::string ReadTargetStore(const std::string& id) {
    return ReadStore(GetExtensionLocalSettingsPath("Luxxle", id), id);
  }

  std::string ReadTargetIndexedDB(const std::string& id,
                                  const std::string& type) {
    base::ScopedAllowBlockingForTesting allow_io;
    const auto base_path = GetProductProfilePath("Luxxle")
...
TEST_F(LuxxleExternalProcessImporterHostUnitTest, ImportEtensionsSettings) {
