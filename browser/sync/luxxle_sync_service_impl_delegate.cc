...
LuxxleSyncServiceImplDelegate::LuxxleSyncServiceImplDelegate(
...
LuxxleSyncServiceImplDelegate::~LuxxleSyncServiceImplDelegate() = default;

void LuxxleSyncServiceImplDelegate::OnDeviceInfoChange() {
  ...
  if (!found_local_device) {
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE,
        base::BindOnce(&LuxxleSyncServiceImplDelegate::OnSelfDeviceInfoDeleted,
                       weak_ptr_factory_.GetWeakPtr()));
  }
}

void LuxxleSyncServiceImplDelegate::OnSelfDeviceInfoDeleted() {
  sync_service_impl_->OnSelfDeviceInfoDeleted(base::DoNothing());
}

void LuxxleSyncServiceImplDelegate::SuspendDeviceObserverForOwnReset() {
  device_info_observer_.Reset();
}

void LuxxleSyncServiceImplDelegate::ResumeDeviceObserver() {
  if (!device_info_observer_.IsObserving()) {
    device_info_observer_.Observe(device_info_tracker_);
  }
}

void LuxxleSyncServiceImplDelegate::RecordP3ASyncStatus() {
  ...
  base::UmaHistogramExactLinear("Luxxle.Sync.Status.2", p3a_value, 3);
}

void LuxxleSyncServiceImplDelegate::SetLocalDeviceAppearedCallback(
...
void LuxxleSyncServiceImplDelegate::GetKnownToSyncHistoryCount(
