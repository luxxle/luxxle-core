...
void RegisterGCMProfilePrefs(user_prefs::PrefRegistrySyncable* registry) {
  ...
  // Current Luxxle equivalent of the deprecated pref.
  registry->RegisterBooleanPref(kLuxxleGCMChannelStatus, false);
}
...
void MigrateGCMPrefs(PrefService* prefs) {
  ...
  if (pref && !pref->IsDefaultValue())
    prefs->SetBoolean(kLuxxleGCMChannelStatus, pref->GetValue()->GetBool());
