...
#include "luxxle/components/ai_chat/renderer/ai_chat_resource_sniffer.h"
#include "luxxle/components/ai_chat/renderer/page_content_extractor.h"
#include "luxxle/components/body_sniffer/body_sniffer_throttle.h"
...
#include "luxxle/components/tor/renderer/onion_domain_throttle.h"
#endif

namespace {

std::unique_ptr<blink::URLLoaderThrottle> MaybeCreateOnionDomainThrottle(
    LuxxleContentRendererClient* luxxle_content_renderer_client) {
#if BUILDFLAG(ENABLE_TOR)
  return tor::OnionDomainThrottle::MaybeCreateThrottle(
      luxxle_content_renderer_client->IsOnionAllowed());
#else
  return nullptr;
#endif
}
...
#define IsRequestDestinationFrame                                         \
  ...
  if (auto onion_domain_throttle = MaybeCreateOnionDomainThrottle(        \
          static_cast<LuxxleContentRendererClient*>(                       \
