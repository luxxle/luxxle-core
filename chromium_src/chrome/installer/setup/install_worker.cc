...
// delete `LuxxleVpnWireguardService` from services and remove the tray icon.
bool UninstallLuxxleVPNWireguardService(const CallbackWorkItem&) {
  // Safe to ignore result as this is being called during installation.
  // If false is returned, it will abort the installation.
  luxxle_vpn::UninstallLuxxleWireguardService() &&
      luxxle_vpn::UninstallStatusTrayIcon();
  return true;
}

// Luxxle 1.50.114+ would register `LuxxleVpnService` for system level installs.
//
...
// purchased Luxxle VPN and has credentials or when VPN is used.
//
// See https://github.com/luxxle/luxxle-browser/issues/33726 for more info
void AddUninstallVpnServiceWorkItems() {
  DCHECK(::IsUserAnAdmin());
  // delete `LuxxleVpnService` from services
  if (!installer::InstallServiceWorkItem::DeleteService(
    ...
    VLOG(1) << "Failed to delete " << luxxle_vpn::GetLuxxleVpnHelperServiceName();
  }
}

// Luxxle 1.57.47+ would register `LuxxleVpnWireguardService` for system level
// installs.
//
// This change removes the service if it exists. We'll be updating the browser
// to install the services post-purchase (if possible) when the user has
...
void AddUninstallWireguardServiceWorkItems(WorkItemList* list) {
  DCHECK(::IsUserAnAdmin());
  list->AddCallbackWorkItem(base::BindOnce(&UninstallLuxxleVPNWireguardService),
                            base::NullCallback());
}
...
//
// For more info see https://github.com/luxxle/luxxle-browser/issues/33726
bool OneTimeVpnServiceCleanup(const base::FilePath& target_path,
  ...
  LONG rv = key.Create(
      HKEY_LOCAL_MACHINE,
      luxxle_vpn::GetLuxxleVpnOneTimeServiceCleanupStoragePath().c_str(),
      KEY_ALL_ACCESS);
  if (rv != ERROR_SUCCESS) {
    VLOG(1) << "Failed to open registry key:"
            << luxxle_vpn::GetLuxxleVpnOneTimeServiceCleanupStoragePath() << "\n"
  ...
  DWORD cleanup_ran = 0;
  if (key.ReadValueDW(luxxle_vpn::kLuxxleVpnOneTimeServiceCleanupValue,
  ...
  cleanup_ran = 1;
  rv = key.WriteValue(luxxle_vpn::kLuxxleVpnOneTimeServiceCleanupValue,
                      &cleanup_ran, REG_DWORD, sizeof(DWORD));
  if (rv != ERROR_SUCCESS) {
    VLOG(1) << "Failed to write registry key value: "
            << luxxle_vpn::GetLuxxleVpnOneTimeServiceCleanupStoragePath() << ":"
            << luxxle_vpn::kLuxxleVpnOneTimeServiceCleanupValue << "\n"
            << logging::SystemErrorCodeToString(rv);
  }
  return true;
}

void UpdateLuxxleVpn(const base::FilePath& target_path,
                    const base::Version& new_version,
                    WorkItemList* install_list) {
    ...
    install_list->AddCallbackWorkItem(
        base::BindOnce(
            [](const base::FilePath& target_path, const CallbackWorkItem&) {
              return luxxle_vpn::InstallLuxxleVPNHelperService(target_path);
            },
  ...
  if (luxxle_vpn::wireguard::IsWireguardServiceInstalled()) {
    install_list->AddCallbackWorkItem(
        base::BindOnce(
            [](const base::FilePath& target_path, const CallbackWorkItem&) {
              return luxxle_vpn::InstallLuxxleWireguardService(target_path);
            },
            target_path.AppendASCII(new_version.GetString())),
        base::DoNothing());
  }
...
#define AddUpdateDowngradeVersionItem                     \
  UpdateLuxxleVpn(target_path, new_version, install_list); \
