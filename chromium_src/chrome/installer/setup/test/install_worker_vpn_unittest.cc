...
class LuxxleSetupInstallWorkerTest : public testing::Test {
 public:
  LuxxleSetupInstallWorkerTest()
...
// chromium_src/chrome/install_static/luxxle_user_data_dir_win_unittest.cc
...
TEST_F(LuxxleSetupInstallWorkerTest, CleanupAlreadyRan) {
  ...
  base::win::RegKey key(
      ...
      KEY_ALL_ACCESS);
  DWORD cleanup_ran = 1;
  key.WriteValue(luxxle_vpn::kLuxxleVpnOneTimeServiceCleanupValue, cleanup_ran);
...
TEST_F(LuxxleSetupInstallWorkerTest, CleanupNotRanYetNoKey) {
  registry_util::RegistryOverrideManager override_manager;
  std::wstring temp;
  ASSERT_NO_FATAL_FAILURE(
      override_manager.OverrideRegistry(HKEY_LOCAL_MACHINE, &temp));
  ...
  base::win::RegKey key(
      ...
      KEY_ALL_ACCESS);
  DWORD cleanup_ran = 0;
  LONG rv = key.ReadValueDW(luxxle_vpn::kLuxxleVpnOneTimeServiceCleanupValue,
                            &cleanup_ran);
...
TEST_F(LuxxleSetupInstallWorkerTest, CleanupNotRanKeyExists) {
  registry_util::RegistryOverrideManager override_manager;
  std::wstring temp;
  ASSERT_NO_FATAL_FAILURE(
      override_manager.OverrideRegistry(HKEY_LOCAL_MACHINE, &temp));
