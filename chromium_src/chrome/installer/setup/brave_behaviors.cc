...
// changes:
//
// - `distribution_data` not appended as Luxxle does not record histograms.
// - `kLuxxleUninstallSurveyUrl` used instead of `kUninstallSurveyUrl`

constexpr std::wstring_view kLuxxleUninstallSurveyUrl(
    L"https://luxxle.com/uninstall-survey/?p=luxxle_uninstall_survey");
...
void DoPostUninstallOperations(const base::Version& version,
  ...
  const std::wstring survey_url = std::wstring(kLuxxleUninstallSurveyUrl);
#if DCHECK_IS_ON()
