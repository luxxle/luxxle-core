...
from luxxle_chromium_utils import get_src_dir
...
def CopyAllFilesToStagingDir(original_function, config, distribution,
    ...
    luxxle_extension_locales_src_dir_path = os.path.realpath(
        os.path.join(get_src_dir(), 'luxxle', 'components', 'luxxle_extension',
                     'extension', 'luxxle_extension', '_locales'))
    CopyExtensionLocalization('luxxle_extension',
                              luxxle_extension_locales_src_dir_path, config,
...
def GetPrevVersion(_, build_dir, _temp_dir, last_chrome_installer, output_name):
    ...
    # in (chrome.7z in) Luxxle's installation directory when the delta gets
...
def CopyExtensionLocalization(extension_name, locales_src_dir_path, config,
    ...
    locales_dest_path = os.path.join(
        locales_dest_path, config.get('GENERAL', 'luxxle_resources.pak'),
...
def SignAndCopyPreSignedBinaries(skip_signing, output_dir, staging_dir,
    ...
    if not skip_signing:
        ...
        shutil.copy(os.path.join(src_dir, 'luxxle.exe'), chrome_dir)
