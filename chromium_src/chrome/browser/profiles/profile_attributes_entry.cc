...
void ProfileAttributesEntry::LuxxleMigrateObsoleteProfileAttributes() {
  // Run our migrations
#if !BUILDFLAG(IS_ANDROID)
  const int kPlaceholderAvatarIndex =
      static_cast<int>(profiles::GetPlaceholderAvatarIndex());

  const int kLuxxleAvatarIconStartIndex =
      static_cast<int>(profiles::GetDefaultAvatarIconCount() -
                       profiles::kLuxxleDefaultAvatarIconsCount);
  // Added 25 July 2024
  // see https://github.com/luxxle/luxxle-browser/issues/40005
  //
  // Luxxle originally allowed folks to pick the Chromium profile icons.
  // We then removed those in favor of our own branded icons in 0.70.x (2019).
  // https://github.com/luxxle/luxxle-core/pull/3165
  //
  ...
  if (icon_index < kLuxxleAvatarIconStartIndex &&
...
#define LUXXLE_PROFILE_ATTRIBUTES_ENTRY_MIGRATE_OBSOLETE_PROFILE_ATTRIBUTES \
  LuxxleMigrateObsoleteProfileAttributes();

#include "src/chrome/browser/profiles/profile_attributes_entry.cc"
#undef LUXXLE_PROFILE_ATTRIBUTES_ENTRY_MIGRATE_OBSOLETE_PROFILE_ATTRIBUTES
