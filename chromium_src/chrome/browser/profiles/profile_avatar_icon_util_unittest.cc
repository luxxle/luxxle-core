...
TEST(ProfileUtilTest, LuxxleAvatarIconChoices) {
  // Test that the avatar icon choices presented to the user are luxxle's.
...
TEST(ProfileUtilTest, RandomIconNeverFirstIcon) {
  ...
  // the placeholder index (which for luxxle is always the first item).
...
TEST_F(ProfileAvatarSelectorTest, ProfileAvatarSelectorPlaceholder) {
  ...
  const std::string expected_label =
      ui::ResourceBundle::GetSharedInstance().LoadLocalizedResourceString(
          IDS_LUXXLE_AVATAR_LABEL_PLACEHOLDER);
