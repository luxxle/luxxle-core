...
TEST_F(ChromeMetricsServiceClientTest, LuxxleTestRegisterUKMProviders) {
  ...
  size_t observed_count = chrome_metrics_service_client->GetUkmService()
                              ...
                              .size();
  // In Luxxle, we expect 0 UKM providers regardless of feature flag
  EXPECT_EQ(0ul, observed_count);
}

TEST_F(ChromeMetricsServiceClientTest, LuxxleRegisterMetricsServiceProviders) {
  ...
  // In Luxxle, we expect only 2 metrics providers (the ones added in the
