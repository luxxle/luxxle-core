...
bool IsLuxxleVPNConnected(PrefService* local_state) {
  if (!base::FeatureList::IsEnabled(
          luxxle_vpn::features::kLuxxleVPNDnsProtection)) {
    return false;
  }
  return !local_state->GetString(prefs::kLuxxleVpnDnsConfig).empty();
}
...
bool ShouldOverride(net::SecureDnsMode secure_dns_mode,
                    ...
                    bool is_managed) {
  if (!IsLuxxleVPNConnected(local_state))
    return false;
...
net::DnsOverHttpsConfig MaybeOverrideDnsConfig(
  ...
  if (ShouldOverride(secure_dns_mode, local_state, management_mode,
    ...
    return net::DnsOverHttpsConfig::FromStringLax(
        local_state->GetString(prefs::kLuxxleVpnDnsConfig));
  }
  return doh_config;
}
