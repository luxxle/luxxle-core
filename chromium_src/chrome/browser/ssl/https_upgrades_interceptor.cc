...
#define MaybeCreateLoader(...)                                                \
  MaybeCreateLoader(__VA_ARGS__) {                                            \
    if (luxxle_shields::IsHttpsByDefaultFeatureEnabled()) {                    \
      ...
      if (!map ||                                                             \
          !luxxle_shields::ShouldUpgradeToHttps(                               \
              map, tentative_resource_request.url,                            \
              g_luxxle_browser_process->https_upgrade_exceptions_service())) { \
        std::move(callback).Run({});                                          \
        return;                                                               \
      }                                                                       \
      http_interstitial_enabled_by_pref_ = luxxle_shields::ShouldForceHttps(   \
          map, tentative_resource_request.url);                               \
    }                                                                         \
