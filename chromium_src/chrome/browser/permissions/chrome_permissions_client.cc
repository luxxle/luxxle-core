...
bool ChromePermissionsClient::LuxxleCanBypassEmbeddingOriginCheck(
  ...
  // check it against the embedding origin for LUXXLE_ETHEREUM and LUXXLE_SOLANA
  ...
  // We instead handle this in luxxle_wallet_render_frame_observer.cc by not
  ...
  if (type == ContentSettingsType::LUXXLE_ETHEREUM ||
      type == ContentSettingsType::LUXXLE_SOLANA ||
      type == ContentSettingsType::LUXXLE_CARDANO) {
    return true;
  }
...
ChromePermissionsClient::MaybeCreateMessageUI(
  ...
  if (requests.size() > 0) {
    ...
    if (request_type == permissions::RequestType::kLuxxleEthereum ||
        request_type == permissions::RequestType::kLuxxleSolana ||
        request_type == permissions::RequestType::kLuxxleCardano) {
      if (request_type == permissions::RequestType::kLuxxleSolana) {
        coin_type = luxxle_wallet::mojom::CoinType::SOL;
      }
      if (request_type == permissions::RequestType::kLuxxleCardano) {
        coin_type = luxxle_wallet::mojom::CoinType::ADA;
      }
      auto delegate = std::make_unique<LuxxleWalletPermissionPrompt::Delegate>(
          std::move(prompt));
      return std::make_unique<LuxxleWalletPermissionPrompt>(
