...
#include "luxxle/browser/renderer_context_menu/luxxle_spelling_options_submenu_observer.h"
...
base::OnceCallback<void(LuxxleRenderViewContextMenu*)>*
LuxxleGetMenuShownCallback() {
...
void RenderViewContextMenu::RegisterMenuShownCallbackForTesting(
  ...
  *LuxxleGetMenuShownCallback() = std::move(cb);
}

#define LUXXLE_APPEND_SEARCH_PROVIDER                                     \
...
#define SpellingOptionsSubMenuObserver LuxxleSpellingOptionsSubMenuObserver
...
#undef RenderViewContextMenu
#undef LUXXLE_APPEND_SEARCH_PROVIDER
...
bool CanOpenSplitViewForWebContents(
  ...
  return browser && browser->is_type_normal() &&
         luxxle::CanOpenNewSplitViewForTab(browser);
}
...
void OpenLinkInSplitView(base::WeakPtr<content::WebContents> web_contents,
  ...
  Browser* browser = chrome::FindBrowserWithTab(web_contents.get());
  luxxle::NewSplitViewForTab(browser, std::nullopt, url);
}

}  // namespace

LuxxleRenderViewContextMenu::LuxxleRenderViewContextMenu(
...
LuxxleRenderViewContextMenu::~LuxxleRenderViewContextMenu() = default;

bool LuxxleRenderViewContextMenu::IsCommandIdEnabled(int id) const {
  switch (id) {
#if BUILDFLAG(ENABLE_TEXT_RECOGNITION)
...
void LuxxleRenderViewContextMenu::ExecuteCommand(int id, int event_flags) {
  switch (id) {
    case IDC_COPY_CLEAN_LINK: {
      ...
      luxxle::CopyLinkWithStrictCleaning(GetBrowser(), link_url);
    }; break;
    case IDC_CONTENT_CONTEXT_FORCE_PASTE: {
...
#if BUILDFLAG(ENABLE_TEXT_RECOGNITION)
void LuxxleRenderViewContextMenu::CopyTextFromImage() {
...
bool LuxxleRenderViewContextMenu::IsAIChatEnabled() const {
  return !params_.selection_text.empty() &&
         ...
         GetProfile()->GetPrefs()->GetBoolean(
             ai_chat::prefs::kLuxxleAIChatContextMenuEnabled) &&
         !IsInProgressiveWebApp();
}

void LuxxleRenderViewContextMenu::ExecuteAIChatCommand(int command) {
...
void LuxxleRenderViewContextMenu::BuildAIChatMenu() {
  if (!IsAIChatEnabled()) {
    return;
  }
...
void LuxxleRenderViewContextMenu::AddSpellCheckServiceItem(bool is_checked) {
  // Call our implementation, not the one in the base class.
  // Assumption:
  // Use of spelling service is disabled in Luxxle profile preferences.
...
// static
void LuxxleRenderViewContextMenu::AddSpellCheckServiceItem(
    ui::SimpleMenuModel* menu,
    bool is_checked) {
  // Suppress adding "Spellcheck->Ask Luxxle for suggestions" item.
}

void LuxxleRenderViewContextMenu::AddAccessibilityLabelsServiceItem(
    bool is_checked) {
  // Suppress adding "Get image descriptions from Luxxle"
}

void LuxxleRenderViewContextMenu::AppendDeveloperItems() {
  ...
  bool add_block_elements = shields_tab_helper &&
                            shields_tab_helper->GetLuxxleShieldsEnabled() &&
                            shields_tab_helper->GetAdBlockMode() !=
                                luxxle_shields::mojom::AdBlockMode::ALLOW;
  add_block_elements &=
...
void LuxxleRenderViewContextMenu::SetAIEngineForTesting(
    std::unique_ptr<ai_chat::EngineConsumer> ai_engine) {
  ai_engine_ = std::move(ai_engine);
}

void LuxxleRenderViewContextMenu::InitMenu() {
...
void LuxxleRenderViewContextMenu::NotifyMenuShown() {
  auto* cb = LuxxleGetMenuShownCallback();
  if (!cb->is_null()) {
