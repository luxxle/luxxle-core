...
#include "luxxle/components/luxxle_private_cdn/private_cdn_helper.h"
...
void SanitizedImageSource::OnImageLoaded(
  ...
  if (pcdn_domain_.empty()) {
    pcdn_domain_ = luxxle_domains::GetServicesDomain("pcdn");
  }
  ...
  if (loader->NetError() == net::OK && body &&
    ...
    if (!luxxle::private_cdn::RemovePadding(&body_payload)) {
