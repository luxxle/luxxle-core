...
// Luxxle Shields Metadata is also cleared.
TEST_F(SiteSettingsHandlerTest, ClearLuxxleShieldMetadata) {
  const GURL urls[] = {
  ...
  for (const auto& url : urls) {
    host_content_settings_map->SetWebsiteSettingDefaultScope(
        url, url, ContentSettingsType::LUXXLE_SHIELDS_METADATA,
  ...
  EXPECT_EQ(2U, host_content_settings_map
                    ->GetSettingsForOneType(
                        ContentSettingsType::LUXXLE_SHIELDS_METADATA)
                    .size());
