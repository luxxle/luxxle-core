...
ui::SimpleMenuModel* DownloadShelfContextMenuView::GetMenuModel() {
  ...
  if (auto index =
    ...
    model->InsertItemAt(
        *index + 1, static_cast<int>(LuxxleDownloadCommands::REMOVE_FROM_LIST),
...
bool DownloadShelfContextMenuView::IsCommandIdEnabled(int command_id) const {
  if (static_cast<LuxxleDownloadCommands>(command_id) ==
      LuxxleDownloadCommands::REMOVE_FROM_LIST) {
    return true;
  }
