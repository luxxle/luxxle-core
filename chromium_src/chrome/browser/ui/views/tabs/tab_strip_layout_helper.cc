...
#define CalculateTabBounds                                                     \
  FillTiledState(tab_widths, static_cast<LuxxleTabStrip*>(tab_strip_.get())) && \
          use_vertical_tabs_&& FillGroupInfo(tab_widths)                       \
      ? tabs::CalculateVerticalTabBounds(                                      \
            tab_widths, available_width,                                       \
            GetLuxxleTabStrip() -> IsVerticalTabsFloating())                    \
      : CalculateTabBounds
...
LuxxleTabStrip* TabStripLayoutHelper::GetLuxxleTabStrip() const {
  return static_cast<LuxxleTabStrip*>(tab_strip_.get());
}
...
bool TabStripLayoutHelper::FillTiledState(
  ...
  for (int i = 0; i < static_cast<int>(slots_.size()); i++) {
    auto index = GetLuxxleTabStrip()->GetModelIndexOf(slots_.at(i).view);
    if (!index) {
      continue;
    }
