...
#include "luxxle/browser/luxxle_wallet/luxxle_wallet_tab_helper.h"
...
bool IsLuxxlePanel(content::WebContents* content) {
  return content->GetVisibleURL().EqualsIgnoringRef(
      GURL(kLuxxleUIWalletPanelURL));
}
...
void OnWindowClosing(views::Widget* anchor_widget) {
  ...
  auto* tab_helper =
      luxxle_wallet::LuxxleWalletTabHelper::FromWebContents(active);
  if (tab_helper) {
...
namespace views {
class LuxxleBubbleDialogDelegateView : public views::BubbleDialogDelegateView {
 public:
  LuxxleBubbleDialogDelegateView() : BubbleDialogDelegateView() {}
...
Browser* FindBrowserAndAdjustBubbleForLuxxleWalletPanel(
    content::WebContents* contents) {
  if (!IsLuxxlePanel(contents))
...
#define FindBrowserWithTab FindBrowserAndAdjustBubbleForLuxxleWalletPanel
#define GetActiveWebContents                           \
  GetActiveWebContents() && !IsLuxxlePanel(contents) && \
      browser->tab_strip_model()->GetActiveWebContents

#define BubbleDialogDelegateView LuxxleBubbleDialogDelegateView
