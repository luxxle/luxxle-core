...
void UpdateLuxxleScheme(NavigateParams* params) {
  if (params->url.SchemeIs(content::kLuxxleUIScheme)) {
...
bool IsURLAllowedInIncognitoLuxxleImpl(const GURL& url) {
  ...
  if (host == kRewardsPageHost || host == chrome::kChromeUISyncInternalsHost ||
      host == chrome::kLuxxleUISyncHost || host == kAdblockHost ||
      host == kWelcomeHost || host == kLuxxleGettingStartedHost) {
    return false;
  }
...
#define LUXXLE_IS_URL_ALLOWED_IN_INCOGNITO     \
  if (!IsURLAllowedInIncognitoLuxxleImpl(url)) \
    return false;
#define LUXXLE_ADJUST_NAVIGATE_PARAMS_FOR_URL UpdateLuxxleScheme(params);
#include "src/chrome/browser/ui/browser_navigator.cc"
#undef LUXXLE_ADJUST_NAVIGATE_PARAMS_FOR_URL
#undef LUXXLE_IS_URL_ALLOWED_IN_INCOGNITO
