...
class LuxxleStartupBrowserCreatorImpl final : public StartupBrowserCreatorImpl {
 public:
  LuxxleStartupBrowserCreatorImpl(const base::FilePath& cur_dir,
...
LuxxleStartupBrowserCreatorImpl::LuxxleStartupBrowserCreatorImpl(
...
// won't be launched.
void LuxxleStartupBrowserCreatorImpl::Launch(
    Profile* profile,
    chrome::startup::IsProcessStartup process_startup,
    bool restore_tabbed_browser) {
#if BUILDFLAG(ENABLE_TOR)
  if (StartupBrowserCreatorImpl::command_line_->HasSwitch(switches::kTor)) {
    ...
    // one runs luxxle-browser --tor "? search query" the search query is not
...
#define StartupBrowserCreatorImpl LuxxleStartupBrowserCreatorImpl
