...
import { RegisterPolymerTemplateModifications } from '//resources/luxxle/polymer_overriding.js'
...
RegisterPolymerTemplateModifications({
  'site-favicon': (templateContent) => {
    ...
    if (!downloadedFavicon) {
      throw new Error(
        `[Luxxle Password Manager Overrides] Could not find '#downloadedFavicon'`
      )
    } else {
