...
// the version of Luxxle engines as well: kCurrentDataVersion is defined in
...
int GetDataVersion(PrefService* prefs) {
  ...
  return (data_version + kLuxxleCurrentDataVersion);
}

// Chromium picks Google (if on the list, otherwise the first prepopulated on
// the list). We should return the default engine by country, or Luxxle.
std::unique_ptr<TemplateURLData> GetPrepopulatedFallbackSearch(
    LuxxlePrepopulatedEngineID default_engine_id,
    PrefService& prefs,
    std::vector<const TemplateURLPrepopulateData::PrepopulatedEngine*>
        regional_prepopulated_engines) {
  LOG(ERROR) << "LUXXLE DEBUG: GetPrepopulatedFallbackSearch called with engine ID: " << static_cast<int>(default_engine_id);
  ...
  std::unique_ptr<TemplateURLData> luxxle_engine;
  for (auto& engine : prepopulated_engines) {
    LOG(ERROR) << "LUXXLE DEBUG: Checking engine with ID: " << engine->prepopulate_id << " name: " << engine->short_name();
    if (engine->prepopulate_id == static_cast<int>(default_engine_id)) {
      LOG(ERROR) << "LUXXLE DEBUG: Found requested engine ID " << static_cast<int>(default_engine_id) << ", returning it";
      return std::move(engine);
    } else if (engine->prepopulate_id ==
               TemplateURLPrepopulateData::PREPOPULATED_ENGINE_ID_LUXXLE) {
      LOG(ERROR) << "LUXXLE DEBUG: Found Luxxle engine, saving as backup";
      luxxle_engine = std::move(engine);
    }
  }

  // Default engine wasn't found, then return Luxxle, if found.
  if (luxxle_engine) {
    LOG(ERROR) << "LUXXLE DEBUG: Returning Luxxle engine as fallback";
    return luxxle_engine;
  }

  // If all else fails, return the first engine on the list.
  LOG(ERROR) << "LUXXLE DEBUG: Returning first engine in list as last resort";
