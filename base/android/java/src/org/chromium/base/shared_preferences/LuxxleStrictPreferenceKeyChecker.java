...
public class LuxxleStrictPreferenceKeyChecker extends StrictPreferenceKeyChecker {
    LuxxleStrictPreferenceKeyChecker(PreferenceKeyRegistry registry) {
        super(registry);
    }

    @Override
    public void checkIsKeyInUse(String key) {
        if (!isKeyInUse(key) && !LuxxlePreferenceKeys.isLuxxleKeyInUse(key)) {
            // Do nothing here for now. We might reconsider it in the future
            // in case we want to make some kind of sanitation of pref keys
            // used in Luxxle.
            // Upstreams function isKeyInUse is called here and Luxxle's
            // LuxxlePreferenceKeys.isLuxxleKeyInUse is just a stub method
        }
    }
